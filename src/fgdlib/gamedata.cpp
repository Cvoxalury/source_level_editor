	...
	while (1)
	{
			...
			)
		{
			...
			else
			{
...
#ifdef SLE // extend class for fgd
				else if ( IsToken(szToken, "extendclass") )	// Can append or override params for existing classes
				{
					pNewClass->SetExtendClass(true);
				}
#endif
				...
				{
#ifdef SLE // extend class for fgd
					// for extend classes, append their info to the matching existing class's data
					if ( pNewClass->IsExtendClass())
					{
					//	GDError(tr, "Found extend declaration for %s", pExistingClass->GetName());

						// copy the original class's variables into the extend class
						pExistingClass->AppendClass(pNewClass);

						// replace the original class with the extended one.
						m_Classes.InsertAfter(nExistingClassIndex, pExistingClass);
				...
				{
#ifdef SLE // extend class for fgd
					if ( pNewClass->IsExtendClass() ) // if an extend class doesn't match anything existing, ignore it.		
					{
					//	GDError(tr, "Found extend declaration for %s, but no such existing class!", pNewClass->GetName());
					}
					else
#endif

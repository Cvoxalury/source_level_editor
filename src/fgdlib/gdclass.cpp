...
GDclass::GDclass(void)
{
	...
	m_bNPC = false;
	m_bFilter = false;
#ifdef SLE // extend class for fgd
	m_bExtend = false;
#endif
...
{
	int iAppendIndex;
	Parent->ClassForName(pAppend->GetName(), &iAppendIndex);

	ConMsg("Appending from class %s, index %d", pAppend->GetName(), iAppendIndex);
	
	// add the helpers
	for ( int h = 0; h < pAppend->GetHelperCount(); h++ )
	{
		AddHelper(pAppend->GetHelper(h));
	}

	// add the I/O
	for ( int i = 0; i < pAppend->GetInputCount(); i++ )
	{
		AddInput(pAppend->GetInput(i));
	}

	for ( int o = 0; o < pAppend->GetOutputCount(); o++ )
	{
		AddOutput(pAppend->GetOutput(o));
	}

	// add the variables
	for ( int v = 0; v < pAppend->GetVariableCount(); v++ )
	{
		//GDinputvariable *pVar = pAppend->GetVariableAt(v);
		//if ( pVar != NULL )
		//{
		//m_Variables.AddToTail(pVar);
		//}
		AddVariable(pAppend->GetVariableAt(v), this, -1, m_Variables.Count(), true); // -1 signifies a new variable
	}
}
#endif
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
#if 1 //def SLE // extend class for fgd
BOOL GDclass::AddVariable(GDinputvariable *pVar, GDclass *pBase, int iBaseIndex, int iVarIndex, bool allowTypeOverride)
// allow type override for extend classes - one of the features is updating existing KVs with new types like browsable
// names (for skies, cables, soundscapes...) or replacing 'string' with 'target_destination', etc
#else
...
#endif
{
	...
	if (pThisVar != NULL)
	{
		...
		//
#if 1 //def SLE // extend class for fgd
		// allow override for extend classes
		if (pThisVar->GetType() != pVar->GetType() && !allowTypeOverride)
#else

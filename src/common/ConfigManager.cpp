...
bool CGameConfigManager::LoadConfigsInternal( const char *baseDir, bool bRecursiveCall )
{
	...
	{
#ifdef SLE
		// Try importing an existing config
...
{
#ifdef SLE
	return pIn->LoadFromFile(g_pFullFileSystem, DEFAULT_GAME_BLOCK_FILENAME, "EXECUTABLE_PATH");
#else
...
bool CGameConfigManager::LoadConfigsInternal(const char *baseDir)
{
	...
	if (!m_pData->LoadFromFile(g_pFullFileSystem, GAME_CONFIG_FILENAME, "EXECUTABLE_PATH"))
	{
		// Try importing an existing config
		if (!m_pData->LoadFromFile(g_pFullFileSystem, IMPORT_GAME_CONFIG_FILENAME, "EXECUTABLE_PATH"))
		{
...
bool CGameConfigManager::CreateDefaultConfig(void)
{
	...
	configBlock->SaveToFile(g_pFullFileSystem, GAME_CONFIG_FILENAME, "EXECUTABLE_PATH");
...
bool CGameConfigManager::SaveConfigs(const char *baseDir)
{
	...
	return m_pData->SaveToFile(g_pFullFileSystem, GAME_CONFIG_FILENAME, "EXECUTABLE_PATH");
}
...
bool CGameConfigManager::ResetConfigs(const char *baseDir /*= NULL*/)
{
	...
	// Delete the file
	g_pFullFileSystem->RemoveFile(GAME_CONFIG_FILENAME, "EXECUTABLE_PATH");
	if (g_pFullFileSystem->FileExists(GAME_CONFIG_FILENAME, "EXECUTABLE_PATH"))
		return false;
...
bool CGameConfigManager::GetDefaultGameBlock(KeyValues *pIn)
{
	return pIn->LoadFromFile(g_pFullFileSystem, DEFAULT_GAME_CONFIG_FILENAME, "EXECUTABLE_PATH");
}
#endif

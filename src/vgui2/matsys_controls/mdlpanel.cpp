...
CMDLPanel::CMDLPanel( vgui::Panel *pParent, const char *pName ) : BaseClass( pParent, pName )
{
	...
	m_bShowAllAttachments = false; //// SLE NEW - attachments tab
	m_nSelectedAttachment = -1;
...
void CMDLPanel::SetShowAllAttachments(bool bShow)
{
	m_bShowAllAttachments = bShow;
}

void CMDLPanel::SetSelectedAttachmentId(int id)
{
	m_nSelectedAttachment = id;
}
...
//	pOverrideMaterial = NULL;
	...
	if (m_bShowAllAttachments || m_nSelectedAttachment != -1) //// SLE NEW - attachments tab
	{
		studiohdr_t *hdr = vgui::MDLCache()->GetStudioHdr(m_RootMDL.m_MDL.GetMDL());
		if (!hdr)
			return;
		...
		{
			mstudioattachment_t &pattachments = (mstudioattachment_t &)hdr->pAttachment(i);

			matrix3x4_t m;
			ConcatTransforms(pBoneToWorld[hdr->GetAttachmentBone(i)], pattachments.local, m);
			
			CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
			IMesh* pMesh = pRenderContext->GetDynamicMesh(true, NULL, NULL, m_Wireframe);
			CMeshBuilder meshBuilder;

			if (m_nSelectedAttachment != -1)
			{
				if (i != m_nSelectedAttachment) continue;
			}
			...
			for (int k = 0; k < 3; k++)
			{
				...
				meshBuilder.Begin(pMesh, MATERIAL_LINE_STRIP, 1);

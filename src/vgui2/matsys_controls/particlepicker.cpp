	...
	BaseClass(pParent,pName)
{
	...
	m_pToolPanel->SetSizer( new CBoxSizer(ESLD_HORIZONTAL) ); // disable for now
	...
	m_pToolPanel->GetSizer()->AddPanel( m_pPreviewCheckbox, SizerAddArgs_t() ); // disable for now
...
void CParticleSnapshotGrid::MapSystemRelatives( )
{
	...
	for ( int i = 0; i < g_pParticleSystemMgr->GetParticleSystemCount(); i++ )
	{
		...
		CUtlReference<CParticleSystemDefinition> pPotentialParent = g_pParticleSystemMgr->FindParticleSystem( pPotentialParentName );
		...
		{
			CUtlReference<CParticleSystemDefinition> pChild = NULL;
...
// Open the manifest file, and read the particles specified inside it
	KeyValues *manifest = new KeyValues(manifestPath);
	if (manifest->LoadFromFile(g_pFullFileSystem, manifestPath, "GAME"))
	{
		for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
		{
			if (!Q_stricmp(sub->GetName(), "file"))
			{
				pcfList.AddToTail(sub->GetString());
				continue;
			}

			Warning("CParticleMgr::Init:  Manifest '%s' with bogus file type '%s', expecting 'file'\n", manifestPath, sub->GetName());
		}
	}
	else
	{

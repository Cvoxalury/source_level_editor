...
#ifdef SLE //// SLE CHANGE - always add a new camera via tool, then set active tool to pointer.
		// Done here to ensure existing cameras have been counted, and not add any unless there's no cameras at all.
		// There's a counterpart of it in MapDoc.cpp's Initialize() to spawn a camera for newly made documents.
	//	AddNewCameraExternal();
#endif
	}
...
#ifdef SLE //// SLE NEW - function to add a new camera when initialising a new MapDoc
void Camera3D::AddNewCameraExternal()
{
	if( Cameras.Count() > 0) return;
	//
	// Create a new camera.
	//
	m_vOrgPos = vec3_origin;
	m_MoveCamera.position[ MovePos ] = vec3_origin;
	m_MoveCamera.position[ MoveLook ] = vec3_origin;
	m_nMovePositionIndex = MoveLook;

	// set as active camera
	AddCamera(m_MoveCamera);
	m_iActiveCamera = 0;
	EnsureMaxCameras();
}
#endif

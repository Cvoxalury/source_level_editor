...
// Purpose: 
//// SLE TODO: 
// 1. Fix lightmap allocation/offset on bumped surfaces. Currently bumpmaps need to be disabled.
...
#ifdef SLE
	Term();
	if (m_pBSPLightingThread) m_pBSPLightingThread->Release();
	m_bLightingInProgress = false;
#endif
	delete this;
}
	...
	for( int iCountFace=0; iCountFace < file.numfaces; iCountFace++ )
	{
			...
			)
		{
			...
			if( !(pTexInfo->flags & SURF_NODRAW) 
				...
				&& !(pTexInfo->flags & SURF_SKIP)
				&& !(pTexInfo->flags & SURF_TRIGGER)
				&& !(pTexInfo->flags & SURF_NOLIGHT)
				/*&& !(pTexInfo->flags & SURF_TRANS )*/
#endif
				)
			{
				++nFaces;
		...
		FOR_EACH_LL( pMat->m_MaterialBufs, iBuf )
		{
			...
			mb.Begin( pBuf->m_pMesh, MATERIAL_TRIANGLES, pBuf->m_nVerts, pBuf->m_nIndices );
				...
				FOR_EACH_LL( pBuf->m_Faces, iFace )
				{
					...
					{
#ifdef SLE  //// SLE TODO - fix bsplighting on displacements
						continue;
#endif
						// It's a displacement.
...
bool CBSPLighting::LoadVRADDLL(char const *pFilename)
{
	...
	if (!m_pVRadDLL->Init(pFilename, g_pFullFileSystem, g_pFileSystem))
	{
		Sys_UnloadModule(m_hVRadDLL);
		return false;
	}
...
void CBSPLighting::CreateDisplacements( CBSPInfo &file, CUtlVector<CFace> &faces, CUtlVector<CDispInfoFaces> &dispInfos )
{
	...
	for( int iFace=0; iFace < faces.Size(); iFace++ )
	{
		...
		//	ddisp_lod_vert_t *pInVert = &file.ddispverts[ pInDisp->m_LODs[0].m_iVertStart + iVert ];
			...
			for( int iCoord=0; iCoord < 2; iCoord++ )
			{
				float flVal = pInVert->m_vLightCoords[iCoord];
				...
				pOutVert->m_vTexCoords[iCoord] = pInVert->m_vTexCoords[iCoord];

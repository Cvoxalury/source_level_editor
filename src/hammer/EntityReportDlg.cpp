...
#endif
{
	...
	if (pParms)
	{
		...
		s_pDlg->m_bFilterReverse = pParms->m_bFilterReverse;
...
#ifdef SLE //// SLE CHANGE - make Entity Report resizable
...
void CEntityReportDlg::UpdateAnchors(CWnd *pPage)
{
	if ( !GetSafeHwnd() )
		return;
	...
	{
		CAnchorDef(IDC_ENTITIES, k_eSimpleAnchorAllSides),
		CAnchorDef(IDC_RADIO2, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_RADIO3, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERBYTYPE, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERBYTYPE, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERBYHIDDEN, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERINSTANCES, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERBYKEYVALUE, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTEREQUALSIGN, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_EXACTVALUE, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERKEY, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERVALUE, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERBYCLASS, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERREVERSE, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERCLASS, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FILTERBOX, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_GOTO, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_DELETE, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_PROPERTIES, k_eSimpleAnchorRightSide),
		CAnchorDef(IDOK, k_eSimpleAnchorBottomRight),
	};
	m_AnchorMgr.Init(GetSafeHwnd(), anchorDefs, ARRAYSIZE(anchorDefs));
}

void CEntityReportDlg::OnSize(UINT nType, int cx, int cy)
{
	m_AnchorMgr.OnSize();
}
#endif
...
#ifdef SLE //// SLE NEW: Let us filter entities that do NOT have the specified value
	DDX_Control(pDX, IDC_FILTERREVERSE, m_cFilterReverse);
#endif
...
#ifdef SLE //// SLE NEW: Let us filter entities that do NOT have the specified value
	DDX_Check(pDX, IDC_FILTERREVERSE, m_bFilterReverse);
#endif
	DDX_Check(pDX, IDC_FILTERBYCLASS, m_bFilterByClass);
#ifdef SLE 
	DDX_Check(pDX, IDC_FILTERBYTYPE, m_iFilterByType);
#else
...
#ifdef SLE //// SLE NEW: Let us filter entities that do NOT have the specified value
	ON_BN_CLICKED(IDC_FILTERREVERSE, OnFilterReverse)
#endif
...
BOOL AddEntityToList(CMapEntity *pEntity, CEntityReportDlg *pDlg)
{
	...
	*/
...
#ifdef SLE
				pWorld->EnumChildrenOfInstance(ENUMMAPCHILDRENPROC(AddEntityToList), DWORD(pDlg), MAPCLASS_TYPE(CMapEntity), pEntity->GetKeyValue("file"));
#else
	...
	{
		CString str;
		str.Format("      (%s)", pEntity->GetKeyValue("targetname"));
		strcat(szString, str);
	}

	//// SLE NEW - keep track of instances for entity repot
	// Append the name of the instance, if inside of one
	if ( pEntity->GetParentInstanceName()[1] )
	{
		CString str;
		str.Format("      (instance: %s)", pEntity->GetParentInstanceName());
		strcat(szString, str);
	}
#endif
	BOOL bAdd = TRUE;
...
#ifdef SLE
//-----------------------------------------------------------------------------
// Purpose:  //// SLE NEW: Let us filter entities that do NOT have the specified value
//-----------------------------------------------------------------------------
void CEntityReportDlg::OnFilterReverse()
{
	m_bFilterReverse = m_cFilterReverse.GetCheck();
	UpdateEntityList();
}
#endif
//-----------------------------------------------------------------------------

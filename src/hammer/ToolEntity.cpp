...
#ifdef SLE //// SLE NEW: different entity tool colours in 2d mode, better contrast
	pRender->SetDrawColor(255, 175, 35);
#else
...
bool CToolEntity::OnLMouseDown3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	if (pObject != NULL)
	{
		...
		if (pSolid == NULL)
		{
#ifdef SLE //// SLE NEW: allow placing objects on top of entities, by placing them in front of the camera.
			CMapClass *pNewObject = NULL;
			Vector cameraPos, cameraForward;
			pView->GetCamera()->GetViewPoint(cameraPos);
			pView->GetCamera()->GetViewForward(cameraForward);
			VectorNormalize(cameraForward);
			cameraPos = cameraPos + cameraForward * 64;

			if (GetMainWnd()->m_ObjectBar.IsEntityToolCreatingEntity())
			{
				//
				// Entity creation.
				//
				...
				pEntity->SetOrigin(cameraPos);
				pEntity->SetClass(CObjectBar::GetDefaultEntityClass());

				pNewObject = pEntity;
			}
			else if (GetMainWnd()->m_ObjectBar.IsEntityToolCreatingPrefab())
			{
				//
				// Prefab creation.
				//
				unsigned int uConstraints = GetConstraints(nFlags);
				m_pDocument->Snap(cameraPos, uConstraints);

				GetHistory()->MarkUndoPosition(m_pDocument->GetSelection()->GetList(), "New Prefab");

				// Get prefab object
				CMapClass *pPrefabObject = GetMainWnd()->m_ObjectBar.BuildPrefabObjectAtPoint(cameraPos);
		...
		if (pFace->TraceLine( HitPos, HitNormal, vFinalStart, vFinalEnd))
		{
			...
			if ( pNewObject )
			{
				...
				if (GetMainWnd()->m_ObjectBar.UseAlignOnEntityPlacement())
				{
					CMapEntity *pEntity = dynamic_cast<CMapEntity *>(pNewObject);
					if (pEntity != NULL)
					{
...
#ifdef SLE //// SLE NEW: clicked on void - place the object in front of the camera. 
	else
	{
		CMapClass *pNewObject = NULL;
		if (GetMainWnd()->m_ObjectBar.IsEntityToolCreatingEntity())

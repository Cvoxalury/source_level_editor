...
SoundType_t CSoundBrowser::GetSoundType() const
{
	...
	else if ( m_SoundType == 2 )
		return SOUND_TYPE_SCENE;
	else
		return SOUND_TYPE_SOUNDSCAPE;
#else
	else
		return SOUND_TYPE_SCENE;
#endif
...
#ifdef SLE
	g_Sounds.StopSound(); // always stop even without autoplay - user likely would change it if they don't want to keep listening
#endif
	if ( m_Autoplay )
	{
		OnPreview();

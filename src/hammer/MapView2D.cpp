...
#ifdef SLE //// SLE NEW - third mirroring option
	ON_COMMAND_EX(ID_FLIP_BACKFRONT, OnFlip)
#endif
...
void CMapView2D::Render()
{
	...
	{
		int width, height, center_x, center_y, alpha;
		width = pDoc->bg_xy.width_float; // convert to int "needlessly", yes, maybe in the future it can be floats
		height = pDoc->bg_xy.height_float;
		alpha = pDoc->bg_xy.alpha_int;
		center_x = pDoc->bg_xy.center_x_int;
		center_y = pDoc->bg_xy.center_y_int;

		DrawBackgroundImage(GetRender(), pDoc->bg_xy.filename_string, 
			(int)(center_x - width / 2), ( int )(center_y - height / 2), 
			( int )(center_x + width / 2), ( int )(center_y + height / 2),
			pDoc->bg_xy.alpha_int);
	}
	////
#endif
	//
...
{
#ifndef SLE
//	if (nChar == VK_TAB) //// SLE REMOVE: ridiculous idea
//	{

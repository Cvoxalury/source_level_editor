...
#ifdef SLE //// SLE NEW - block detailsprites with detail blockers
#include "mapdoc.h"
#include "MapWorld.h" // for worldspawn-defined detail file
#endif
#include "mapface.h"
#include "camera.h"
#include "options.h"
...
#ifdef SLE 
#define MAX_DETAIL_SPRITES_PER_FACE 655350 
#else
...
void DetailObjects::ParseDetailGroup( int detailId, KeyValues* pGroupKeyValues )
{
	...
	while( pIter )
	{
		if (pIter->GetFirstSubKey())
		{
			...
			{
#ifdef SLE //// SLE NEW: Changed detail params and path because the project uses differently named params
				//// and different path.
				if (!Q_strcmp(FindDetailVBSPName(), "scripts/detailtypes.txt"))
				{
					const char *pSpriteData = pIter->GetString("sprite_coords", 0);
					if (pSpriteData)
					{
						...
						//// SLE NEW: Renamed param.
						pSpriteData = pIter->GetString("sprite_size", 0);
						if (pSpriteData)
						{
						...
						//// SLE NEW: Renamed param.
						model.m_flRandomScaleStdDev = pIter->GetFloat("sprite_scale_random", 0.0f);
				...
				else //// SLE NEW: Legacy support for other mods/games that use detail.vbsp.
#endif
				{
...
const char *DetailObjects::FindDetailVBSPName(void)
{
...
#endif
#ifdef SLE //// SLE NEW: Different path because what the project is using.
	// if doesn't exist, use old file name.
#if 0
	// First, try looking up the detail file in the worldspawn settings.
	// For this to work, DetailObjects::LoadEmitDetailObjectDictionary
	// should happen on level load.
	const char *detailFileName = "detail.vbsp";
	
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	if ( pDoc && pDoc->GetMapWorld() && pDoc->GetMapWorld()->GetKeyValue("detailvbsp") != NULL)
	{
		detailFileName = pDoc->GetMapWorld()->GetKeyValue("detailvbsp");
		if ( detailFileName && detailFileName[ 0 ] )
		{
	...
	return detailFileName;
#endif
// If that didn't work, try the default file. 

	const char *detailFileName = "scripts/detailtypes.txt";
	
	// Load detail object descriptions.
	char szGameDir[_MAX_PATH];
	char szFullPath[MAX_PATH];
	APP()->GetDirectory(DIR_MOD, szGameDir);
	Q_MakeAbsolutePath(szFullPath, MAX_PATH, detailFileName, szGameDir);

	detailFileName = szFullPath;

	if (GetFileAttributes(detailFileName) == 0xFFFFFFFF)
	{
		detailFileName = "detail.vbsp";
	}
	else
	{
		detailFileName = "scripts/detailtypes.txt";
	}
	return detailFileName;
#else
	return "detail.vbsp";
...
void	DetailObjects::BuildAnyDetailObjects(CMapFace *pMapFace)
{
	...
	if ( pDetails )
	{
		...
		int	nPoints = pMapFace->GetPointCount();
#ifdef SLE
		if (nPoints == 0) return; // 0 happens when using the clipping tool...
#endif
...
#ifdef SLE
		Assert(nPoints != 0); // just checked it...
#endif
		faceCenter /= nPoints;
...
void DetailObjects::Render3D(CRender3D *pRender)
{
...
#ifdef SLE //// SLE CHANGE - determine drawing within camera fov, not in a box			
			Vector cameraPos, cameraAxis, posWorld;
			pModel->GetOrigin(posWorld);
			pRender->GetCamera()->GetViewPoint(cameraPos);
			pRender->GetCamera()->GetViewForward(cameraAxis);

			if ( IsPointInCone(posWorld, cameraPos, cameraAxis, cos(pRender->GetCamera()->GetFOV()), fDetailDistance ))
			{
	...
	if ( sprites )
	{
		...
		for ( int i = 0; i < sprites; i++ )
		{
			...
			if ( IsPointInBox( viewPoint, Mins, Maxs ) && IsPointInCone )
				pSprite->DrawSprite3D( pRender, color  );
#endif
		}
		pRender->PopRenderMode();

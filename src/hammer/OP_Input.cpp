...
{
#ifdef SLE //// SLE CHANGE - allow multiselect for inputs
	...
	{
#ifndef SLE //// SLE CHANGE - allow multiselect for inputs // relocated up
		int nCount = m_ListCtrl.GetItemCount();
		CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
#endif
		if (nCount > 0 && pDoc)
...
#if 0
#ifdef SLE //// SLE NEW - draw connection to selected input's origin entity in 3d view
void COP_Input::OnHighlight(void)
{
	int nCount = m_ListCtrl.GetItemCount();
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
		...
		if (nCount > 0 && pDoc)
		{
			for (int nItem = nCount - 1; nItem >= 0; nItem--)
			{
				if (m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED)
				{
					...
					Vector entOrigin, myOrigin;
					pEntity->GetOrigin(entOrigin);
					m_pEntity->GetOrigin(myOrigin);
				//	CRender3D *pRender = GetRender;
				//	pRender->DrawLine(entOrigin, myOrigin);
				}
			}
		}
	}
...
#ifdef SLE //// SLE CHANGE - only consider visible entities
				if ( pTestEntity != NULL && pTestEntity->IsVisible() )
#else
				if (pTestEntity != NULL)
#endif
				{

...
#ifdef SLE 
	//// SLE NEW - arrows for Add Height control
	DDX_Control(pDX, IDC_ADDHEIGHTSPIN, m_cAddHeightSpin);
	DDX_Control(pDX, IDC_ADDHEIGHT, m_cAddHeight);
	//// SLE CHANGE - all controls should update preview
	DDX_Control(pDX, IDC_ANGLE, m_cAngle);
...
#ifdef SLE //// SLE CHANGE - all controls should update preview
void CArchDlg::OnUpdateStartAngle() 
{
	if( IsWindowVisible())
		OnArchPreview();
}

void CArchDlg::OnUpdateAddHeight() 
{
	if( IsWindowVisible())
		OnArchPreview();
}
...
void CArchDlg::OnArchPreview() 
{
	...
	InvalidateRect(NULL);
	UpdateWindow();
#ifdef SLE //// SLE CHANGE - save values during preview so the 2d/3d tool block preview can pick them up
	SaveValues();

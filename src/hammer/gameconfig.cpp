...
void CGameConfig::SetActiveGame(CGameConfig *pGame)
{
	...
	else
	{
		...
		pGD = NULL;
#ifdef SLE //// bump up defaults for Source
		g_MAX_MAP_COORD = 16384;
		g_MIN_MAP_COORD = -16384;
#else
		g_MAX_MAP_COORD = 4096;
		g_MIN_MAP_COORD = -4096;
#endif
...
CGameConfig::CGameConfig(void)
{
	...
	memset(szQuickAccessClass1, 0, sizeof(szQuickAccessClass1));
	memset(szQuickAccessClass2, 0, sizeof(szQuickAccessClass2));
	memset(szQuickAccessClass3, 0, sizeof(szQuickAccessClass3));
	memset(szQuickAccessClass4, 0, sizeof(szQuickAccessClass4));
#endif
	...
	strcpy(m_szCordonTexture, "tools\\toolsskybox");
#ifdef SLE //// SLE CHANGE - add default config paths
//	GDFiles.Add(".\\halflife2.fgd");
//	nGDFiles++;
	strcpy(m_szGameExeDir, ".\\..\\");
	strcpy(szBSPDir, ".\\..\\hl2\\maps");
	strcpy(szMapDir, ".\\..\\hl2\\maps");
	strcpy(m_szModDir, ".\\..\\hl2");
//	strcpy(szDefaultPoint, "info_player_start");
//	strcpy(szDefaultSolid, "func_detail");
	strcpy(szBSP, ".\\vbsp.exe");
	strcpy(szVIS, ".\\vvis.exe");
	strcpy(szLIGHT, ".\\vrad.exe");
	strcpy(szExecutable, ".\\..\\hl2.exe");
...
#ifndef SLE //// SLE REMOVE - irrelevant now, might come back but unlikely
	else
	{
		mapformat = mfQuake;
	}
...
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
	file.read(szQuickAccessClass1, sizeof szQuickAccessClass1);
	file.read(szQuickAccessClass2, sizeof szQuickAccessClass2);
	file.read(szQuickAccessClass3, sizeof szQuickAccessClass3);
	file.read(szQuickAccessClass4, sizeof szQuickAccessClass4);
#endif
	if (fVersion >= 1.2f)
	{
...
bool CGameConfig::Load(KeyValues *pkv)
{
	...
	m_bIsDefault = (pkvHammer->GetInt("IsDefault", 0));
#endif
...
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
	Q_strncpy(szQuickAccessClass1, pkvHammer->GetString("QuickAccessClass1"), sizeof(szQuickAccessClass1));
	Q_strncpy(szQuickAccessClass2, pkvHammer->GetString("QuickAccessClass2"), sizeof(szQuickAccessClass2));
	Q_strncpy(szQuickAccessClass3, pkvHammer->GetString("QuickAccessClass3"), sizeof(szQuickAccessClass3));
	Q_strncpy(szQuickAccessClass4, pkvHammer->GetString("QuickAccessClass4"), sizeof(szQuickAccessClass4));
#endif
	...
	m_MaterialFavouritesCount = pkvHammer->GetInt("MaterialFavouritesCount");
	for ( int j = 0; j < m_MaterialFavouritesCount; j++ )
	{
		sprintf(szFavourites, "-MaterialFavourite%d", j);
		int index = m_MaterialFavourites.AddToTail();
		Q_strncpy(m_MaterialFavourites[ index ].szName, pkvHammer->GetString(szFavourites), sizeof(m_MaterialFavourites[ index ].szName));
		Q_StripTrailingSlash(m_MaterialFavourites[ index ].szName);
		m_MaterialFavourites[ index ].bUserGenerated = true;
	}
#endif
...
#ifdef SLE //// SLE CHANGE - since now default config is composed, nConfigs is never 0; detect instead if it was created from default
	pkvHammer->SetInt("IsDefault", 0); // saved configs are never the default one
#endif
...
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
	pkvHammer->SetString("QuickAccessClass1", szQuickAccessClass1);
	pkvHammer->SetString("QuickAccessClass2", szQuickAccessClass2);
	pkvHammer->SetString("QuickAccessClass3", szQuickAccessClass3);
	pkvHammer->SetString("QuickAccessClass4", szQuickAccessClass4);
#endif
...
#ifdef SLE //// SLE NEW - Favourites for material browser
	char szFavourites[ MAX_PATH ];
	pkvHammer->SetInt("MaterialFavouritesCount", m_MaterialFavouritesCount);
	for ( int i = 0; i < m_MaterialFavouritesCount; i++ )
	{
		sprintf(szFavourites, "-MaterialFavourite%d", i);
		pkvHammer->SetString(szFavourites, m_MaterialFavourites[ i ].szName);
	}
#endif
...
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
	file.write(szQuickAccessClass1, sizeof szQuickAccessClass1);
	file.write(szQuickAccessClass2, sizeof szQuickAccessClass2);
	file.write(szQuickAccessClass3, sizeof szQuickAccessClass3);
	file.write(szQuickAccessClass4, sizeof szQuickAccessClass4);
#endif
}
...
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
	strcpy(szQuickAccessClass1, pConfig->szQuickAccessClass1);
	strcpy(szQuickAccessClass2, pConfig->szQuickAccessClass2);
	strcpy(szQuickAccessClass3, pConfig->szQuickAccessClass3);
	strcpy(szQuickAccessClass4, pConfig->szQuickAccessClass4);
#endif
...
#ifdef SLE //// SLE NEW - Favourites for material browser
	pConfig->m_MaterialFavouritesCount = m_MaterialFavouritesCount;
	for ( int i = 0; i < m_MaterialFavouritesCount; i++ )
	{
		strcpy(m_MaterialFavourites[ i ].szName, pConfig->m_MaterialFavourites[ i ].szName);
	}
#endif
}

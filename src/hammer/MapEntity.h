...
#define ENTITY_FLAG_IS_SKY_CAMERA		8
#endif
...
public:
	...
	virtual bool ShouldAppearInLightingPreview(void)
	{
		...
		if ( ClassNameMatches("scripted")
			...
			|| ClassNameMatches("env")
			|| ClassNameMatches("path")
			|| ClassNameMatches("ambient")
	...
	{
		return ShouldAppearInLightingPreview();
	}

	//// SLE NEW - inform entities of their instances
	void		SetParentInstanceName(const char *pszInstancePath)
	{
		strcpy(m_parentInstancePath, pszInstancePath);
		V_strlower(m_parentInstancePath);
		V_FixSlashes(m_parentInstancePath);
	}
	const char	*GetParentInstanceName(void) { return m_parentInstancePath; }
	char		m_parentInstancePath[ MAX_PATH ];

#endif
#ifdef SLE //// SLE NEW: 3d skybox preview
	void UpdateSkyCameraVariables();
#endif

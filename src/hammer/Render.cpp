...
#ifdef SLE
#include "KeyValues.h" //// for creating the wireframe-noZ material
#endif
...
CRender::CRender(void)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		m_pClippingOverlay[i] = NULL;
		m_pHiddenWire[i] = NULL;
#endif
	}
...
void CRender::PushInstanceData( CMapInstance *pInstanceClass, Vector &InstanceOrigin, QAngle &InstanceAngles )
{
	...
	else if ( m_InstanceSelectionDepth > 0 )
	{
		...
		if( CMapDoc::GetActiveMapDoc()->GetShowInstance() != INSTANCES_SHOW_NOTINT
			&&  CMapDoc::GetActiveMapDoc()->GetShowInstance() != INSTANCES_SHOW_NOTINTALL)
...
#ifdef SLE //// SLE NEW - option to never tint instances
	else if ( pInstanceClass->IsEditable() || CMapDoc::GetActiveMapDoc()->GetShowInstance() == INSTANCES_SHOW_NORMAL || CMapDoc::GetActiveMapDoc()->GetShowInstance() == INSTANCES_SHOW_NOTINTALL )
#else
...
#ifdef SLE //// SLE CHANGE - no eye movement in 3d view
...
#ifdef SLE
	pRenderContext->FogMaxDensity(0);
#endif
...
#ifdef SLE
		/*!GetRequiredMaterial("editor/flatignorez", m_pWireframeNoZ[0]) ||*/
#endif
...
#ifdef SLE	//// SLE NEW - material used for see-thru solid edges
			//// Make selected objects in wireframe visible through other solids and through themselves.
			//// Create the wireframe material from code. It needs only to be wireframe, ignorez, and nocull.
	KeyValues *pVMTKeyValues = new KeyValues("Wireframe");
	pVMTKeyValues->SetInt("$ignorez", 1);
	pVMTKeyValues->SetInt("$nocull", 1);
	pVMTKeyValues->SetString("$color", "[1 1 0.5]");
	m_pWireframeNoZ[0] = g_pMaterialSystem->CreateMaterial("editor/wireframeignorez", pVMTKeyValues);
	if (m_pWireframeNoZ[0])
	{
		m_pWireframeNoZ[0]->AddRef(); // do I need this in Hammer?
	}
...
#ifdef SLE //// SLE NEW - material used for see-thru solid edges
	m_pWireframeNoZ[1] = m_pWireframeNoZ[0];
#endif
	m_pDotted[1] = m_pDotted[0];
	m_pSelectionOverlay[1] = m_pSelectionOverlay[0];
#ifdef SLE //// SLE NEW - some extra overlays	
	m_pClippingOverlay[1] = m_pClippingOverlay[0];
	m_pHiddenWire[1] = m_pHiddenWire[0];
#endif
	return true;	
}
...
//------------------------------------------------------------
// Purpose: draws a 3d arrow formed by a cylinder and a cone
//------------------------------------------------------------
...
{
	int iSubDiv = 16;
	float flAngleStep = 360.0f / iSubDiv;
...
void CRender::DrawPolyLineFilled(int nPoints, const Vector *Points, float opacity /*= 1.0f*/)
{
	// Draw the box bottom, top, and one corner edge.

	meshBuilder.Begin(m_pMesh, MATERIAL_POLYGON, nPoints);

	for (int i = 0; i<nPoints; i++)
	{
		meshBuilder.Position3fv(&Points[i].x);
		meshBuilder.Color4ub(m_DrawColor[0], m_DrawColor[1], m_DrawColor[2], (float)(255 * opacity));
		meshBuilder.AdvanceVertex();
	}
...
#ifdef SLE //// SLE CHANGE - replace loop with if inside with if-gated loop
	if (bWireFrame)
	{
...
void CRender::SetRenderMode(EditorRenderMode_t eRenderMode, bool bForce)
{
	...
	switch(eRenderMode)
	{
		...
		case RENDER_MODE_WIREFRAME_NOZ:
			m_pCurrentMaterial = m_pWireframeNoZ[m_nDecalMode];
			break;
#endif
		...
		case RENDER_MODE_INSTANCE_OVERLAY:
			...
			break;
#ifdef SLE //// SLE NEW - some extra overlays			
		case RENDER_MODE_CLIPPING_OVERLAY:
			m_pCurrentMaterial = m_pClippingOverlay[m_nDecalMode];
			break;		
		case RENDER_MODE_HIDDEN_WIRE:
			m_pCurrentMaterial = m_pHiddenWire[m_nDecalMode];
			break;
#endif
	...
	{
#ifdef SLE //// SLE CHANGE - don't shade faces covered in UnlitGeneric materials such as the sky textures
		const char *faceShaderName = m_pCurrentMaterial->GetShaderName();
		
		bool bDoShading = true;

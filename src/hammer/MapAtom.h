...
enum SkyboxState_t
{
	SKYBOX_NONE = 0, // not part of 3d skybox
	SKYBOX_YES,
};
//// SLE NEW - Freeze/unfreeze 
enum FrozenState_t
{
	FROZEN_NO = 0, // not frozen; can be picked in 3d and 2d
	FROZEN_YES, // can't be picked until unfrozen
};
#endif
//
...
public:
	...
	{
		return m_eSkyboxState == SKYBOX_YES;
	}

	virtual SkyboxState_t GetSkyboxState(void) 
	{
		return(m_eSkyboxState);
	}

	virtual SkyboxState_t SetSkyboxState(SkyboxState_t eSkyboxState)
	{
		SkyboxState_t ePrevState = m_eSkyboxState;
		m_eSkyboxState = eSkyboxState;
		return ePrevState;
	}

	//-----------------------------------------------------------------------------
	//// SLE NEW - editor objects display filter/toggle
	virtual bool IsHiddenAsEditorObject(void) 
	{
		return m_bShownAsEditorObject;
	}
	
	virtual void HideThisEditorObject(void)
	{
		m_bShownAsEditorObject = false;
	}

	virtual void UnhideThisEditorObject(void)
	{
		m_bShownAsEditorObject = true;
	}
	//-----------------------------------------------------------------------------
	//// SLE NEW - Freeze/unfreeze 	
	virtual bool IsFrozen(void) 
	{
		return m_eFrozenState == FROZEN_YES;
	}

	virtual FrozenState_t GetFrozenState(void) 
	{
		return(m_eFrozenState);
	}

	virtual FrozenState_t SetFrozenState(FrozenState_t eFrozenState)
	{
		FrozenState_t ePrevState = eFrozenState;
		m_eFrozenState = eFrozenState;
		return ePrevState;
	}
	//-----------------------------------------------------------------------------
#endif
protected:
	...
	//-----------------------------------------------------------------------------
	// DoTransform functions. Virtual, called by base Transform functions.
	//-----------------------------------------------------------------------------
...
#ifdef SLE
	SkyboxState_t m_eSkyboxState;		//// SLE NEW: 3d skybox preview
	bool m_bShownAsEditorObject;		//// SLE NEW - editor objects display filter/toggle
	FrozenState_t m_eFrozenState;		//// SLE NEW - Freeze/unfreeze 
#endif

...
#ifdef SLE //// SLE NEW - more data types, backported from 2015, for VScript
	// Implementation of IDlgListManageBrowse
		virtual bool BrowseScriptList( CStringList &lstBrowse );
#endif
	// Other functions.
		...
		void CreateSmartControls_BrowseScripts( GDinputvariable *pVar, CRect &ctrlrect, HFONT hControlFont );
#endif
...
#ifdef SLE
		afx_msg void OnEntityHelpOnline(void); //// SLE NEW - online help button
		afx_msg void OnEntityPresetList(); //// SLE NEW - quick access class buttons for setting current entity's class
		afx_msg BOOL OnEntityPreset(UINT uCmd);
		afx_msg BOOL InsertEntityPreset(UINT id);
		//	afx_msg void OnQuickAccess1(void);
		//	afx_msg void OnQuickAccess2(void);
		//	afx_msg void OnQuickAccess3(void);
		//	afx_msg void OnQuickAccess4(void);

		afx_msg void OnSetRandomYaw(void); //// SLE NEW: Button to randomise yaw in already placed objects
		afx_msg void OnFixupAngles(void); //// SLE NEW: Button to round angle values.
#endif
...
#ifdef SLE
		void BrowseSkybox(void);
		bool BrowseParticles(char *szParticleSysName, int length); //// SLE NEW - particle browser
#endif

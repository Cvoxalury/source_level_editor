...
{
#ifdef SLE //// SLE CHANGE - rmf export that can be loaded in WC 3.5
	float fVersion = 2.2f;
#else
	float fVersion = 3.7f;
#endif
	...
	if(fIsStoring)	
	{
...
#if 0 //def SLE //// SLE RMF - bring back serialising visgroups for RMFs
		// save groups
		...
		for(int i = 0; i < iSize; i++)
		{
			pGroup = pDoc->VisGroups_GetVisGroup(i);
			if( pGroup->IsAutoVisGroup()) continue; 

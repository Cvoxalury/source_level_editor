...
{
#ifdef SLE //// SLE CHANGE - allow wider FOV
	if ( ( value > 150 ) || ( value < 15 ) )
	{
		AfxMessageBox("FOV must be 15-150.", MB_ICONEXCLAMATION | MB_OK);
		pDX->Fail();
	}
#else
...
#ifdef SLE
	DDX_Check(pDX, IDC_MISSING_FACES_CHECKERBOARD, Options.view3d.bMissingMatAsError); //// SLE NEW - ported from sdk-2013-hammer - display missing texture as emo checkerboard
#else	
	DDX_Text(pDX, IDC_LIGHT_CONE_LENGTH, Options.view3d.fLightConeLength);  //// SLE REMOVE: Does anybody ever use this setting? 
#endif
	DDV_FOVRange(pDX, Options.view3d.fFOV);
	//}}AFX_DATA_MAP

#ifdef SLE //// SLE NEW: Increased max selectable range.
	m_cBackPlane.SetRange(2000, 100000, TRUE);
	m_ModelDistance.SetRange(0, 100000, TRUE);
#else
...
#ifdef SLE //// SLE NEW - ported from sdk-2013-hammer - display missing texture as emo checkerboard
	m_bOldMissingMatAsError = Options.view3d.bMissingMatAsError;
#endif
...
#ifdef SLE //// SLE NEW - ported from sdk-2013-hammer - display missing texture as emo checkerboard
	if ( Options.view3d.bMissingMatAsError != m_bOldMissingMatAsError )
	{
		AfxMessageBox("The changes to the 'Show missing materials as error texture' setting will not take effect for any currently loaded VMFs. Re-open them for the setting to take effect.");
	}
#endif

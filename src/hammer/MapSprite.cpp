...
#ifdef SLE //// SLE CHANGE - prevent sprites from growing too large, for example with the sun the helper
		if (fRadius > 8) fRadius = 8;
#endif
	}
...
CMapClass *CMapSprite::CopyFrom(CMapClass *pObject, bool bUpdateDependencies)
{
	...
	if (pObject != NULL)
	{
		...
		m_bIsClickable = pFrom->m_bIsClickable;
	}
...
void CMapSprite::Render3D(CRender3D *pRender)
{
...
#endif
	int nPasses;
	if ((GetSelectionState() != SELECT_NONE) && (!m_bIsClickable))
	{
	...
	bool render3d = true;
	if (!Options.view3d.bRenderSprites)
	{
		CMapEntity	*mapentity = dynamic_cast<CMapEntity *>(this);
		if (mapentity && !Q_strnicmp(mapentity->GetClassName(), "env_sprite", strlen("env_sprite")))
			render3d = false;
	}
...
#ifdef SLE //// SLE CHANGE - draw icons and sprites normally in both flat modes
				if (pRender->GetCurrentRenderMode() == RENDER_MODE_LIGHTMAP_GRID
					|| pRender->GetCurrentRenderMode() == RENDER_MODE_FLAT)
#else
				if (pRender->GetCurrentRenderMode() == RENDER_MODE_LIGHTMAP_GRID)
#endif
...
#ifdef SLE //// SLE NEW: allow to hide sprites in 3d
			if ( !Options.view3d.bRenderSprites) m_pSpriteInfo->SetScale(0); //// SLE NEW: allow to hide sprites in 3d (only show the little cube and not the glowing texture)
#endif
			float fBlend;

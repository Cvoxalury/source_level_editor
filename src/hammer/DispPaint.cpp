...
bool CDispPaintMgr::PrePaint( SpatialPaintData_t &spatialData )
{
	...
	// sample the mouse pos, it'll be compared to future mouse pos in DoPaintFreeMouse
//	CPoint mousePoint;
//	GetCursorPos(&mousePoint);
//	m_storedMousePoint.Init(mousePoint.x, mousePoint.y);
#endif
	return true;
}
		...
		if ( pDisp )
		{
			...
			if ( PaintSphereDispBBoxOverlap( spatialData.m_vCenter, spatialData.m_flRadius, vBBoxMin, vBBoxMax ) )
			{
				...
				case DISPPAINT_EFFECT_SMOOTH: 
					...
					{
						DoPaintFreeDrag( spatialData, pDisp);
						break;
					}
#endif
				}
...
#ifdef SLE //// SLE NEW - disp free drag brush
void CDispPaintMgr::DoPaintFreeDrag(SpatialPaintData_t &spatialData, CMapDisp *pDisp)
{	
	Vector vPaintPos, vVert;
	float flDistance2;
	...
	for ( int iVert = 0; iVert < nVertCount; iVert++ )
	{
		...
		if ( IsInSphereRadius( spatialData.m_vCenter, spatialData.m_flRadius2, vVert, flDistance2 ) )
		{
			...
			if ( spatialData.m_uiBrushType == DISPPAINT_BRUSHTYPE_SOFT )
			{
				PerformFreeDrag( spatialData, vVert, flDistance2, vPaintPos, false );
			}
			else if ( spatialData.m_uiBrushType == DISPPAINT_BRUSHTYPE_HARD )
			{
				PerformFreeDrag( spatialData, vVert, flDistance2, vPaintPos, true );
			}
			AddToUndo( &pDisp );
			pDisp->Paint_SetValue( iVert, vPaintPos );
...
#ifdef SLE //// SLE NEW - disp free drag brush
void CDispPaintMgr::PerformFreeDrag( const SpatialPaintData_t &spatialData,
							    const Vector &vPos, float flDistance2, Vector &vNewPos, bool hard )
{
	...
	// compare mouse positions in previous sampling and now
	// to determine mouse movement

	dragDelta.x = currentMousePos.x - Options.m_storedMousePos.x;
	dragDelta.y = currentMousePos.y - Options.m_storedMousePos.y;

	Vector horz, vert, depth;	
	pView->GetBestTransformPlane( horz, vert, depth );
			
	Vector delta = horz * dragDelta.x + vert * -dragDelta.y;
	VectorNormalize(delta);
	
	float scale;
	if ( hard )
	{
		scale = spatialData.m_flScalar;
	}
	else
	{		
		scale = 1.0f - ( flDistance2 * spatialData.m_flOORadius2 );
		scale *= spatialData.m_flScalar;
	}

	delta *= scale;
	VectorAdd( vPos, delta, vNewPos );
}
#endif

...
#include "datamodel/idatamodel.h" //// SLE NEW - for commentary editor
...
#ifdef SLE //// SLE NEW - used for example for the grid size, which is a float now
#define KeyFloat( key, dest ) \
	if (stricmp(szKey, key) != 0) \
		; \
	else \
	{ \
		CChunkFile::ReadKeyValueFloat(szValue, dest); \
	}
#endif
#define MAX_REPLACE_LINE_LENGTH 256
#ifdef SLE //// SLE CHANGE - make it less dense
#define LOGICAL_SPACING 1000
#else
...
BEGIN_MESSAGE_MAP(CMapDoc, CDocument)
...
#ifdef SLE
	ON_COMMAND(ID_MAP_ROTATIONPIVOT, OnMapUseRotationPivot) //// SLE NEW: rotation pivot
	ON_UPDATE_COMMAND_UI(ID_MAP_ROTATIONPIVOT, OnUpdateMapUseRotationPivot)
	ON_COMMAND(ID_MAP_ROTATIONPIVOTCENTER, OnMapCenterRotationPivot)
	ON_COMMAND(ID_MAP_ROTATIONPIVOTRESET, OnMapResetRotationPivot)
	ON_COMMAND(ID_EDIT_PASTE_ALIGN, OnEditPasteAlign) //// SLE NEW: Shift + V pastes the copied object while aligning it on the surface, 3d view only
	ON_UPDATE_COMMAND_UI(ID_EDIT_PASTE_ALIGN, OnUpdateEditPasteAlign)
#endif
	...
	ON_COMMAND(ID_MAP_GRID_0125, OnMapGrid0125)
	ON_COMMAND(ID_MAP_GRID_0250, OnMapGrid0250)
	ON_COMMAND(ID_MAP_GRID_0500, OnMapGrid0500)
	ON_COMMAND(ID_MAP_GRID_1, OnMapGrid1)
	ON_COMMAND(ID_MAP_GRID_2, OnMapGrid2)
	ON_COMMAND(ID_MAP_GRID_4, OnMapGrid4)
	ON_COMMAND(ID_MAP_GRID_8, OnMapGrid8)
	ON_COMMAND(ID_MAP_GRID_16, OnMapGrid16)
	ON_COMMAND(ID_MAP_GRID_32, OnMapGrid32)
	ON_COMMAND(ID_MAP_GRID_64, OnMapGrid64)
	ON_COMMAND(ID_MAP_GRID_128, OnMapGrid128)
	ON_COMMAND(ID_MAP_GRID_256, OnMapGrid256)
	ON_COMMAND(ID_MAP_GRID_512, OnMapGrid512)
	ON_COMMAND(ID_MAP_GRID_1024, OnMapGrid1024)
	ON_COMMAND(ID_MAP_GRID_2048, OnMapGrid2048)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_0125, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_0250, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_0500, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_1, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_2, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_4, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_8, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_16, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_32, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_64, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_256, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_512, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_1024, OnUpdateMapGridChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_GRID_2048, OnUpdateMapGridChoices)
	// and for snap angles
	ON_COMMAND(ID_MAP_SNAPANGLE_025, OnMapSnapAngle025)
	ON_COMMAND(ID_MAP_SNAPANGLE_050, OnMapSnapAngle050)
	ON_COMMAND(ID_MAP_SNAPANGLE_100, OnMapSnapAngle100)
	ON_COMMAND(ID_MAP_SNAPANGLE_250, OnMapSnapAngle250)
	ON_COMMAND(ID_MAP_SNAPANGLE_500, OnMapSnapAngle500)
	ON_COMMAND(ID_MAP_SNAPANGLE_750, OnMapSnapAngle750)
	ON_COMMAND(ID_MAP_SNAPANGLE_15, OnMapSnapAngle15)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_025, OnUpdateMapAngleChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_050, OnUpdateMapAngleChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_100, OnUpdateMapAngleChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_250, OnUpdateMapAngleChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_500, OnUpdateMapAngleChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_750, OnUpdateMapAngleChoices)
	ON_UPDATE_COMMAND_UI(ID_MAP_SNAPANGLE_15, OnUpdateMapAngleChoices)
#endif
...
#ifdef SLE
	ON_COMMAND(ID_TOOLS_CREATEINSTANCE, OnToolsCreateInstance) //// SLE NEW - save current selection as instance
	ON_UPDATE_COMMAND_UI(ID_TOOLS_CREATEINSTANCE, OnUpdateEditSelection) //// SLE NEW - save current selection as instance
#endif
...
#ifdef SLE
	ON_COMMAND(IDC_KEEPPASTEGROUP, OnToggleKeepPasteGroup) //// SLE NEW - option to copy-paste while keeping the visgroup
	ON_UPDATE_COMMAND_UI(IDC_KEEPPASTEGROUP, OnUpdateKeepPasteGroup)
	ON_COMMAND(IDC_KEEPCLONEGROUP, OnToggleKeepCloneGroup) //// SLE NEW - menu toggle for keeping visgroup on clone-dragging
	ON_UPDATE_COMMAND_UI(IDC_KEEPCLONEGROUP, OnUpdateKeepCloneGroup)
#endif
...
#ifdef SLE
	ON_COMMAND(ID_TOOLS_DISPVERTEXLOCK, OnToolsDispVertexLock)  //// SLE NEW: button toggle, works as holding shift w/ moving vertices in disp tool. (locks selection on vertex until you let go lmb)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_DISPVERTEXLOCK, OnUpdateToolsDispVertexLock)
#endif
...
#ifdef SLE
	ON_COMMAND(ID_VIEW_RENDERSPRITESIN3D, OnViewRenderSpritesIn3D) //// SLE NEW: allow to hide sprites in 3d (only show the little cube and not the glowing texture)
	ON_UPDATE_COMMAND_UI(ID_VIEW_RENDERSPRITESIN3D, OnUpdateViewRenderSpritesIn3D)
#endif
...
#ifdef SLE
	ON_COMMAND(ID_SHOW_TOOLBRUSH_FACES, OnShowToolBrushFaces) //// SLE NEW: Tool Brush Texture display filter/toogle.
	ON_UPDATE_COMMAND_UI(ID_SHOW_TOOLBRUSH_FACES, OnUpdateShowToolBrushFaces)
	ON_COMMAND(ID_SHOW_EDITOR_OBJECTS, OnShowEditorObjects) //// SLE NEW: Editor Objects display filter/toogle.
	ON_UPDATE_COMMAND_UI(ID_SHOW_EDITOR_OBJECTS, OnUpdateShowEditorObjects)
	ON_COMMAND(ID_SHOW_TOOLSSKY_FACES, OnShowToolsSkyFaces) //// SLE NEW: ToolsSky Texture display filter/toogle.
	ON_UPDATE_COMMAND_UI(ID_SHOW_TOOLSSKY_FACES, OnUpdateShowToolsSkyFaces)
	ON_COMMAND(ID_PLAY_MODEL_ANIMATIONS, OnPlayModelAnimations) //// SLE NEW: Easy menu key to toggle model animations on/off.
...
#ifdef SLE
	ON_COMMAND(ID_CYCLE_GROUPOBJECTSSOLIDS, OnCycleSelectionMode)
	ON_UPDATE_COMMAND_UI(ID_CYCLE_GROUPOBJECTSSOLIDS, OnUpdateCycleSelectionMode)
#endif
	ON_COMMAND(ID_VSCALE_TOGGLE, OnVscaleToggle)
	ON_COMMAND(ID_MAP_ENTITYREPORT, OnMapEntityreport)
#ifdef SLE
	ON_COMMAND(ID_TOGGLE_15DEGSNAP, OnToggle15DegSnap) //// SLE NEW: quick access via hotkey for the 15 degrees rotation snap option.
	ON_UPDATE_COMMAND_UI(ID_TOGGLE_15DEGSNAP, OnUpdate15DegSnap)
#endif
...
#ifdef SLE //// SLE NEW - option to never tint instances
	ON_COMMAND(ID_TOOLS_INSTANCES_NOTINT, OnToolsInstancesShowNoTint)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_INSTANCES_NOTINT, OnUpdateToolsInstancesShowNoTint)
	ON_COMMAND(ID_TOOLS_INSTANCES_NOTINTALL, OnToolsInstancesShowNoTintAll)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_INSTANCES_NOTINTALL, OnUpdateToolsInstancesShowNoTintAll)
#endif
	...
	ON_COMMAND(ID_VTRANSX_CHANGED, OnChangeVertexscale)
	ON_COMMAND(ID_VTRANSY_CHANGED, OnChangeVertexscale)
	ON_COMMAND(ID_VTRANSZ_CHANGED, OnChangeVertexscale)
	ON_COMMAND(ID_TOOLS_SPRINKLE, &CMapDoc::OnToolsSprinkle) //// SLE NEW - sprinkle function from 2015 Hammer
	ON_UPDATE_COMMAND_UI(ID_TOOLS_SPRINKLE, &CMapDoc::OnUpdateToolsSprinkle)
	ON_COMMAND(ID_UPDATE_RADIUSCULLING, OnRadiusCullingFollowCamera)
	ON_UPDATE_COMMAND_UI(ID_UPDATE_RADIUSCULLING, OnUpdateRadiusCullingFollowCamera)
	ON_COMMAND(ID_SEND_SELECTION_TO_SELECTED_VISGROUP, OnSendSelectionToSelectedVisgroup) //// SLE NEW - hotkey to instantly send selected objects to selected visgroup

	ON_COMMAND(ID_TOOLS_DROPTOFLOOR, OnToolsDropToFloor) //// SLE NEW - simplest physics test
	ON_UPDATE_COMMAND_UI(ID_TOOLS_DROPTOFLOOR, OnUpdateEditSelection)

	ON_COMMAND(ID_EDIT_TO3DSKY, OnEditTo3dSky) //// SLE NEW: 3d skybox preview
	ON_UPDATE_COMMAND_UI(ID_EDIT_TO3DSKY, OnUpdateEditSelection)
	ON_COMMAND(ID_EDIT_UNDO3DSKY, OnEditUndo3dSky)
	ON_UPDATE_COMMAND_UI(ID_EDIT_UNDO3DSKY, OnUpdateEditSelection)

	ON_COMMAND(ID_EDIT_FREEZE_SEL, OnEditFreezeSelected)	//// SLE NEW - Freeze/unfreeze
	ON_UPDATE_COMMAND_UI(ID_EDIT_FREEZE_SEL, OnUpdateEditSelection)
	ON_COMMAND(ID_EDIT_FREEZE_UNSEL, OnEditFreezeUnselected)
//	ON_UPDATE_COMMAND_UI(ID_EDIT_FREEZE_UNSEL, OnUpdateEditSelection)
	ON_COMMAND(ID_EDIT_UNFREEZE, OnEditUnfreeze)
//	ON_UPDATE_COMMAND_UI(ID_EDIT_UNFREEZE, OnUpdateEditSelection)

	ON_COMMAND(ID_VIEW_HOME, OnViewHome) //// SLE NEW - Home brings you to map origin
	ON_COMMAND(ID_VIEW_PLAYERSTART, OnViewPlayerStart) //// SLE NEW - Ctrl + Home cycles you through info_player_start with lowest id
	ON_COMMAND(ID_VIEW_SKYCAMERA, OnViewSkyCamera) //// SLE NEW - Home brings you to map origin
	ON_UPDATE_COMMAND_UI(ID_VIEW_HOME, OnUpdateViewHome)
	ON_UPDATE_COMMAND_UI(ID_VIEW_PLAYERSTART, OnUpdateViewHome)
	ON_UPDATE_COMMAND_UI(ID_VIEW_SKYCAMERA, OnUpdateViewHome)
#else
...
{
public:
	CBackgroundXYRegenerator(unsigned char *ImageData, int Width, int Height, enum ImageFormat Format) :
...
CMapDoc::CMapDoc(void)
{
	...
	m_use_rotation_pivot_bool = false;
	m_rotation_pivot_vec.Init();
#endif

#ifdef SLE //// SLE CHANGE - by default, show them without tint
	...
	bg_xy.alpha_int = 0;
	bg_xy.center_x_int = 0;
	bg_xy.center_y_int = 0;
#endif
}
...
CMapEntity *CMapDoc::CreateEntity(const char *pszClassName, float x, float y, float z)
{
	...
	if (pEntity != NULL)
	{
		...
		if ((strnicmp(pszClassName, "light", 5) == 0))
		{
			OnUpdateLightPreview();
		}
#endif
	}
...
{
	CStrDlg dlg(0, "", "Coordinates to go to (x y z), ex: 100 -200 300\nex: setpos -1 2 3; setang 4 5 6", "Go to coordinates");
	while (dlg.DoModal() == IDOK)
	{
		...
		{
			//// SLE NEW: First change made to see if compiled program works.
			if ((posVec.x == 100) && (posVec.y == -200) && (posVec.z == 300))
			{
...
#ifdef SLE //// SLE CHANGE - set active tool to pointer
	if ( GetTools() )
	{
		GetTools()->SetTool(TOOL_POINTER);
	}
#endif
}
...
ChunkFileResult_t CMapDoc::LoadEntityCallback(CChunkFile *pFile, CMapDoc *pDoc)
{
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		delete pEntity; //// Ported from Hammer-2013
#endif
	return(ChunkFile_Ok);
}
...
ChunkFileResult_t CMapDoc::LoadCordonBoxCallback(CChunkFile *pFile, Cordon_t *pCordon)
{
	...
	return pFile->ReadChunk((KeyHandler_t)LoadCordonBoxKeyCallback, &box);
}
...
ChunkFileResult_t CMapDoc::LoadCordonCallback(CChunkFile *pFile, CMapDoc *pDoc)
{
	...
	CChunkHandlerMap Handlers;
	Handlers.AddHandler("box", (ChunkHandler_t)CMapDoc::LoadCordonBoxCallback, &cordon);

	pFile->PushHandlers(&Handlers);
	ChunkFileResult_t eResult = pFile->ReadChunk((KeyHandler_t)LoadCordonKeyCallback, &cordon);
	pFile->PopHandlers();
...
ChunkFileResult_t CMapDoc::LoadViewSettingsKeyCallback(const char *szKey, const char *szValue, CMapDoc *pDoc)
{
	...
	KeyFloat( "nGridSpacing", pDoc->m_nGridSpacing );
#else
...
#ifdef SLE //// SLE CHANGE - add to MRU right away, else a crash or process termination will not leave the map in it
		SetPathName( pszFileName, TRUE);
#else
		SetPathName( pszFileName, FALSE );
#endif
	}
#ifndef SLE //// SLE REMOVE - saving here is broken now 
	//
...
#ifdef SLE //// SLE CHANGE - load the detail file here so it can be changed during runtime
//	DetailObjects::LoadEmitDetailObjectDictionary(GetGame()->m_szModDir);
#endif
	...
	for ( KeyValues *sub = kvMapSettings->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey() )
	{
		if ( !Q_stricmp(sub->GetName(), GetTitle()) )
		{
			KeyValues *sub_xy = sub->FindKey("background_xy");
			if ( sub_xy )
			{
				bool report = sub_xy->GetBool("report", 0);
				bg_xy.is_enabled_bool = sub_xy->GetBool("enabled");
				if ( bg_xy.is_enabled_bool )
				{
					bg_xy.width_float = sub_xy->GetFloat("width");
					bg_xy.height_float = sub_xy->GetFloat("height");
					bg_xy.alpha_int = sub_xy->GetInt("alpha");
					
					bg_xy.filename_string = sub_xy->GetString("filename");
#if 0
					if ( bg_xy.filename_string )
					{
						...
						if ( report )
						{
							if ( !g_pFileSystem->ReadFile(bg_xy.filename_string, "GAME", buf) )
							{
								CString str;
								str.Format("Cannot read file %s for XY background for map %s!", bg_xy.filename_string, GetTitle());
								AfxMessageBox(str);
							}
						}

						if ( TGALoader::GetInfo(buf, &bg_xy.image_width_int, &bg_xy.image_height_int, &imageFormat, &sourceGamma) )
						{
							if ( bg_xy.image_char )
							{
								delete[] bg_xy.image_char;
							...
							int memRequired = ImageLoader::GetMemRequired(bg_xy.image_width_int, bg_xy.image_height_int, 1, imageFormat, false);
							bg_xy.image_char = new unsigned char[ memRequired ];

							buf.SeekGet(CUtlBuffer::SEEK_HEAD, 0);
							TGALoader::Load(bg_xy.image_char, buf, bg_xy.image_width_int, bg_xy.image_height_int, imageFormat, sourceGamma, false);

							bg_xy.texture_itex = dynamic_cast< ITextureInternal * >( g_pMaterialSystem->CreateProceduralTexture("background_xy", 
								TEXTURE_GROUP_OTHER, bg_xy.image_width_int, bg_xy.image_height_int, IMAGE_FORMAT_BGRX8888,
								TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_PROCEDURAL) );

							ITextureRegenerator *regen = new CBackgroundXYRegenerator(bg_xy.image_char,
								bg_xy.image_width_int, bg_xy.image_height_int,
								imageFormat);
							bg_xy.texture_itex->SetTextureRegenerator(regen);
							bg_xy.texture_itex->Download();
						}
						else
						{
							if ( report )
							{
								CString str;
								str.Format("Cannot load TGA file %s for XY background for map %s!", bg_xy.filename_string, GetTitle());
								AfxMessageBox(str);
							}
						}
					}
#endif
					// read the center coords from the string
					char buf[ 32 ];
					Q_strncpy(buf, sub_xy->GetString("center"), sizeof(buf));

					char *token = strtok(buf, ",");
					if ( token )
					{
						bg_xy.center_x_int = atoi(token);
						token = strtok(NULL, ",");
						if ( token )
						{
							bg_xy.center_y_int = atoi(token);
						}
					}
					if ( report )
					{
						CString str;
						str.Format("XY background for map %s is enabled; filename %s; width %.1f, height %.1f, alpha %i, center %ix%i", GetTitle(),
							bg_xy.filename_string,
							bg_xy.width_float,
							bg_xy.height_float,
							bg_xy.alpha_int,
							bg_xy.center_x_int,
...
void CMapDoc::ManifestPaste( CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate, CMapClass *pParent, bool bMakeEntityNamesUnique, const char *pszEntityNamePrefix )
{
	...
	GetHammerClipboard()->Objects.PurgeAndDeleteElements();
		...
		if ( pEntity && stricmp( pEntity->GetClassName(), "func_instance" ) == 0 && ( !bOnlySelected || pEntity->GetSelectionState() != SELECT_NONE ) )
		{
			...
			if ( pMapInstance )
			{
				if ( pMapInstance->GetInstancedMap() )
				{
					...
					InstanceCount++;
#ifdef SLE //// SLE CHANGE: make Prefix/Postfix/None rename style actually WORK!
					int renameStyle = -1;
					if (pMapInstance->GetEditorKeyValue("fixup_style") != NULL) renameStyle = atoi(pMapInstance->GetEditorKeyValue("fixup_style"));

					switch (renameStyle)
					{
					case 0:
					{
						if(pMapInstance->GetEditorKeyValue("targetname"))
							sprintf(temp, pMapInstance->GetEditorKeyValue("targetname"));
						else
							sprintf(temp, "");
					}
					break;
...
BOOL CMapDoc::SelectDocType(void)
{
	...
	//
#ifndef SLE //// SLE REMOVE - With Materials tab being removed from config, there's no reason for this obsolete check & message.
			//// SLE TODO - in the future WAD support will likely be added and this'll be relevant again.
...
BOOL CMapDoc::Serialize(std::fstream& file, BOOL fIsStoring, BOOL bRMF, BOOL bCommentaryTXT)
#else
...
#endif
{
	...
	if ( bCommentaryTXT )
	{
		if (m_pWorld->SerializeCommentaryTXT(file, fIsStoring ) < 0)
		{
			AfxMessageBox("There was a file error.", MB_OK | MB_ICONEXCLAMATION);
			return(FALSE);
		}
		else
			return TRUE; // saving as TXT doesn't need the rest to happen
	}
#endif
	if (bRMF) //// SLE UNDONE: bring back RMF loading. Sometimes you just gotta load an old RMF to look at something. 
	{
	...
	else
	{
		...
		if( Cordon_IsCordoning())
			CordonBox = m_Cordons[m_nEditCordon].m_Boxes[m_nEditCordonBox];
#else
...
#ifndef SLE //// SLE REMOVED - not needed now
...
void CMapDoc::ShowNewVisGroupsDialog(CMapObjectList &Objects, bool bUnselectObjects)
{
	...
	if (!pVisGroup)
	{
#ifdef SLE //// SLE CHANGE - do not let users create Auto visgroups through this dialogue
		CString check;
		dlg.GetName(check);
		if ( Q_strcmp(check, "Auto") == 0 || Q_strcmp(check, "auto") == 0 )
		{
			AfxMessageBox("Cannot create an \"Auto\" Visgroup - this name is reserved for editor-created groups only!");
			return;
		}
#endif
		dlg.GetName(str);
...
BOOL CMapDoc::OnOpenDocument(LPCTSTR lpszPathName) 
{
	...
	CMapSolid::PreloadWorld();
#endif
	if ((bRMF) || (bMAP)) //// SLE UNDONE: bring back RMF loading. Sometimes you just gotta load an old RMF to look at something. 
	{
...
BOOL CMapDoc::OnSaveDocument(LPCTSTR lpszPathName) 
{
	...
	BOOL bRMF = FALSE;
	BOOL bMAP = FALSE;
#ifdef SLE //// SLE NEW - allow saving the map as a commentary txt file (or potentially in the future, mapadd)
	BOOL bCommentaryTXT = FALSE;
	if ( !stricmp(lpszPathName + strlen(lpszPathName) - 3, "txt") )
	{
		bCommentaryTXT = TRUE;
		...
		BeginWaitCursor();
		if ( !Serialize(file, TRUE, bRMF, bCommentaryTXT) )
		{
			EndWaitCursor();
			return( FALSE );
		}
		...
		// restore the original extension, or the .txt will be stuck 
...
#ifdef SLE //// SLE NEW: Restore Animation preview (keyframe/move system)
//	GetMainWnd()->GlobalNotify(WM_MAPDOC_CHANGED);
#endif
}
...
static BOOL SelectInBox(CMapClass *pObject, SelectBoxInfo_t *pInfo)
{
	...
	if ( pObject->IsFrozen() )
	{
		return TRUE; // frozen objects can't be picked at all
	}
#endif
...
void CMapDoc::UpdateStatusbar(void)
{
	...
	{
#ifdef SLE //// SLE NEW - display how many faces are nodraw //// SLE TODO - add NULL for HL1 too?
		int nNodraw = 0;
		char szCurrentTexture[MAX_PATH];
		for (int i = 0; i < GetMainWnd()->m_pFaceEditSheet->GetFaceListCount(); i++)
		{
			GetMainWnd()->m_pFaceEditSheet->GetFaceListDataFace(i)->GetTextureName(szCurrentTexture);
			if(!Q_strnicmp(szCurrentTexture, "tools/toolsnodraw", strlen("tools/toolsnodraw")))
				nNodraw++;
		}
		CString str;
		str.Format("%d faces selected, %d are NODRAW", GetMainWnd()->m_pFaceEditSheet->GetFaceListCount(), nNodraw );
#else
			...
			case 1:
				{
					...
					while (p)
					{
						...
						if (pView)
						{
							...
							str += strDist;
#ifdef SLE
							//// SLE NEW - print polygon count, useful info
							int nTris = 0;
							CMapEntity *pEntity = (CMapEntity *)pobj;
							CMapStudioModel *pModel = pEntity->GetChildOfType((CMapStudioModel *)NULL);
							if (pModel != NULL)
							{
								nTris = pModel->GetTriangleCount();

								V_snprintf(strDist, sizeof(strDist), "   [%i tris]", nTris);
								str += strDist;
							}

							//// SLE NEW - print VMF ID
							int id = pobj->GetID();

							V_snprintf(strDist, sizeof(strDist), "   [ID: %i]", id);
							str += strDist;
#endif
							break;
						}
...
#ifdef SLE //// SLE FIX: changing between docs didn't update snap size to the active doc's size, fix that
	if( m_pMapDoc) m_pMapDoc->UpdateStatusBarSnap();
#endif
}
...
#ifdef SLE //// SLE NEW: rotation pivot
void CMapDoc::OnMapUseRotationPivot(void)
{
	m_use_rotation_pivot_bool = !m_use_rotation_pivot_bool;
//	UpdateStatusBarRotationPivot(); // not implemented yet
}

void CMapDoc::OnUpdateMapUseRotationPivot(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(m_use_rotation_pivot_bool);
}

// Purpose: set the rotation pivot to the center of the selection
void CMapDoc::OnMapCenterRotationPivot(void)
{
	if ( GetSelection()->GetCount() > 0 )
	{
		GetSelection()->GetBoundsCenter(m_rotation_pivot_vec);
	}
}

// Purpose: re-init rotation pivot at 0,0,0
void CMapDoc::OnMapResetRotationPivot(void)
{
	m_rotation_pivot_vec.Init();
}
#endif
...
//-----------------------------------------------------------------------------
// Purpose: apply skybox state to selected objects
//-----------------------------------------------------------------------------
void CMapDoc::Edit3dSky(CSelection *m_pSelection, bool undo)
{
	//
	// Build a list of every solid in the selection, whether part of a solid entity or not.
	//
	...
	for ( int i = 0; i < pSelList->Count(); i++ )
	{
		...
		if ( undo )
		{
			if ( pObject->Is3dSkybox() )
			{
				pObject->SetSkyboxState(SKYBOX_NONE);
				...
				if ( pObject->IsMapClass(MAPCLASS_TYPE(CMapSolid)) )
				{
					...
					if ( pSolid )
					{
						pSolid->SetSkyboxState(SKYBOX_NONE);
						int nFaces = pSolid->GetFaceCount();
						for ( int i = 0; i < nFaces; i++ )
						{
							CMapFace *pFace = pSolid->GetFace(i);
							if ( pFace )
								pFace->SetSkyboxState(SKYBOX_NONE);
						}
					}
				}
				/* // todo, finish later
				// for instances, assume the mapper wants all of it to change 3d sky state 
				// (else they would go inside the instance to change it selectively)
				else if ( pObject->IsMapClass(MAPCLASS_TYPE(CMapEntity)) )
				{
					CMapEntity *pEntity = dynamic_cast< CMapEntity* >( pObject );
					...
					CMapInstance *pInstance = pEntity->GetChildOfType(( CMapInstance* )NULL);
					if ( !pInstance )
						return;

					CMapDoc		*pInstDoc = pInstance->GetInstancedMap();
					if ( !pInstDoc )
						return;

					CMapWorld *pWorld = pInstDoc->GetMapWorld();
					EnumChildrenPos_t instPos;
					CMapClass *pChild = pWorld->GetFirstDescendent(instPos);
					while ( pChild != NULL )
					{
						if ( pChild->IsVisible() )
						{
							pChild->SetSkyboxState(SKYBOX_NONE);

							// sigh, now do the faces thing...
						...
						pChild = pWorld->GetNextDescendent(instPos);
					}
				}
				*/
			}
		...
		else
		{
			if ( !pObject->Is3dSkybox() )
			{
				pObject->SetSkyboxState(SKYBOX_YES);
				...
				if ( pObject->IsMapClass(MAPCLASS_TYPE(CMapSolid)) )
				{
					...
					if ( pSolid )
					{
						pSolid->SetSkyboxState(SKYBOX_YES);
						int nFaces = pSolid->GetFaceCount();
						for ( int i = 0; i < nFaces; i++ )
						{
							CMapFace *pFace = pSolid->GetFace(i);
							if ( pFace )
								pFace->SetSkyboxState(SKYBOX_YES);
						}
					}
				}
				/* // todo, finish later
				...
				else if ( pObject->IsMapClass(MAPCLASS_TYPE(CMapEntity)) )
				{
					...
					while ( pChild != NULL )
					{
						if ( pChild->IsVisible() )
						{
							pChild->SetSkyboxState(SKYBOX_YES);
...
void CMapDoc::OnEditTo3dSky(void)
{
	extern GameData *pGD;
	...
	int selectionMode = (int)m_pSelection->GetMode();

	// quick and lame hack. Perform moving objects to selection on both Objects and Solid levels, because they behave differently in regards to cull boxes otherwise.
	m_pSelection->SetMode(selectSolids);
	Edit3dSky(m_pSelection, false);
	m_pSelection->SetMode(selectObjects);
	Edit3dSky(m_pSelection, false);
	m_pSelection->SetMode(selectGroups);
	Edit3dSky(m_pSelection, false);
	m_pSelection->SetMode(( SelectMode_t )selectionMode);
//	SetModifiedFlag();
}

void CMapDoc::OnEditUndo3dSky(void)
{
	extern GameData *pGD;
...
#ifdef SLE //// SLE NEW - Freeze/unfreeze 
//-----------------------------------------------------------------------------
// Purpose: applies selection freeze/unfreeze state
//-----------------------------------------------------------------------------
void CMapDoc::EditFreeze( bool selected, bool freeze )
{	
	EnumChildrenPos_t pos;
	...
	while ( pObject )
	{
		if ( pObject->IsSelected() == selected )
		{
			pObject->SetFrozenState(freeze ? FROZEN_YES : FROZEN_NO);
			
			CMapSolid *pSolid = dynamic_cast< CMapSolid* >( pObject );
			if ( pSolid )
			{
				pSolid->SetFrozenState(freeze ? FROZEN_YES : FROZEN_NO);
				int nFaces = pSolid->GetFaceCount();
				for ( int i = 0; i < nFaces; i++ )
				{
					CMapFace *pFace = pSolid->GetFace(i);
					if ( pFace )
						pFace->SetFrozenState(freeze ? FROZEN_YES : FROZEN_NO);
				}
			}
		}
	...
	// clear the selection if selected objects were frozen
	if ( selected && freeze )
	{
...
void CMapDoc::OnEditFreezeSelected(void)
{
	EditFreeze(true, true);
}

void CMapDoc::OnEditFreezeUnselected(void)
{	
	EditFreeze(false, true);
}

void CMapDoc::OnEditUnfreeze(void)
{	
	EditFreeze(false, false);
}
#endif
//-----------------------------------------------------------------------------
...
bool CMapDoc::DropTraceOnDisplacementsAndClips(const Vector &vPosition, Vector *pHitPosition, bool *pHitClip)
{
	...
	FOR_EACH_VEC(objects, it)
	{
		...
		for ( int nFace = 0; nFace < nFaces; ++nFace )
		{
			...
			{
				// We only care about two kinds of faces: Those with displacements, and non-tool texture faces.
				bool bIsClip = false;
				if ( !pFace->HasDisp() )
				{
					if ( !V_strncmp(pFace->GetTexture()->GetFileName(), "tools/", 6) )
						continue;
...
void CMapDoc::DropTraceObjectRecurse(CMapClass *pObject, const Vector &vTraceStart, CUtlVector< CMapSolid* > &objects)
{
		...
		vMins.z < vTraceStart.z )
	{
		...
		//	CMapClass *pChild = ( CUtlReference< CMapClass > )( *pChildren )[ nChild ];
			CMapClass *pChild = pChildren->Element(nChild);
			Assert(pChild);
...
void CMapDoc::Copy( IHammerClipboard *pClipboard )
{
	...
	for (int i = 0; i < pSelList->Count()	; i++)
	{
		...
		if ( !Options.view2d.bKeepPasteGroup )
#endif
		{
...
//			pParent - 
//-----------------------------------------------------------------------------
#ifdef SLE //// SLE NEW - pasting with aligning on surface //// SLE TODO - not finished
void CMapDoc::Paste(CMapObjectList &Objects, CMapWorld *pSourceWorld, CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate, CMapClass *pParent, bool bMakeEntityNamesUnique, const char *pszEntityNamePrefix, bool bAlignOnSurface)
#else
...
#endif
{
	...
	if (bAlignOnSurface)
	{
#if 0 //// SLE TODO - pasting with align unfinished
		FOR_EACH_OBJ(Objects, pos)
		{
			CMapEntity *pEntity = dynamic_cast<CMapEntity *>(PasteList.Element(pos));
			if (pEntity != NULL)
			{
				Vector xaxis(1.0f, 0.0f, 0.0f);
				Vector yaxis;
				CrossProduct(vFinalHitNormal, xaxis, yaxis);
				...
				CrossProduct(yaxis, vFinalHitNormal, xaxis);
				VectorNormalize(xaxis);

				VMatrix entToWorld;
				entToWorld.SetBasisVectors(xaxis, yaxis, vFinalHitNormal);
				...
				pEntity->GetAngles(angles);
				sprintf(szAngles, "%g %g %g", (double)angles[PITCH], (double)angles[YAW], (double)angles[ROLL]);
				pEntity->SetKeyValue("angles", szAngles);
			}
		}
#endif
...
{
#ifdef SLE //// SLE CHANGE - Ctrl+M opens vertex transform if morph tool is active
	bool bMorphSnap = m_pToolManager->GetActiveToolID() == TOOL_MORPH 
		&& (pCmdUI->m_nID == ID_TOOLS_SNAPSELECTEDTOGRID || pCmdUI->m_nID == ID_TOOLS_TRANSFORM);
#else
...
// Purpose: 
//-----------------------------------------------------------------------------
void CMapDoc::OnEditPasteAlign(void)
{
	BeginWaitCursor();
	GetHistory()->MarkUndoPosition(m_pSelection->GetList(), "Paste");
...
void CMapDoc::OnUpdateEditPasteAlign(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(GetHammerClipboard()->Objects.Count() &&
		(m_pToolManager->GetActiveToolID() != TOOL_FACEEDIT_MATERIAL) &&
		!GetMainWnd()->IsShellSessionActive());
...
void CMapDoc::OnEditSelectall(void)
{
	...
	FOR_EACH_OBJ( *pChildren, pos )
	{
		...
		if (pChild->IsFrozen())
		{
			continue; // frozen objects can't be picked at all
		}
#endif
		if (pChild->IsVisible())
		{
...
void CMapDoc::OnFileSaveAs(void)
{
	...
	do
	{
		...
		if ( m_pManifest )
		{
			pszFilter = "Map Manifest Files (*.vmm)|*.vmm||";
		}
		...
		{
#ifdef SLE //// SLE CHANGE - move saving as .map to export menu
		...
		// Make sure we've got a .vmf extension, or else compile tools won't work.
...
#endif
		{
#ifdef SLE //// SLE CHANGE - there's now more than one Source map format (HL2 + TF2)
			if ( m_pGame->mapformat == mfHalfLife )
#else
			if ( m_pGame->mapformat != mfHalfLife2 )
#endif
			{
				//// SLE CHANGE - rmf export (map doesn't work currently)
...
{
#ifdef SLE //// SLE CHANGE - allow raising grid size to 2048
	if(m_nGridSpacing >= 2048)
#else
...
#ifdef SLE //// SLE NEW: drop-down for all grid sizes
void CMapDoc::OnMapGrid0125()
{
	m_nGridSpacing = 0.125f;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid0250()
{
	m_nGridSpacing = 0.250f;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid0500()
{
	m_nGridSpacing = 0.500f;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid1()
{
	m_nGridSpacing = 1;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid2()
{
	m_nGridSpacing = 2;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid4()
{
	m_nGridSpacing = 4;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid8()
{
	m_nGridSpacing = 8;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid16()
{
	m_nGridSpacing = 16;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid32()
{
	m_nGridSpacing = 32;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid64()
{
	m_nGridSpacing = 64;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid128()
{
	m_nGridSpacing = 128;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid256()
{
	m_nGridSpacing = 256;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid512()
{
	m_nGridSpacing = 512;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid1024()
{
	m_nGridSpacing = 1024;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
}
void CMapDoc::OnMapGrid2048()
{
	m_nGridSpacing = 2048;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
	UpdateStatusBarSnap();
...
void CMapDoc::OnUpdateMapGridChoices(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(false);
	if ( pCmdUI->m_nID == ID_MAP_GRID_0125 && m_nGridSpacing == 0.125f )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_0250 && m_nGridSpacing == 0.250f )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_0500 && m_nGridSpacing == 0.500f )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_1 && m_nGridSpacing == 1 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_2 && m_nGridSpacing == 2 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_4 && m_nGridSpacing == 4 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_8 && m_nGridSpacing == 8 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_16 && m_nGridSpacing == 16 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_32 && m_nGridSpacing == 32 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_64 && m_nGridSpacing == 64 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_128 && m_nGridSpacing == 128 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_256 && m_nGridSpacing == 256 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_512 && m_nGridSpacing == 512 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_1024 && m_nGridSpacing == 1024 )
		pCmdUI->SetCheck(true);
	if ( pCmdUI->m_nID == ID_MAP_GRID_2048 && m_nGridSpacing == 2048 )
		pCmdUI->SetCheck(true);
}

void CMapDoc::OnMapSnapAngle025()
{
	Options.view2d.iRotateConstrainAngle = 0; // 0th pos in the box
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}
void CMapDoc::OnMapSnapAngle050()
{
	Options.view2d.iRotateConstrainAngle = 1;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}
void CMapDoc::OnMapSnapAngle100()
{
	Options.view2d.iRotateConstrainAngle = 2;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}
void CMapDoc::OnMapSnapAngle250()
{
	Options.view2d.iRotateConstrainAngle = 3;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}
void CMapDoc::OnMapSnapAngle500()
{
	Options.view2d.iRotateConstrainAngle = 4;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}
void CMapDoc::OnMapSnapAngle750()
{
	Options.view2d.iRotateConstrainAngle = 5;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}
void CMapDoc::OnMapSnapAngle15()
{
	Options.view2d.iRotateConstrainAngle = 6;
	UpdateAllViews(MAPVIEW_OPTIONS_CHANGED);
}

void CMapDoc::OnUpdateMapAngleChoices(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(false);
	// todo: store them in the document, like the grid?
	
	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_025 && Options.view2d.iRotateConstrainAngle == 0 ) // 0th pos in the box
		pCmdUI->SetCheck(true);

	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_050 && Options.view2d.iRotateConstrainAngle == 1 )
		pCmdUI->SetCheck(true);

	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_100 && Options.view2d.iRotateConstrainAngle == 2 )
		pCmdUI->SetCheck(true);

	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_250 && Options.view2d.iRotateConstrainAngle == 3 )
		pCmdUI->SetCheck(true);

	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_500 && Options.view2d.iRotateConstrainAngle == 4 )
		pCmdUI->SetCheck(true);

	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_750 && Options.view2d.iRotateConstrainAngle == 5 )
		pCmdUI->SetCheck(true);

	if ( pCmdUI->m_nID == ID_MAP_SNAPANGLE_15 && Options.view2d.iRotateConstrainAngle == 6 )
		pCmdUI->SetCheck(true);
...
#ifndef SLE //// SLE REMOVE - has been moved into Tools 
	else
	{
		//
...
void CMapDoc::OnFileRunmap(void)
{
	...
	while (1)
	{
		...
		{
#ifdef SLE //// sometimes gets stuck
	...
	cmd.bEnable = TRUE;
#ifdef SLE //// SLE NEW - process window option for non-expert compile
	cmd.bUseProcessWnd = dlg.m_bUseProcessWnd;
#else
	...
	if ((dlg.m_iQBSP) && (m_pGame->szBSP[0] != '\0'))
	{
		...
		sprintf(cmd.szParms, "-game $gamedir %s%s%s$path\\$file",
			dlg.m_iQBSP == 2 ? "-onlyents " : "",
			dlg.m_bBSPAllow ? "-allowdynamicpropsasstatic " : "",
			dlg.m_bBSPNoCubemap ? "-nodefaultcubemap " : "");
#else
		sprintf(cmd.szParms, "-game $gamedir %s$path\\$file.$ext", dlg.m_iQBSP == 2 ? "-onlyents " : "");
#endif
		
#ifndef SLE //// deprecated, Quake - but may come back one day		
...
#ifdef SLE  //// SLE NEW - extra normal compile options
		CString strThreads;
		if ( !dlg.m_strVisThreads.IsEmpty() && atoi(dlg.m_strVisThreads) > 0 )
		{
			strThreads.Format("-threads %i ", atoi(dlg.m_strVisThreads));
		} 
		else
			strThreads.Format("");
	...
	if ((dlg.m_iLight) && (m_pGame->szLIGHT[0] != '\0'))
	{
...
#ifdef SLE  //// SLE NEW - extra normal compile options
		CString strVrad;
		if ( dlg.m_bVRadLDR && !dlg.m_bHDRLight )
			strVrad.Format("-ldr");
		else if ( !dlg.m_bVRadLDR && dlg.m_bHDRLight )
			strVrad.Format("-hdr");
		else if ( dlg.m_bVRadLDR && dlg.m_bHDRLight )
			strVrad.Format("-both");
		else
		...
		CString strThreads;
		if ( !dlg.m_strVRadThreads.IsEmpty() && atoi(dlg.m_strVRadThreads) > 0 )
		{
			strThreads.Format("-threads %i ", atoi(dlg.m_strVRadThreads));
		} 
		else
			strThreads.Format("");
		...
		sprintf(cmd.szParms, "%s%s -game $gamedir %s%s%s%s%s%s$path\\$file", strThreads, strVrad, 
			...
			dlg.m_bVRadSPL ? "-staticproplighting " : "",
			dlg.m_bVRadSPP ? "-staticproppolys " : "",
			dlg.m_bVRadNOSR ? "-noskyboxrecurse " : "");
#else
...
void CMapDoc::UpdateTitle(CView *pView)
{
	...
	if (pView3D != NULL)
	{
		switch (pView3D->GetDrawType())
		{
			...
			case VIEW3D_LIGHTING_PREVIEW: // 2 //// SLE NEW: BSP Lighting
			{
...
#ifdef SLE //// SLE CHANGE, the option was renamed in the UI
	pCmdUI->SetCheck(m_bHideItems ? FALSE : TRUE);
#else
...
void CMapDoc::OnUpdateToolsHideEntityNames(CCmdUI *pCmdUI) 
{
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
#ifdef SLE //// SLE CHANGE, the option was renamed in the UI
	pCmdUI->SetCheck(CMapEntity::GetShowEntityNames() ? TRUE : FALSE);
#else
...
void CMapDoc::SelectFace(CMapSolid *pSolid, int iFace, int cmd, bool manual /*=false*/)
#else
...
#ifdef SLE //// SLE CHANGE - add argument for manual vs automatic clicking, because of m_pickByAngle_bool
	pSheet->ClickFace( pSolid, iFace, uFaceCmd, nClickMode, manual );
#else
...
#ifdef SLE //// SLE CHANGE - don't change the camera's angles when centering 3d view
		// todo: make into a setting?
		pView3D->SetCamera(vPos, vec3_invalid);
#else
...
#ifdef SLE //// SLE NEW - Home brings you to map origin
void CMapDoc::OnViewHome(void)
{
	Center2DViewsOn(vec3_origin);
	Center3DViewsOn(vec3_origin);
}

//// SLE NEW - Ctrl + Home cycles you through info_player_start starting with lowest id
void CMapDoc::OnViewPlayerStart(void)
{
	if ( !GetMapWorld() ) return;

	Vector playerStartPos = vec3_invalid;

	EnumChildrenPos_t pos;
	CMapClass *pChild = GetMapWorld()->GetFirstDescendent(pos);
	CMapEntityList playerStartsVisited;
	while ( pChild != NULL )
	{
		if ( pChild->IsVisible() )
		{
			CMapEntity *pEntity = dynamic_cast < CMapEntity * > ( pChild );
			if (pEntity != NULL && 
				(pEntity->ClassNameMatches("info_player_start") ||
				pEntity->ClassNameMatches("info_player_deathmatch") ||
				pEntity->ClassNameMatches("info_player_rebel") || // HL2: DM
				pEntity->ClassNameMatches("info_player_combine") || // HL2: DM
				pEntity->ClassNameMatches("info_player_coop"))
				&& !(playerStartsVisited.HasElement(pEntity)))
			{
				playerStartsVisited.AddToTail(pEntity);
			}
		}
		pChild = GetMapWorld()->GetNextDescendent(pos);
	}

	if ( playerStartsVisited.Count() < 1 )
	{		
		AfxMessageBox("No valid player start entities were found", MB_OK);
		return;
	}

	CMapEntity* currentPlayerStart = playerStartsVisited.Element(m_lastPlayerStartID);

	if ( !currentPlayerStart ) return;

	currentPlayerStart->GetOrigin(playerStartPos);

	m_lastPlayerStartID++;

	if ( m_lastPlayerStartID >= playerStartsVisited.Count() ) // reset the loop to the beginning
	{
		m_lastPlayerStartID = 0;
	}

	if ( playerStartPos != vec3_invalid )
	{
		Center2DViewsOn(playerStartPos);
		Center3DViewsOn(playerStartPos);
	}
}

//// SLE NEW - Ctrl + Shift + Home cycles you through sky cameras starting with lowest id
void CMapDoc::OnViewSkyCamera(void)
{
	if ( !GetMapWorld() ) return;

	Vector skyCameraPos = vec3_invalid;

	EnumChildrenPos_t pos;
	CMapClass *pChild = GetMapWorld()->GetFirstDescendent(pos);
	CMapEntityList skyCamerasVisited;
	while ( pChild != NULL )
	{
		if ( pChild->IsVisible() )
		{
			CMapEntity *pEntity = dynamic_cast < CMapEntity * > ( pChild );
			if ( pEntity != NULL && pEntity->ClassNameMatches("sky_camera"))
			{
				skyCamerasVisited.AddToTail(pEntity);
			}
		}
		pChild = GetMapWorld()->GetNextDescendent(pos);
	}

	if ( skyCamerasVisited.Count() < 1 )
	{
		AfxMessageBox("No valid sky cameras were found", MB_OK);
		return;
	}

	CMapEntity* currentSkyCamera = skyCamerasVisited.Element(m_lastPlayerStartID);

	if ( !currentSkyCamera ) return;

	currentSkyCamera->GetOrigin(skyCameraPos);

	m_lastSkyCameraID++;

	if ( m_lastSkyCameraID >= skyCamerasVisited.Count() ) // reset the loop to the beginning
	{
		m_lastSkyCameraID = 0;
	}

	if ( skyCameraPos != vec3_invalid )
	{
		Center2DViewsOn(skyCameraPos);
		Center3DViewsOn(skyCameraPos);
	}
}

void CMapDoc::OnUpdateViewHome(CCmdUI *pCmdUI)
{
	pCmdUI->Enable();
}
#endif
...
void CMapDoc::OnToolsHollow(void)
{
	...
	if (abs(iWallWidth) < 1)
	{
		AfxMessageBox("The width of the walls must be <= -1 or >= 1.");
		return;
	}
...
void CMapDoc::OnEditPastespecial(void)
{
	...
	else
	{
		...
		vecPasteOffset[0] = dlg.m_fOffsetX;
		vecPasteOffset[1] = dlg.m_fOffsetY;
		vecPasteOffset[2] = dlg.m_fOffsetZ;
#else
	...
	if ( dlg.m_bCenterOnCamera )
	{
		...
		while(p)
		{
			...
			Vector origin;
			pView3D->GetCamera()->GetViewPoint(origin);

			vecPasteOffset = origin;
			vecPasteOffset -= GetHammerClipboard()->vecOriginalCenter;

			if ( dlg.m_bUseCameraAngles )
			{
				int cameraYaw;
				float cameraPitch = pView3D->GetCamera()->GetPitch();
				cameraYaw = (-90 - (int)(pView3D->GetCamera()->GetYaw())) % 360;
			//	if (cameraYaw < 180 && cameraYaw > -360) cameraYaw += 360;
			//	if (cameraYaw < 0 && cameraYaw > -180) cameraYaw += 360;

				vecPasteAngles[1] = -cameraPitch;
				vecPasteAngles[2] = cameraYaw + 180;
				vecPasteAngles[0] = 0; // camera never has roll
			}
...
#ifdef SLE //// SLE FIX - prevent the crash in the Clipper Tool if the clipped solid was Un-created
	if ( m_pToolManager->GetActiveToolID() == TOOL_CLIPPER || m_pToolManager->GetActiveToolID() == TOOL_CLIPPER_3POINT)
	{
		Clipper3D *clipper = dynamic_cast<Clipper3D*>( m_pToolManager->GetActiveTool() );
		if ( clipper && !clipper->IsEmpty() )
		{
			AfxMessageBox("You must cancel or finish the clipping operation to enable undo/redo.");
			return( TRUE );
		}
	}
#endif
...
BOOL CMapDoc::OnViewHideObjects(UINT nID)
{
	...
	for (int pos = Objects.Count()-1; pos>=0; pos --)
	{
		...
		if (!VisGroups_ObjectCanBelongToVisGroup(pObject))
		{
#ifdef SLE //// SLE CHANGE - presumably faster?
...
#ifdef SLE //// SLE NEW: allow to hide sprites in 3d 
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnUpdateViewRenderSpritesIn3D(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.view3d.bRenderSprites ? 1 : 0);
}

void CMapDoc::OnViewRenderSpritesIn3D(void)
{
	Options.view3d.bRenderSprites = !Options.view3d.bRenderSprites;
	UpdateVisibilityAll();
}
#endif
//-----------------------------------------------------------------------------
...
#ifdef SLE //// SLE NEW: Tool Brush Texture display filter/toogle.
void CMapDoc::OnShowToolBrushFaces(void)
{
	Options.general.bShowToolBrushFaces = !Options.general.bShowToolBrushFaces;
	UpdateAllViews(MAPVIEW_UPDATE_TOOL);
}

void CMapDoc::OnUpdateShowToolBrushFaces(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.general.bShowToolBrushFaces ? 1 : 0);
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
}

//// SLE NEW: Editor objects display filter/toggle.
void CMapDoc::OnShowEditorObjects(void)
{
	Options.general.bShowEditorObjects = !Options.general.bShowEditorObjects;
	// this function separation exists so that instance docs don't cause the options setting
	// to bounce back
	OnShowEditorObjectsInternal();
}

void CMapDoc::OnShowEditorObjectsInternal()
{
	// Scan the map entities and either give them or remove a hiding flag,
	// if they pass the necessary checks
	const CMapEntityList *pEntityList = m_pWorld->EntityList_GetList();
	FOR_EACH_OBJ(*pEntityList, pos)
	{
		CMapEntity	*pMapEntity = dynamic_cast<CMapEntity *>( pEntityList->Element(pos) );
		if ( pMapEntity != NULL )
		{
			if ( pMapEntity->IsPointClass() && (
				pMapEntity->IsNodeClass()
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "scripted_", strlen("scripted_")) ) // scripted_sequence and scripted_sentence
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "logic_", strlen("logic_")) ) // all logic_entities			
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "math_", strlen("math_")) ) // all math_ entities
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "path_", strlen("path_")) ) // path_track, path_corner...		
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "env_", strlen("env_")) ) // env_cubemap, env_shooter, env_physimpact...			
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "point_", strlen("point_")) ) // point_viewcontrol, point_camera, point_teleport...
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "info_", strlen("info_")) ) // info_target, info_lighting...		
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "game_", strlen("info_")) ) // game_ui, game_text...		
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "color_", strlen("info_")) ) // color_correction		
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "assault_", strlen("assault_")) ) // assault entities	
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "ladder_", strlen("ladder_")) ) // assault entities
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "filter_", strlen("filter_")) ) // filter entities
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "phys_", strlen("phys_")) ) // phys_explosion, phys_constraint...
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "test_", strlen("test_")) ) // a few test entities
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "vgui_", strlen("vgui_")) ) // vgui entities
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "ai", strlen("ai")) ) // ai_relationship, ai_goal_*, aischedule_scripted...	
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "ambient", strlen("ambient")) ) // ambient_generics
				|| ( !Q_strnicmp(pMapEntity->GetClassName(), "light", strlen("light")) ) // all light sources				
				) )
			{
				if ( Options.general.bShowEditorObjects )
				{
					pMapEntity->HideThisEditorObject();
				} else
				{
					pMapEntity->UnhideThisEditorObject();

				}
			} else
			{
				pMapEntity->UnhideThisEditorObject(); // failsafe for all non-editor entities
			}

			if ( !Q_strnicmp(pMapEntity->GetClassName(), "env_fog", strlen("env_fog")) // always show these
				|| !Q_strnicmp(pMapEntity->GetClassName(), "env_headcrabcanister", strlen("env_headcrabcanister"))
				|| !Q_strnicmp(pMapEntity->GetClassName(), "info_overlay", strlen("info_overlay"))
				|| !Q_strnicmp(pMapEntity->GetClassName(), "func_instance", strlen("func_instance")) )
			{
				pMapEntity->UnhideThisEditorObject();
			}

			// tell all instances to hide or unhide their editor objects, too
			if ( !Q_strnicmp(pMapEntity->GetClassName(), "func_instance", strlen("func_instance")) )
			{
				CMapInstance *pMapInstance = pMapEntity->GetChildOfType(( CMapInstance * )NULL);
				if ( pMapInstance )
				{
					CMapDoc	*pInstDoc = pMapInstance->GetInstancedMap();
					if ( pInstDoc )
					{
						pInstDoc->OnShowEditorObjectsInternal();
					}
				}
			}
		}
...
void CMapDoc::OnUpdateShowEditorObjects(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.general.bShowEditorObjects ? 0 : 1);
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
}

//// SLE NEW: ToolsSky Texture display filter/toogle.
void CMapDoc::OnShowToolsSkyFaces(void)
{
	Options.general.bShowToolsSkyFaces = !Options.general.bShowToolsSkyFaces;
	UpdateAllViews(MAPVIEW_UPDATE_TOOL);
}

void CMapDoc::OnUpdateShowToolsSkyFaces(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.general.bShowToolsSkyFaces ? 0: 1);
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
}

//// SLE NEW: Easy menu key to toggle model animations on/off.
void CMapDoc::OnPlayModelAnimations(void)
{
	Options.view3d.bAnimateModelsToggle = !Options.view3d.bAnimateModelsToggle;
	UpdateAllViews(MAPVIEW_UPDATE_ANIMATION);
}

void CMapDoc::OnUpdatePlayModelAnimations(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.view3d.bAnimateModelsToggle ? 1 : 0);
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
}

//// SLE NEW - option to render selected solids' edges in wireframe noz
void CMapDoc::OnShowSolidEdgesNoZ(void)
{
	Options.view3d.bSolidsEdgesNoZ = !Options.view3d.bSolidsEdgesNoZ;
	UpdateAllViews(MAPVIEW_UPDATE_TOOL);
}
...
#ifdef SLE  //// SLE NEW - save current selection as instance
//-----------------------------------------------------------------------------
// Purpose: Takes the current selection and saves it as an instance. The user is
//			prompted for a folder under the instance folder in which to place
//			the instance.
//-----------------------------------------------------------------------------
void CMapDoc::OnToolsCreateInstance(void)
{
	if ( m_pSelection->IsEmpty() )
	{
		AfxMessageBox("This feature creates an instance with the selected objects. You must select some objects before you can use it.", MB_ICONINFORMATION | MB_OK);
		return;
	}
	...
	if ( szBaseDir[ 0 ] == '\0' )
	{
		APP()->GetDirectory(DIR_MOD, szBaseDir);
	}
	...
	//
	// Create an instance world to contain the selected items. Add the selected
	// items to the new world.
	//
	CMapWorld *pNewWorld = new CMapWorld(NULL);
	pNewWorld->SetTemporary(TRUE);
	...
	if ( eResult == ChunkFile_Ok )
	{
		...
		if ( eResult == ChunkFile_Ok )
		{
			// HACK: make sure we save it as a normal map, not a prefab
			bool bPrefab = m_bPrefab;
			m_bPrefab = false;
			eResult = SaveVersionInfoVMF(&File);
			m_bPrefab = bPrefab;
...
void CMapDoc::ReplaceTextures(LPCTSTR pszFind, LPCTSTR pszReplace, BOOL bEverything, int iAction, BOOL bHidden, bool bRescaleTextureCoordinates)
{
	...
	if (!info.bMarkOnly)
	{
#ifdef SLE //// SLE NEW - type what it's replaced with
		str.Format("%d textures replaced with %s.", info.nReplaced, pszReplace);
#else
...
#ifdef SLE  //// SLE NEW - option to also keep visgroup when copy-pasting
void CMapDoc::OnToggleKeepPasteGroup()
{
	Options.view2d.bKeepPasteGroup = !Options.view2d.bKeepPasteGroup;
}

void CMapDoc::OnUpdateKeepPasteGroup(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck( Options.view2d.bKeepPasteGroup );
}

void CMapDoc::OnToggleKeepCloneGroup()
{
	Options.view2d.bKeepclonegroup = !Options.view2d.bKeepclonegroup;
}

void CMapDoc::OnUpdateKeepCloneGroup(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck( Options.view2d.bKeepclonegroup );
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
#ifdef SLE //// SLE NEW - simplest physics test
//-----------------------------------------------------------------------------
// Purpose: simplest physics test
//-----------------------------------------------------------------------------
void CMapDoc::OnToolsDropToFloor()
{
	if (m_pSelection->IsEmpty())
		return;
	...
	if (pSelList->Count() == 1)
	{
		...
		if (pEntity )
		{
			...
			if (GetActiveWorld() != NULL)
			{
				...
				EnumChildrenPos_t pos;
				float dist = 1024;
				CMapClass *pChild = GetActiveWorld()->GetFirstDescendent(pos);
				while (pChild != NULL && pChild != pObject)
				{
					CMapSolid *pSolid = dynamic_cast <CMapSolid *> (pChild);
					if ((pSolid != NULL))
					{
						Vector solidOrigin;
						pSolid->GetOrigin(solidOrigin);
						if (solidOrigin.DistTo(vecOrigin) < dist)
						{
							dist = solidOrigin.DistTo(vecOrigin);
							vecMove = solidOrigin - vecOrigin;
						}
					}
					pChild = GetActiveWorld()->GetNextDescendent(pos);
				}

				if (dist <= 128)
				{
					GetHistory()->MarkUndoPosition(pSelList, "Drop to floor");
					pEntity->TransMove(vecMove);
				}
				else
				{
				}
...
void CMapDoc::Snap(Vector &pt, int nFlags)
{
	...
	else if (nFlags & constrainSnap )
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				pt[i] = V_rint(pt[i] / flGridSpacing) * flGridSpacing;
			}
			else
			{
				pt[i] = (pt[i] / flGridSpacing) * flGridSpacing;
			}
#else
			pt[i] = V_rint(pt[i] / flGridSpacing) * flGridSpacing;
#endif
...
void CMapDoc::OnToolsTransform(void)
{
	...
	CTransformDlg dlg;
#ifndef SLE //// SLE NEW - transform dialogue now remembers the mode
	dlg.m_iMode = 0;
#endif
	if(dlg.DoModal() != IDOK)
		return;
	...
	for (int i = 0; i < pSelList->Count(); i++)
	{
		...
		{
#ifdef SLE
			if (dlg.m_bUseLocalTransform)
			{
				pObject->GetOrigin(vOrigin); // in case of local transform, rotate each object around its own origin
			}
#endif
		...
		{
			if (dlg.m_bUseLocalTransform) // in case of local transform, scale each object individually
			{
				pObject->GetOrigin(vOrigin);
			}
			pObject->TransScale(vOrigin, vDelta);
#ifdef SLE //// SLE NEW - relay transform scale to modelscale keyvalues
			if ( dlg.m_bScaleModelEntities )
			{
				//// SLE TODO - what to do with branches/mods that support non-uniform scaling?
				float scale = max(dlg.m_X, max(dlg.m_Y, dlg.m_Z));
				CMapEntity *pEntity = dynamic_cast<CMapEntity *>(pObject);
				if ( pEntity && pEntity->GetKeyValue("modelscale")) // this is a weak safeguard to not have modelscale written into prop physics, which is dangerous/bugged
				{
				//	char *scalechar = "";
				//	sprintf(scalechar, "%.6f", scale);
				//	AfxMessageBox(scalechar, MB_OK);
				//	pEntity->SetKeyValue("modelscale", scalechar);
				//	free(scalechar);

					CString str; 
					str.Format("%g", scale);
					pEntity->SetKeyValue("modelscale", str);
				}
			}
#endif
		}
		...
		{
#ifdef SLE
			if (dlg.m_bUseLocalTransform) // in case of local transform, treat typed in values as direct coordinates
			{
				Vector origin;
				pObject->GetOrigin(origin);
				vAbsValues.x = origin.x * -1 + dlg.m_X;
				vAbsValues.y = origin.y * -1 + dlg.m_Y;
				vAbsValues.z = origin.z * -1 + dlg.m_Z;
				pObject->TransMove(vAbsValues);
			}
			else
#endif
			{
...
#ifdef SLE //// SLE NEW - transform dialogue now remembers the mode
	dlg.SaveValues();
#endif
	SetModifiedFlag();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnToolsDispVertexLock(void)
{
	Options.SetDispVertexLock(!Options.IsDispVertexLockEnabled());
	SetStatusText(SBI_PROMPT, Options.IsDispVertexLockEnabled() ? "Paint Geo Vertex Lock ON" : "Paint Geo Vertex Lock OFF");
}

//-----------------------------------------------------------------------------
// Purpose: //// SLE NEW: button toggle, 
// works as holding shift w/ moving vertices in disp tool. 
// (locks selection on vertex until you let go lmb)
//-----------------------------------------------------------------------------
void CMapDoc::OnUpdateToolsDispVertexLock(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.IsDispVertexLockEnabled());
}
#endif
//-----------------------------------------------------------------------------
...
//	if (GetSelection())
//	{
//		GetSelection()->GetLastValidBounds(mins, maxs);
//	}
//	else
	{
		mins = m_vCordonMins;
...
#ifdef SLE	//// SLE CHANGE. Be able to cycle through Groups-Objects-Solids 
			//// selection mode w/ hotkey. The "ig" button still toggles Groups and Solids.
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnCycleSelectionMode(void)
{
	SelectMode_t eSelectMode = m_pSelection->GetMode();

	//// SLE CHANGE: using the hotkey ("I", was Ctrl+W) cycles through three modes
	// instead of switching betweel Solids and Groups.
	switch (eSelectMode)
	{
	case selectSolids:
...
//-----------------------------------------------------------------------------
// Purpose: //// SLE CHANGE. Be able to cycle through Groups-Objects-Solids 
// selection mode w/ hotkey. The "ig" button still toggles Groups and Solids.
//-----------------------------------------------------------------------------
void CMapDoc::OnUpdateCycleSelectionMode(CCmdUI *pCmdUI) 
{
...
#ifdef SLE //// SLE NEW: quick access via hotkey for the 15 degrees rotation snap option.
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnToggle15DegSnap(void)
{
	Options.view2d.bRotateConstrain = !Options.view2d.bRotateConstrain;
}
void CMapDoc::OnUpdate15DegSnap(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(Options.view2d.bRotateConstrain);
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
#endif
//-----------------------------------------------------------------------------
// Purpose: //// SLE CHANGE - moved from File to Tools because now it works on selections
//-----------------------------------------------------------------------------
...
//	info.bVisOnly = true; //dlg.bVisibles!=0;
	info.nObject = 0;
	info.pWorld = m_pWorld;
	info.fp = fp;
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnFileExportMAP(void) 
{
	static char szBaseDir[MAX_PATH] = "";

	bool bSave = true;
	CString str;
	CString strOriginal = GetPathName(); // will be used to restore it at the end of export
	...
	do
	{
		...
		// Serialising relies on the extension
		CString wantedExtension = ".map";
	...
	*/
	SetPathName(strOriginal, FALSE); // restore the original vmf/rmf name and don't add to MRU
}
#endif
#ifdef SLE //// SLE NEW: add exporting commentary nodes to txt
//-----------------------------------------------------------------------------
// Purpose:
// todo: reduce code duplication here
//-----------------------------------------------------------------------------
void CMapDoc::OnFileExportCommentary(void)
{		
	static char szBaseDir[MAX_PATH] = "";
	...
	do
	{
		...
		char *pszFilter;
		pszFilter = "Commentary Files (*.txt)|*.txt||";
		...
		if ( pos != -1 )
		{
			...
			extension.MakeLower();
			if ( extension != wantedExtension )
				str = str.Left( pos ) + "_commentary" + wantedExtension;
		}
		else
		{	// no extension
			str += wantedExtension;
...
#ifdef SLE //// SLE CHANGE - use for both Source map formats, HL2 and TF2
...
void CMapDoc::AddToAutoVisGroup( CMapClass *pObject )
{
	...
	if ( pObject->IsMapClass( MAPCLASS_TYPE(CMapEntity) ) )
	{
		...
		{
			// Point entities.
#ifdef SLE //// SLE NEW - more auto visgroups
			if (!Q_strnicmp(pMapEntity->GetClassName(), "env_cubemap", strlen("env_cubemap")))
			{
				AddChildGroupToAutoVisGroup(pObject, "Cubemaps", "Entities"); //// SLE NEW: Also count 'light' entities in the Lights auto-group
			}

			if (!Q_strnicmp(pMapEntity->GetClassName(), "light", strlen("light")))
			{
				AddChildGroupToAutoVisGroup(pObject, "Lights", "Entities"); //// SLE NEW: Also count 'light' entities in the Lights auto-group
			}
			...
			if (!Q_strnicmp(pMapEntity->GetClassName(), "logic_", strlen("logic_")))
			{
				AddChildGroupToAutoVisGroup(pObject, "Logic", "Entities");
			}

			AddChildGroupToAutoVisGroup(pObject, "Point Entities", "Entities");
			if (pMapEntity->IsNodeClass()) //// SLE NEW - don't count nodes as separate visgroup, instead they toggle on and off together with Point Entities
			{
			...
			if (!Q_stricmp(pMapEntity->GetClassName(), "func_instance")) //// SLE NEW - add instances autovisgroup
			{
				AddToAutoVisGroup(pObject, "Instances");
			}

			if (!Q_strnicmp(pMapEntity->GetClassName(), "scripted_sequence", strlen("scripted_sequence")))
			{
				AddChildGroupToAutoVisGroup(pObject, "Sequences", "Entities");
			}

			if (!Q_strnicmp(pMapEntity->GetClassName(), "ambient_generic", strlen("ambient_generic")))
			{
				AddChildGroupToAutoVisGroup(pObject, "Sounds", "Entities");
			}

			if (!Q_strnicmp(pMapEntity->GetClassName(), "env_soundscape", strlen("env_soundscape")))
			{
				AddChildGroupToAutoVisGroup(pObject, "Soundscapes", "Entities");
			}
#else
			// Point entities.
...
#ifdef SLE //// SLE NEW - more auto visgroups
			if (pMapEntity->IsClass("func_detail_blocker")) //// SLE NEW: Auto group for detail blockers
			{
				AddChildGroupToAutoVisGroup(pObject, "Detail Blockers", "Entities");
			}
#endif
...
#ifdef SLE //// SLE NEW - more auto visgroups
			if (pMapEntity->IsClass("func_vehicleclip"))
			{
				AddChildGroupToAutoVisGroup(pObject, "Vehicle Clips", "Clips");
			}
#endif
		}
	}
...
#ifdef SLE //// SLE CHANGED - texture on a single face is enough to qualify, don't run repetetive checks on these brushes
		bool bNodrawAdded = false;
		bool bSkyAdded = false;
		bool bBlockLOSAdded = false;
		bool bBlockBulletsAdded = false;
		bool bBlockLightAdded = false;
		bool bNPCClipAdded = false;
		bool bPClipAdded = false;
		bool bClipAdded = false;
		bool bOccluderAdded = false;
		bool bAreaportalAdded = false;
		bool bInvisibleAdded = false;
		bool bLadderAdded = false;
		bool bSkipAdded = false;
		bool bHintAdded = false;
		bool bTriggerAdded = false;
		bool bClipsAdded = false; // parent visgroup, not individual Clip
		bool bBlockAdded = false; // ditto
		bool bInvisiblesAdded = false; // ditto
#endif
...
#ifdef SLE //// SLE CHANGED - adding to auto visgroups has been optimised and some groups deprecated
			if (strstr(buf, "tools/tools"))
			{
				if (!bNodrawAdded) // Single face is enough to add it, no need to check more than that. 
				{
				...
				{				
					...
					if ( !bBlockBulletsAdded )
					{
						if ( strstr(buf, "blockbullets") )
						{
							AddChildGroupToAutoVisGroup(pObject, "Bullets", "Block");
							bBlockBulletsAdded = true;
							continue;
						}
					}					
					if ( !bBlockLightAdded )
					{
						if ( strstr(buf, "blocklight") )
						{
							AddChildGroupToAutoVisGroup(pObject, "Light", "Block");
							bBlockLightAdded = true;
							continue;
						}
					}
				}				
...
ChunkFileResult_t CMapDoc::Cordon_SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo)
{
		...
		return eResult;
	{
		...
		for ( int i = 0; i < m_Cordons.Count(); i++ )
		{
			...
			if ( ( eResult = pFile->EndChunk() ) != ChunkFile_Ok ) // close the "cordon" chunk
				return eResult;
		}
	}
	if ( eResult == ChunkFile_Ok )
	{
		eResult = pFile->EndChunk(); // close the "cordons" chunk
	}
...
#ifdef SLE
	GetMainWnd()->ModifyStyle( FWS_ADDTOTITLE, 0 ); 
	GetMainWnd()->ModifyStyle( 0, FWS_ADDTOTITLE );
#endif
	if (eResult == ChunkFile_Ok)
	{
		CSaveInfo SaveInfo;
...
bool CMapDoc::ShouldObjectBeVisible(CMapClass *pObject, UpdateVisibilityData_t *pData)
{
	...
	//
	if (pObject && pObject->IsClutter() && !Options.GetShowHelpers() )
	{
		return false;
	}
...
#ifdef SLE //// SLE NEW - allow grid as float, for sub-1 grid sizes
	eResult = pFile->WriteKeyValueFloat("nGridSpacing", m_nGridSpacing);
#else
...
void CMapDoc::InternalEnableLightPreview( bool bCustomFilename )
{
	...
	while( viewPos )
	{
		...
		{
			CMapView3D *pView3D = (CMapView3D *)pView;
			pView3D->SetDrawType( VIEW3D_LIGHTING_PREVIEW ); // 2 //// SLE NEW: BSP Lighting
			break;
		}
	}
#endif
...
void CMapDoc::InternalUpdateLightPreview(void )
{
	if ( !GetActiveMapDoc() ) return; // happens when closing the doc
	OnUpdateLightPreview();
}
...
void CMapDoc::OnDisableLightPreview()
{
	...
	while (p)
	{
		...
		if (pView->IsKindOf(RUNTIME_CLASS(CMapView3D)))
		{
			...
			if( pView3D->GetDrawType() == VIEW3D_LIGHTING_PREVIEW ) // was 2 //// SLE NEW: BSP Lighting
...
{
#if 1 //// SLE NEW: Test to see if this cut functionality can work.
	if( m_pBSPLighting )
	{
		POSITION p = GetFirstViewPosition();
		while (p)
		{
			...
			if (pView->IsKindOf(RUNTIME_CLASS(CMapView3D)))
			{
				...
				if( pView3D->GetDrawType() == VIEW3D_LIGHTING_PREVIEW ) // 2 //// SLE NEW: BSP Lighting
					pView3D->SetDrawType( VIEW3D_TEXTURED );
				else if( pView3D->GetDrawType() == VIEW3D_TEXTURED )
					pView3D->SetDrawType( VIEW3D_LIGHTING_PREVIEW ); // 2
			}
		}
	}
	else
...
//// SLE NEW: Restore Animation preview (keyframe/move system)
...
#ifdef SLE //// SLE NEW - option to never tint instances
void CMapDoc::OnToolsInstancesShowNoTint(void)
{
	m_tShowInstance = INSTANCES_SHOW_NOTINT;
	UpdateAllViews( MAPVIEW_RENDER_NOW );
}

void CMapDoc::OnUpdateToolsInstancesShowNoTint(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
	pCmdUI->SetCheck( ( m_tShowInstance == INSTANCES_SHOW_NOTINT ) );
}

void CMapDoc::OnToolsInstancesShowNoTintAll(void)
{
	m_tShowInstance = INSTANCES_SHOW_NOTINTALL;
	UpdateAllViews( MAPVIEW_RENDER_NOW );
}

void CMapDoc::OnUpdateToolsInstancesShowNoTintAll(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
	pCmdUI->SetCheck( ( m_tShowInstance == INSTANCES_SHOW_NOTINTALL ) );
}
#endif
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: SLE NEW - sprinkle function from 2015 Hammer
//-----------------------------------------------------------------------------
...
{
#ifdef SLE //// SLE NEW - prompt user about Radius Culling
	if (Options.general.bRadiusCulling == false)
	{
		if (AfxMessageBox("This will turn on Radius Culling. Is this what you want?", MB_YESNO) == IDNO)
		{
			return;
		}
	}
...
#ifdef SLE //// SLE TODO - doesn't work as intended yet, should be a culling orb that follows the camera
void CMapDoc::OnRadiusCullingFollowCamera(void)
{
	Options.general.bRadiusCullingFollowCamera = !Options.general.bRadiusCullingFollowCamera;
	UpdateVisibilityAll();
}

void CMapDoc::OnUpdateRadiusCullingFollowCamera(CCmdUI * pCmdUI)
{
	pCmdUI->SetCheck(Options.general.bRadiusCullingFollowCamera ? 1 : 0);
	pCmdUI->Enable(!GetMainWnd()->IsShellSessionActive());
}
#endif
#ifdef SLE //// SLE NEW: hotkey to instantly send selected objects to selected visgroup
//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnSendSelectionToSelectedVisgroup(void)
{	
	if (GetMainWnd() && GetMainWnd()->m_FilterControl && GetMainWnd()->m_FilterControl.GetCurrentlySelectedVisgroup() != NULL)
	{
		const CMapObjectList *pSelList = m_pSelection->GetList();
		for (int index = 0; index < pSelList->Count(); index++)
		{
			CMapClass *pObject = pSelList->Element(index);
			pObject->AddVisGroup(GetMainWnd()->m_FilterControl.GetCurrentlySelectedVisgroup());
			GetMainWnd()->m_FilterControl.UpdateGroupList();
			GetMainWnd()->m_FilterControl.UpdateGroupListChecks();
		}
	}
}
#endif

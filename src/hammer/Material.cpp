...
#ifdef HAMMER2013_PORT_TBROWSER_TRANSPARENCY
#include "pixelwriter.h"
#endif
#ifdef HAMMER2013_PORT_TBROWSER_NEWCACHE
#include "vtffile.h"
...
#ifndef SLE //// these are all the header includes that were thrown out for SLE.
#include <process.h>
#include <io.h>
#include <sys\stat.h>
#include <fcntl.h>
...
public:
...
#ifdef HAMMER2013_PORT_TBROWSER_TRANSPARENCY // only consider real translucent/alphatest material to be translucent
		*isTranslucent = entry.m_bIsTranslucent && (pMaterial->GetMaterialVarFlag(MATERIAL_VAR_TRANSLUCENT) || pMaterial->GetMaterialVarFlag(MATERIAL_VAR_ALPHATEST));
#else
	...
	static PreviewImageRetVal_t GetPreviewImage(IMaterial* pMaterial, unsigned char* pData, int width, int height, ImageFormat imageFormat)
	{
		...
		FileHandle_t file;
		if (file = g_pFullFileSystem->Open(name, "rb"))
...
private:
	static const char* GetPreviewImageName(IMaterial* pMaterial)
	{
		...
		if (pMaterial->FindVar("%noToolTexture", &found, false) && found)
			return nullptr;
		if (auto tex = pMaterial->FindVar("%toolTexture", &found, false))
		{
		...
		if (auto tex = pMaterial->FindVar("$baseTexture", &found, false))
		{
			if (tex->GetType() == MATERIAL_VAR_TYPE_STRING)
				return tex->GetStringValue();
		//	if (tex->GetType() == MATERIAL_VAR_TYPE_TEXTURE)
...
#endif // HAMMER2013_PORT_TBROWSER_NEWCACHE
...
{
#ifndef SLE //// SLE CHANGE - this is set in the editor fgd now, and can be disabled there
...
#ifndef SLE //// SLE REMOVE - error icon is no longer used, it was inconsistent
...
#endif
#endif
#ifdef SLE //// SLE REMOVE - error icon is no longer used, it was inconsistent
		Assert( pTranslucentIcon && pOpaqueIcon && pSelfIllumIcon && pBaseAlphaEnvMapMaskIcon);
#else
...
void CMaterial::DrawBitmap( CDC *pDC, RECT& srcRect, RECT& dstRect )
{
	...
	if (m_TranslucentBaseTexture)
	{
		...
		const int boxSize = 32; // size of the grid of the transparent background
		...
		for (int y = 0; y < m_nHeight; ++y)
		{
			...
			for (int x = 0; x < m_nWidth; ++x)
			{
				if ((x & boxSize) ^ (y & boxSize))
					writer.WritePixel(204, 204, 204, 255); // colours of the transparent background
				else
					writer.WritePixel(255, 255, 255, 255);
			}
		}
...
#ifdef SLE
		//// SLE NEW: if a material has detailtype specified, print it over the preview
		// draw chosen vmt data
		if ( true )
		{
			bool findVar;
			IMaterialVar *pMaterialVar = GetMaterial()->FindVar("%detailtype", &findVar, false);
			if ( findVar && pMaterialVar )
			{
				CString str;
				str.Format("has detail '%s'", pMaterialVar->GetStringValue());
				CSize size = pDC->GetTextExtent(str);
			//	pDC->SetBkColor(RGB(0, 0, 255));
				pDC->TextOut(rect.right - ( size.cx + 4 ), rect.top - ( iFontHeight - 8 ), str);
			}
		}

		//// SLE NEW: inform if the material is portalable
		if ( true )
		{
			bool findVar;
			IMaterialVar *pMaterialVar = GetMaterial()->FindVar("%noportal", &findVar, false);
			if ( findVar && pMaterialVar ) //&& (!Q_strcmp(pMaterialVar->GetStringValue(), "0")))
			{
				CString str;
				str.Format("is not portalable");
				CSize size = pDC->GetTextExtent(str);
				//	pDC->SetBkColor(RGB(0, 0, 255));
				pDC->TextOut(rect.right - ( size.cx + 4 ), rect.top - ( iFontHeight - 24 ), str);
			}
		}
#endif
	}
...
{
	bool bFound;
#ifdef SLE //// SLE CHANGE - more inclusive way to find out if water
	IMaterialVar *pVar = m_pMaterial->FindVar( "%compilewater", &bFound, false );
	if ( bFound )
	{
		return true;
	}
...
//-----------------------------------------------------------------------------
// Purpose: utility function for finding variables by name in VMTs
//-----------------------------------------------------------------------------
bool CMaterial::HasVariable(const char* var) const
{
	bool bFound;
	IMaterialVar *pVar = m_pMaterial->FindVar(var, &bFound, false);
	if (bFound)
	{
		return true;
	}
...
bool CMaterial::HasVariableWithValue(const char* var, const char* val) const
{
	bool bFound;
	IMaterialVar *pVar = m_pMaterial->FindVar(var, &bFound, false);
	if (bFound)
	{
		if (!strcmp(val, pVar->GetStringValue()))
			return true;
	}
...
#endif
#ifndef HAMMER2013_PORT_TBROWSER_TRANSPARENCY
//-----------------------------------------------------------------------------
...
#ifdef HAMMER2013_PORT_TBROWSER_TRANSPARENCY // premultiply alpha for the material
//	const auto maxWidth = Min(m_nWidth, 512);
//	const auto maxHeight = Min(m_nHeight, 512);
	const auto size = m_nWidth * m_nHeight * (m_TranslucentBaseTexture ? 4 : 3);
	m_pData = malloc(size);
	Assert(m_pData);
...
{
#ifdef SLE //// SLE CHANGE - our own preferable material config
	int dxlevel = CommandLine()->FindParm("-dxlevel");
	if ( dxlevel < 60 || dxlevel > 95 ) dxlevel = 95;
	pConfig->dxSupportLevel = dxlevel;
	pConfig->ForceHWSync();
	pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_ENABLE_HDR, false);
	pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_FORCE_TRILINEAR, true);
	pConfig->m_nAAQuality = 0;
	pConfig->m_nAASamples = 0;
	pConfig->bEditMode = true;
	pConfig->bShowNormalMap = false;
	pConfig->bCompressedTextures = true;
	pConfig->ForceTrilinear();
//	pConfig->bVisualizeFillRate = true;
	pConfig->SetFlag( MATSYS_VIDCFG_FLAGS_DISABLE_BUMPMAP, false); //// SLE NEW: Enable bump maps in editor. 
	pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_NO_WAIT_FOR_VSYNC, true);
...
#ifdef SLE //// SLE NEW - create framebuffer rendertargets so shaders like Refract can work in-editor
void CreateFramebufferTextures(void)
{
...
#ifdef SLE //// SLE CHANGE - bump up the rendertargets resolution
#define LPR_RT_SIZE 4096
#else
#define LPR_RT_SIZE 512
#endif
	...
	if (! bHaveAllocated )
	{
		...
		for(int idx=0;idx<NELEMS(sg_ExtraFP16Targets);idx++)
			sg_ExtraFP16Targets[idx].Init(
				materials->CreateNamedRenderTargetTextureEx2(
					s_rt_names[idx],
					LPR_RT_SIZE, LPR_RT_SIZE, RT_SIZE_DEFAULT, s_rt_formats[idx],
					MATERIAL_RT_DEPTH_SHARED, 
...
bool CMaterial::Initialize( HWND hwnd )
{
	...
	{
#ifdef SLE	//// SLE NEW - control material cache size with a command line param
		int cacheSize = Options.view3d.iMaterialCacheSize; // .CommandLine()->FindParm("-materialcachesize");
		if (cacheSize <= 0) cacheSize = 256;

		g_pMaterialImageCache = new CMaterialImageCache(cacheSize);
#else		

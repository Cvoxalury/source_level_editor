...
#ifndef HAMMER2013_PORT_KEYBINDS
//
...
CMapView3D::CMapView3D(void)
{
	...
	m_nLastRaytracedBitmapRenderTimeStamp = -1; // unused, should be removed unless there's plans to re-add it later
...
void CMapView3D::DrawWindowTitle(void)
{
...
#ifdef SLE
		case VIEW3D_LIGHTING_PREVIEW: //// SLE NEW: BSP Lighting
		{
...
#ifdef SLE
		//// SLE NEW: Attempt to re-enable "Engine View". So far doesn't do anything.
		case VIEW3D_ENGINE:
		{
			eRenderMode = RENDER_MODE_TEXTURED;
...
void CMapView3D::InitializeKeyMap(void)
{
	...
	BEGIN_KEYMAP("MapView3D")
	{
		...
		{ 
			...
			_ADD_KEY("LOGICAL_KEY_DOWN_NN", LOGICAL_KEY_DOWN);
			_ADD_KEY("LOGICAL_KEY_UP_NN", LOGICAL_KEY_UP);
		}
		...
		ADD_KEY(LOGICAL_KEY_SELNEXT);
		ADD_KEY(LOGICAL_KEY_SELPREV);
		ADD_KEY(LOGICAL_KEY_SELNEXT_ALT);
		ADD_KEY(LOGICAL_KEY_SELPREV_ALT);
		ADD_KEY(LOGICAL_KEY_TOGGLE_MOUSELOOK);
		ADD_KEY(LOGICAL_KEY_BACKPLANE_INCR);
		ADD_KEY(LOGICAL_KEY_BACKPLANE_DECR);
		ADD_KEY(LOGICAL_KEY_MODELDIST_INCR);
		ADD_KEY(LOGICAL_KEY_MODELDIST_DECR);
		ADD_KEY(LOGICAL_KEY_TOGGLE_3DGRID);
		ADD_KEY(LOGICAL_KEY_TOGGLE_CULLBOXES);
		ADD_KEY(LOGICAL_KEY_TOGGLE_STATS);
	}
#else
	if (!Options.view2d.bNudge)
	{
...
#ifdef SLE
		//// SLE NEW: Q and E buttons to move camera up/down 
		//// (in camera's local coordinate system).
		m_Keyboard.AddKeyMap('Q', 0, LOGICAL_KEY_UP);
		m_Keyboard.AddKeyMap('E', 0, LOGICAL_KEY_DOWN);
#endif
		m_Keyboard.AddKeyMap('T', 0, LOGICAL_KEY_BACK);
	}
	else
	{
...
void CMapView3D::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if (CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_TOGGLE_3DGRID), 0.5f))
	{
		pDoc->OnToggle3DGrid();
	}

	if (CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_TOGGLE_STATS), 0.5f))
	{
		m_pRender->DebugHook1();
	}

	if (CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_TOGGLE_CULLBOXES), 0.5f))
	{
		m_pRender->DebugHook2();
	}

//	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_BACKPLANE_DECR), 0.5f) )
	if ( m_Keyboard.GetKeyScale(LOGICAL_KEY_BACKPLANE_DECR) >= 0.25f )
	{
...
//	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_BACKPLANE_INCR), 0.5f) )
	if ( m_Keyboard.GetKeyScale(LOGICAL_KEY_BACKPLANE_INCR) >= 0.25f )
	{
		float fBack = m_pCamera->GetFarClip();
		if ( ( fBack <= 300000 ) && ( fBack >= 1000 ) )
		{
			m_pCamera->SetFarClip(fBack + 1000);
			Options.view3d.iBackPlane = fBack;
		} 
...
#ifdef SLE //// SLE NEW: hold shift + 1 or 2 to move model rendering distance clip // doesn't work
	/*
	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_MODELDIST_INCR), 0.5f) )
	{
		float fModelBack = Options.view3d.nModelDistance + ( Options.view3d.nModelDistance >= 2000 ) ? 1000 : 500;
		if ( fModelBack > 100000 ) fModelBack = 100000;
		Options.view3d.nModelDistance = fModelBack;
	}

	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_MODELDIST_DECR), 0.5f) )
	{
		float fModelBack = Options.view3d.nModelDistance - (Options.view3d.nModelDistance >= 2000) ? 1000 : 500;
		if (fModelBack < 500) fModelBack = 500;
		Options.view3d.nModelDistance = fModelBack;
	}
	*/
#endif
	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_SELNEXT), 0.5f)
		|| CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_SELNEXT_ALT), 0.5f))
	{
		pDoc->OnCmdMsg(ID_EDIT_SELNEXT, CN_COMMAND, NULL, NULL);
	}

	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_SELPREV), 0.5f) 
		|| CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_SELPREV_ALT), 0.5f))
	{
	...
	switch (nChar)
	{
...
#ifndef HAMMER2013_PORT_KEYBINDS
#ifdef SLE
		case '3': //// SLE NEW - bind 3 and 4 to cycling thru selecton, easier to reach with left hand
#endif
		case VK_NEXT:
		{
			pDoc->OnCmdMsg(ID_EDIT_SELNEXT, CN_COMMAND, NULL, NULL);
...
#ifdef SLE
		case '4': //// SLE NEW - bind 3 and 4 to cycling thru selecton, easier to reach with left hand
#endif
		case VK_PRIOR:
		{
			pDoc->OnCmdMsg(ID_EDIT_SELPREV, CN_COMMAND, NULL, NULL);
		...
		case '1':
		{
#ifdef SLE //// SLE NEW: hold shift + 1 to move model rendering distance clip // doesn't work
		...
		case '2':
		{
#ifdef SLE	//// SLE NEW: hold shift + 2 to move model rendering distance clip // doesn't work
			if ((GetAsyncKeyState(VK_SHIFT) & 0x8000) == 0)
#endif
			{
				float fBack = m_pCamera->GetFarClip();
...
#ifdef SLE //// SLE NEW - since C is unbound from Radius Culling, use it for toggling Cull boxes
		case 'C':
		case 'c':
		{
			m_pRender->DebugHook2();
...
#endif
#ifndef SLE
//// SLE Removed: this hotkey is considered useless and can be used for something more useful.
		case 'I':
		case 'i':
		{
			m_pRender->DebugHook2();
...
{
	//// SLE TODO: These are the functions that decide in which plane to use a object during 3d Edit.
...
#ifdef SLE //// SLE NEW - rotate the 3d camera by pressing mousewheel
//-----------------------------------------------------------------------------
// Purpose: handle pressing the mousewheel in 3d view
//-----------------------------------------------------------------------------
void CMapView3D::OnMButtonDown(UINT nFlags, CPoint point)
{
	EnableRotating(true);
	return;
...
//-----------------------------------------------------------------------------
// Purpose: handle depressing the mousewheel in 3d view
//-----------------------------------------------------------------------------
void CMapView3D::OnMButtonUp(UINT nFlags, CPoint point)
{
	if (m_bRotating)
	{
		EnableRotating(false);
...
{
#ifdef SLE
	SetTimer(MVTIMER_PICKNEXT, 5, NULL);
#else
...
void CMapView3D::OnInitialUpdate(void)
{
	...
	m_fStrafeSpeedMax = m_fForwardSpeedMax * 0.75f;
	m_fVerticalSpeedMax = m_fForwardSpeedMax * 0.5f;
...
#ifdef SLE	//// fast toggle for the View menu, not saved 
	CMapStudioModel::EnableAnimation(Options.view3d.bAnimateModels && Options.view3d.bAnimateModelsToggle);
#else
...
//-----------------------------------------------------------------------------
// Purpose: Turns on BSP lighting preview mode from the floating "Camera" menu.
//-----------------------------------------------------------------------------
void CMapView3D::OnView3dBSPLightingPreview(void)
{
...
#ifdef SLE
//-----------------------------------------------------------------------------
// Purpose: //// SLE NEW - LOD contronl option
//-----------------------------------------------------------------------------
void CMapView3D::OnLODIncrease(void)
{
	if (Options.view3d.nLOD > -1) Options.view3d.nLOD--;
}

void CMapView3D::OnLODDecrease(void)
{
	if (Options.view3d.nLOD < 5) Options.view3d.nLOD++;
}

//-----------------------------------------------------------------------------
// Purpose: //// SLE NEW - Picmip contronl option
//-----------------------------------------------------------------------------
void CMapView3D::OnPicmipIncrease(void)
{
	if (Options.view3d.nPicmip > -1) Options.view3d.nPicmip--;
}

void CMapView3D::OnPicmipDecrease(void)
{
	if (Options.view3d.nPicmip < 2) Options.view3d.nPicmip++;
}

void CMapView3D::OnPicmipVeryHigh(void)
{
	Options.view3d.nPicmip = -1;
}

void CMapView3D::OnPicmipHigh(void)
{
	Options.view3d.nPicmip = 0;
}

void CMapView3D::OnPicmipMedium(void)
{
	Options.view3d.nPicmip = 1;
}

void CMapView3D::OnPicmipLow(void)
{
	Options.view3d.nPicmip = 2;
}

void CMapView3D::OnUpdatePicmipValue(void)
{
//	MaterialSystemConfig(). 
}

//// SLE NEW - flipping in 3d view
BOOL CMapView3D::OnFlip(UINT nID)
{
	CMapDoc *pDoc = GetMapDoc();
...
void CMapView3D::OnEditProperties()
{
	// kludge for trackpopupmenu()
	GetMainWnd()->pObjectProperties->ShowWindow(SW_SHOW);
}

void CMapView3D::OnUpdateEditFunction(CCmdUI *pCmdUI)
{
...
{
	SetDrawType(VIEW3D_ENGINE); //// SLE NEW: Attempt to re-enable "Engine View" function. So far doesn't do anything.
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
#ifdef SLE
			if ( (dwTimeElapsed/1000.0f) > 1.0f/30.0f) // 1.0f/20.0f //// SLE NEW: Sets the idle framerate to 100 instead of 20. Doesn't mean the window will always maintain 60 at minimum.
#else
...
#ifdef SLE	//// fast toggle for the View menu, not saved 
	if (nFlags & MAPVIEW_UPDATE_ANIMATION)
	{
		CMapStudioModel::EnableAnimation(Options.view3d.bAnimateModels && Options.view3d.bAnimateModelsToggle);
	//	CMapStudioModel::EnableAnimation(Options.view3d.bAnimateModels);
	}
...
#ifdef SLE //// SLE NEW - holding down shift acts as a camera speed boost
	bool bShift = ((GetAsyncKeyState(VK_SHIFT) & 0x8000) != 0);
	if (bShift)
	{
		m_fForwardSpeed *= 3.5;
		m_fStrafeSpeed *= 3.5;
		m_fVerticalSpeed *= 3.5;
	}
#endif	
	//

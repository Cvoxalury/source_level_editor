...
BEGIN_MESSAGE_MAP(COP_Model, CObjectPage)
	...
	*/
	...
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
#ifdef SLE //// SLE CHANGE - no need to make it this fine
const int FRAME_SCROLLBAR_RANGE = 100;
#else
...
#ifdef SLE //// SLE FIX - make frame slider in Object Properties work
	m_flFrameSliderValue = 0;
#endif
}
...
void COP_Model::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MODEL_TAB_SEQUENCE, m_ComboSequence);
	DDX_Control(pDX, IDC_MODEL_TAB_FRAME_SCROLLBAR, m_ScrollBarFrame);
#ifdef SLE
	DDX_Control(pDX, IDC_MODEL_TAB_PLAYANIMS, m_PlayAnimsToggle);
#endif
	//}}AFX_DATA_MAP
}
...
BOOL COP_Model::OnSetActive()
{
	...
	if ( pModel != NULL )
	{
		if ( pModel->m_pStudioModel )
			pModel->m_pStudioModel->SetCycle(m_flFrameSliderValue);
	}

	UpdateFrameText(m_flFrameSliderValue);

	m_PlayAnimsToggle.SetCheck(Options.view3d.bAnimateModelsToggle);
#endif
...
void COP_Model::FillInModelData(void)
{
	if ( GetModelHelper() )
	{
		StudioModel *model = GetModelHelper()->m_pStudioModel;
		int sequence = GetModelHelper()->m_pStudioModel->GetSequence();
		char activity[ 128 ];
		GetModelHelper()->m_pStudioModel->GetActivityNameForSequence(sequence, activity);
		GetDlgItem(IDC_MODEL_TAB_ACTIVITY_TEXT)->SetWindowText(activity);

		float mass;
		model->GetModelMass(&mass);
		char surfaceprop[ 64 ];
...
void COP_Model::OnPlayModelAnimations(void)
{
	...
	if ( !Options.view3d.bAnimateModelsToggle && GetModelHelper() ) // if pausing, put the slider on the current frame
	{
		m_ScrollBarFrame.SetPos((int)(GetModelHelper()->m_pStudioModel->GetCycle() * m_ScrollBarFrame.GetRangeMax()));

		UpdateFrameText(GetModelHelper()->m_pStudioModel->GetCycle());
	}
}

void COP_Model::OnUpdatePlayModelAnimations(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(Options.view3d.bAnimateModelsToggle ? 0 : 1);
}
/*
void COP_Model::OnLButtonUp(UINT nFlags, CPoint point)
{
	if ( m_ScrollBarFrame )
	{
		CMapStudioModel *pModel = GetModelHelper();
		if ( pModel != NULL )
		{
			float flFrame = ( float )m_ScrollBarFrame.GetPos() / ( float )FRAME_SCROLLBAR_RANGE;
#ifdef SLE //// SLE FIX - make frame slider in Object Properties work
			m_flFrameSliderValue = flFrame;
...
void COP_Model::UpdateFrameText(float flFrame)
{
	...
	GetDlgItem(IDC_MODEL_TAB_FRAME_TEXT)->SetWindowText(szFrame);
}
...
#ifdef SLE
		if ( GetModelHelper() && GetModelHelper()->m_pStudioModel )
		{
		//	int sequence = GetModelHelper()->m_pStudioModel->GetSequence();
			char activity[ 128 ];
			GetModelHelper()->m_pStudioModel->GetActivityNameForSequence(iSequence, activity);
			GetDlgItem(IDC_MODEL_TAB_ACTIVITY_TEXT)->SetWindowText(activity);

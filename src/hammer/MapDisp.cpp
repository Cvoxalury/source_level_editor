...
#ifdef SLE //// round up to an even number, as odd values aren't recommended
		if ((nLightmapScale % 2) != 0) nLightmapScale += 1;
#endif
...
void CMapDisp::UpdateWalkable( void )
{
	...
	int nTriCount = GetTriCount();
#ifndef SLE	//// SLE REMOVE: this isn't used anymore, only manual tagging is used
	for ( int iTri = 0; iTri < nTriCount; ++iTri )
	{
		Vector v1, v2, v3;
...
void CMapDisp::UpdateTriRemove( void )
{
	...
	for ( int iTri = 0; iTri < nTriCount; ++iTri )
	{
		if ( IsTriRemove( iTri ) )
		{
			...
			if (IsTriTag(iTri, COREDISPTRI_TAG_FORCE_REMOVE_BIT))
			{
				m_aForcedRemoveIndices.AddToTail(newTriIndices[0]);
				m_aForcedRemoveIndices.AddToTail(newTriIndices[1]);
				m_aForcedRemoveIndices.AddToTail(newTriIndices[2]);
			}
			else
			{
...
void CMapDisp::UpdateMesh()
{
	...
	if (true) // Options.view3d.bInvertDisplacementAlpha )
	{
...
{
#ifdef SLE //// SLE CHANGE - re-enable debugging previews
						  ...
						  Color &pColor )
{
	...
	switch ( renderMode )
	{
		...
		case RENDER_MODE_LIGHTMAP_GRID:
		{
			...
			else if (pFace->texture.nLightmapScale <= 2 )
			{
				pColor[0] = 255;
				pColor[1] = 150;
				pColor[2] = 50;
			}
			else if (pFace->texture.nLightmapScale <= 8 )
			{
				pColor[0] = 255;
				pColor[1] = 225;
				pColor[2] = 100;
			}
			else if (pFace->texture.nLightmapScale > 8 && pFace->texture.nLightmapScale < 16)
			{
				pColor[0] = 255;
				pColor[1] = 255;
				pColor[2] = 100;
			}
			else if (pFace->texture.nLightmapScale >= 16 && pFace->texture.nLightmapScale < 32)
			{
				pColor[0] = 255;
				pColor[1] = 255;
				pColor[2] = 255;
			}
			else if (pFace->texture.nLightmapScale >= 32 && pFace->texture.nLightmapScale < 64)
			{
				pColor[0] = 150;
				pColor[1] = 255;
				pColor[2] = 255;
			}
			else if (pFace->texture.nLightmapScale >= 64 && pFace->texture.nLightmapScale < 128)
			{
				pColor[0] = 100;
				pColor[1] = 255;
				pColor[2] = 100;
			}
			else if (pFace->texture.nLightmapScale >= 128)
			{
				pColor[0] = 100;
				pColor[1] = 150;
				pColor[2] = 255;
...
void CMapDisp::Render3D( CRender3D *pRender, bool bIsSelected, SelectionState_t faceSelectionState )
{
	...
	else
	{
		...
		if (bIsSelected && renderMode != RENDER_MODE_WIREFRAME )
		{
			// This renders wireframe twice in selection!
			RenderWireframeSurface( pRender, bIsSelected, faceSelectionState );
		}
#ifdef SLE //// SLE CHANGE - render disps with wireframe to help see their relief
		if ((renderMode == RENDER_MODE_FLAT || renderMode == RENDER_MODE_TRANSLUCENT_FLAT) && !bIsSelected)
		{
			// This renders wireframe twice in selection!
			RenderWireframeSurface(pRender, true, faceSelectionState);
		}
#endif
	}
...
void CMapDisp::RenderOverlaySurface( CRender3D *pRender, bool bIsSelected, SelectionState_t faceSelectionState )
{
	...
	for (int i = 0; i < nVertCount; ++i )
	{
		...
		if (!Options.general.bShowToolsSkyFaces && m_pParent && m_pParent->Is3dSkybox())
		{
			...
			meshBuilder.Position3f((pVert[i].m_Vert.Base()[0] + skyDelta[0]) * skyScale, (pVert[i].m_Vert.Base()[1] + skyDelta[1]) * skyScale, (pVert[i].m_Vert.Base()[2] + skyDelta[2]) * skyScale);
		}
		else
#endif
		{
...
{
#if SLE_STATIC_DISP_MESH
	if ( !m_pMesh && !bIsSelected && faceSelectionState == SELECT_NONE && !pRender->IsPicking() )
		UpdateMesh();
	...
	for (int i = 0; i < numVerts; ++i )
	{
		...
		if (!Options.general.bShowToolsSkyFaces && m_pParent && m_pParent->Is3dSkybox())
		{
			...
			{
				CMapWorld *pWorld = pDoc->GetMapWorld();
				if (pWorld) skyDelta = pWorld->GetSkyCameraDelta();
				if (pWorld) skyScale = pWorld->GetSkyCameraScale();
			}
			meshBuilder.Position3f((pVert[i].m_Vert.Base()[0] + skyDelta[0]) * skyScale, (pVert[i].m_Vert.Base()[1] + skyDelta[1]) * skyScale, (pVert[i].m_Vert.Base()[2] + skyDelta[2]) * skyScale);
		}
		else
...
void CMapDisp::RenderWalkableSurface( CRender3D *pRender, bool bIsSelected, SelectionState_t faceSelectionState )
{
	...
	for ( int iPass = 0; iPass < 2; ++iPass )
	{
		...
		//// SLE TODO - implement 3d skybox scaling
		for (int i = 0; i < nVertCount; ++i )
		{
...
void CMapDisp::RenderRemoveSurface( CRender3D *pRender, bool bIsSelected, SelectionState_t faceSelectionState )
{
	...
	for (int iPass = 0; iPass < 2; ++iPass)
	{
		...
		int nVertCount = m_aRemoveVerts.Count();
		int nIndexCount = m_aForcedRemoveIndices.Count();
		...
		unsigned short *pIndex = m_aForcedRemoveIndices.Base();
		for (int i = 0; i < nIndexCount; ++i)
		{
			meshBuilder.Index(pIndex[i]);
			meshBuilder.AdvanceIndex();
...
//		return;
	...
	for( int i = 0; i < size; i++ )
	{
		...
		if ( pDoc )
		{
			Vector vTraceHitPos = v;
			bool bHit = true;
			if ( !pDoc->DropTraceOnDisplacementsAndClips(v, &vTraceHitPos, &bHit) )
			{
				return;
			}
			Paint_SetValue(i, vTraceHitPos);
		}
#else
	...
	if (!pFace)
	{
		AssertMsg("Attempted to sim-sculpt displacement without parent face!", 0);
		return;
	}
	...
	{		
		...
		GetVert(i, vertexPos);
		vertexPos.z -= 4.0;
		vertexGoal = vertexPos;

		trace_t boxTrace;
		float traceFraction;
		Vector rayStart, rayEnd, rayDelta;
		rayStart = vertexPos;
		rayEnd = rayStart;
		rayEnd.z -= 1024;
		rayDelta = rayEnd - rayStart;
		VectorNormalize(rayDelta);
		
	//	ULONG ulFace;
	//	VMatrix LocalMatrix, LocalMatrixNeg;
	//	CMapClass *pObject = GetView->NearestObjectAt(vPoint, ulFace, FLAG_OBJECTS_AT_RESOLVE_INSTANCES, &LocalMatrix);

		if (IntersectRayWithBox(rayStart, rayDelta, -Vector(4, 4, 4), Vector(4, 4, 4), 1, &boxTrace, &traceFraction))
		{
			vertexGoal = boxTrace.endpos;
			vertexGoal.z *= traceFraction;
		}
		else
			vertexGoal = rayStart + rayDelta * RandomFloat(min, max);
		

		Paint_SetValue(i, vertexGoal);
	}
#endif
...
inline void CMapDisp::GetVert( int index, Vector& v )
{
	...
	if ( !Options.general.bShowToolsSkyFaces && m_pParent && m_pParent->Is3dSkybox() )
	{
		...
		v[0] = (v[0] + skyDelta [0]) * skyScale;
		v[1] = (v[1] + skyDelta [1]) * skyScale;
		v[2] = (v[2] + skyDelta [2]) * skyScale;
	}
#endif
}
#endif

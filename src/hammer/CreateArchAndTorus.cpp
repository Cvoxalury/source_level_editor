...
CMapClass *CreateArch(BoundBox *pBox, float fStartAngle, int iSides, float fArc, int iWallWidth, int iAddHeight, BOOL bPreview)
{
	...
	if (Options.general.bStretchArches)
	{
			...
			boundsize[AXIS_Y] > objsize[AXIS_Y])
		{
			...
			Vector center;
			pBox->GetBoundsCenter(center);
#ifdef SLE //// SLE FIX - fix the bug where stretching also moves the arch/torus along Y axis by half the Y length
			Vector objmins, objmaxs;
			pGroup->GetBoundingBox(objmins, objmaxs);
			float factorXmin = pBox->bmins[0] - objmins[0];
			float factorYmin = pBox->bmins[1] - objmins[1];
			float factorXmax = pBox->bmaxs[0] - objmaxs[0];
			float factorYmax = pBox->bmaxs[1] - objmaxs[1];

			if( factorXmin < 0)
				center[AXIS_X] -= factorXmin;
			if( factorYmin < 0)
				center[AXIS_Y] -= factorYmin;

			if( factorXmax > 0)
				center[AXIS_X] -= factorXmax;
			if( factorYmax > 0)
				center[AXIS_Y] -= factorYmax;
#endif

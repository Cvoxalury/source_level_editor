...
{
//// SLE REMOVED: used to have Ep2 specific hardcoded
//// debug function for a particular entity.
		...
		if (eResult == ChunkFile_Ok)
		{
			...
			for (int i = 0; i < nConnCount; i++)
			{
				...
				if (pConnection != NULL)
				{
					char szTemp[512];					
#ifdef SLE //// SLE NEW - support for modern delimeters (TF2)
					if ( g_pGameConfig->mapformat == mfTF2 )
					{
...
// ----------------------------------------------------------------------------
//// SLE NEW: Restored from 2007 code. In 2013, this's located in a common header,
//// but it creates linking errors in 2013 *SDK* code.
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
#define TOKEN_ERROR "_token_error"
static const char *nexttoken(char *token, const char *str, char sep)
{
	...
	if ( !str )
	{
		...
		{
			CString str;
			str.Format("This map file cannot be properly loaded due to a token parsing error \n\n(%s).\n\nMake sure the map's format matches the Map Format setting in the Game Configuration!", token);
			
			int choice = AfxMessageBox(str, MB_OKCANCEL);
			if ( choice == IDIGNORE )
				doOnce = true;
...
ChunkFileResult_t CEditGameClass::LoadKeyCallback(const char *szKey, const char *szValue, CEditGameClass *pEditGameClass)
{
	...
	//// SLE NEW - support for modern delimeters
	...
	if ( g_pGameConfig->mapformat == mfTF2 )
	{
		...
		{
			chDelim = VMF_IOPARAM_STRING_DELIMITER;
		}
		else
		{
			chDelim = ',';

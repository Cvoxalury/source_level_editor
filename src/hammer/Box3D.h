...
enum WorldUnits_t
{
	...
	Units_Feet_Inches,
#ifdef SLE
	Units_Centimeters,  //// SLE NEW - show units in cm
	Units_Meters_Centimeters,  //// SLE NEW - show units in m + cm
#endif
};
...
#ifdef SLE
	virtual void ToggleTranslateMode(bool allowShear = true); //// SLE CHANGE: use this bool so we can cycle thru modes without Shear in 3D, but still get it in 2D. 
#else
...
#ifdef SLE /// SLE NEW: rotation pivot (determine if the last hit test was on the pivot)
	bool			m_hit_rotation_pivot_bool;
#endif
private:
};

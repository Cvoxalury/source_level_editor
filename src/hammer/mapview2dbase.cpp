...
public:
	...
	virtual void Paint()
	{
#ifdef SLE //// SLE NEW: optimise - don't waste perf drawing viewports that are too narrow to see
		if (GetWide() < 20 || GetTall() < 20 ) return;
#endif
		m_pMapView->Render();
	}
...
void CMapView2DBase::SetColorMode(bool bWhiteOnBlack)
{
	...
	clr = Options.colors.clrMapBounds;
	// Grid 16384 highlight color.
	m_clrGrid16384.SetColor(GetRValue(clr), GetGValue(clr), GetBValue(clr), 255);
#endif
	...
	clr = Options.colors.clr2DBackground;
#else
...
void CMapView2DBase::DrawGrid(CRender2D *pRender, int xAxis, int yAxis, float depth, bool bNoSmallGrid)
{
	...
	int nGridSpacing = MAX(pDoc->GetGridSpacing(), 1);
#else
	int nGridSpacing = pDoc->GetGridSpacing();
#endif
	// never allow a grid spacing smaller than 2 pixels
...
#ifdef SLE
	int xMin = SnapToGrid( (int)max( g_MIN_MAP_COORD * 4, m_ViewMin[xAxis]-nGridSpacing ), nGridSpacing );
	int xMax = SnapToGrid( (int)min( g_MAX_MAP_COORD * 4, m_ViewMax[xAxis]+nGridSpacing ), nGridSpacing );
	int yMin = SnapToGrid( (int)max( g_MIN_MAP_COORD * 4, m_ViewMin[yAxis]-nGridSpacing ), nGridSpacing );
	int yMax = SnapToGrid( (int)min( g_MAX_MAP_COORD * 4, m_ViewMax[yAxis]+nGridSpacing ), nGridSpacing );
#else
	...
	for (int y = yMin; y <= yMax; y += nGridSpacing )
	{
		...
		int bHighlight = HighlightGridLine( pRender, y );
		
		// Don't draw the base grid if it is too small.
		
		if (!bHighlight && bNoSmallGrid)
			continue;

		// Always draw lines for the axes and map boundaries.

		if ((!s_bGridDots) || (bHighlight) || (y == g_MAX_MAP_COORD) || (y == g_MIN_MAP_COORD))
		{
	...
	for (int x = xMin; x <= xMax; x += nGridSpacing )
	{
		...
		int bHighlight = HighlightGridLine( pRender, x );
...
#ifdef SLE	//// SLE NEW - draw outer boundaries of the grid based on @mapsize from fgd 
	// bottom left to top left
	pRender->SetDrawColor(m_clrGrid16384);
	vPointMin[xAxis] = pGD->GetMinMapCoord();
	vPointMin[yAxis] = pGD->GetMinMapCoord();
	vPointMax[xAxis] = pGD->GetMinMapCoord();
	vPointMax[yAxis] = pGD->GetMaxMapCoord();
	pRender->DrawLine(vPointMin, vPointMax);

	// bottom right to top right
	vPointMin[xAxis] = pGD->GetMaxMapCoord();
	vPointMin[yAxis] = pGD->GetMinMapCoord();
	vPointMax[xAxis] = pGD->GetMaxMapCoord();
	vPointMax[yAxis] = pGD->GetMaxMapCoord();
	pRender->DrawLine(vPointMin, vPointMax);

	// top left to top right
	vPointMin[xAxis] = pGD->GetMinMapCoord();
	vPointMin[yAxis] = pGD->GetMaxMapCoord();
	vPointMax[xAxis] = pGD->GetMaxMapCoord();
	vPointMax[yAxis] = pGD->GetMaxMapCoord();
	pRender->DrawLine(vPointMin, vPointMax);
	...
	vPointMin[yAxis] = pGD->GetMinMapCoord();
	vPointMax[xAxis] = pGD->GetMaxMapCoord();
	vPointMax[yAxis] = pGD->GetMinMapCoord();
	pRender->DrawLine(vPointMin, vPointMax);
...
void CMapView2DBase::DrawBackgroundImage(CRender2D *view, CString &filename, int bl_x, int bl_y, int tr_x, int tr_y, int opacity)
{
	...
	if ( TGALoader::LoadRGBA8888(filename, tga, outwidth, outheight) )
	{
		surface()->DrawSetTextureRGBA(id, tga.Base(), width, height, false, true);
		surface()->DrawGetTextureSize(id, width, height);
...
//	else if (viewdir[axHorz] == COORD_EXTENT)
	{
//		view->DrawRectangle(Vector(0, bl_x, bl_y), Vector(0, tr_x, tr_y), true); // y/z
	}
//	else if (viewdir[axVert] == COORD_EXTENT)
	{
...
#ifdef HAMMER2013_PORT_KEYBINDS_X
void CMapView2DBase::InitializeKeyMap(void)
{
	m_Keyboard.RemoveAllKeyMaps();

	BEGIN_KEYMAP("MapView2D")
	{		
		ADD_KEY(LOGICAL_KEY_2D_SELNEXT);
		ADD_KEY(LOGICAL_KEY_2D_SELPREV);
		ADD_KEY(LOGICAL_KEY_2D_SELNEXT_ALT);
		ADD_KEY(LOGICAL_KEY_2D_SELPREV_ALT);
		ADD_KEY(LOGICAL_KEY_2D_PANNING);
		ADD_KEY(LOGICAL_KEY_2D_ZOOM_OUT);
		ADD_KEY(LOGICAL_KEY_2D_ZOOM_IN);
		ADD_KEY(LOGICAL_KEY_2D_ZOOM_OUT_ALT);
		ADD_KEY(LOGICAL_KEY_2D_ZOOM_IN_ALT);
		ADD_KEY(LOGICAL_KEY_2D_ZOOM_MIN);
	}
}
#endif
//-----------------------------------------------------------------------------
...
//	bool bShift = nFlags & MK_SHIFT;
...
#ifdef HAMMER2013_PORT_KEYBINDS_X	
	if ( m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_ZOOM_OUT) >= 0.25f
		|| m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_ZOOM_OUT_ALT) >= 0.25f )
	{
		ZoomOut(bCtrl);
		return;
	}

	if ( m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_ZOOM_IN) >= 0.25f
		|| m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_ZOOM_IN_ALT) >= 0.25f)
	{
		ZoomIn(bCtrl);
		return;
	}
	
	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_SELNEXT), 0.5f)
		|| CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_SELNEXT_ALT), 0.5f) )
	{
		pDoc->OnCmdMsg(ID_EDIT_SELNEXT, CN_COMMAND, NULL, NULL);
		return;
	}
	
	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_SELPREV), 0.5f)
		|| CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_SELPREV_ALT), 0.5f) )
	{
		pDoc->OnCmdMsg(ID_EDIT_SELPREV, CN_COMMAND, NULL, NULL);
		return;
	}
	
	if ( CloseEnough(m_Keyboard.GetKeyScale(LOGICAL_KEY_2D_ZOOM_MIN), 0.5f))
	{
		SetZoom(m_flMinZoom);
		return;
	}
	...
	switch (nChar)
	{
		...
		//
#ifdef SLE //// SLE CHANGE - use 1 & 2 as zoom in/out, reserve 3 and 4 for cycling (like in 3d view), and free up others except 0
		case '1':
		{
			ZoomIn(bCtrl);
			break;
...
#ifdef SLE //// SLE NEW: pan the 2d view with MMB
//-----------------------------------------------------------------------------
// Purpose: View panning
//-----------------------------------------------------------------------------
void CMapView2DBase::OnMButtonDown(UINT nFlags, CPoint point)
{
	if (!m_pToolManager)
		return;
...
void CMapView2DBase::OnMButtonUp(UINT nFlags, CPoint point)
{
	CMapDoc *pDoc = GetMapDoc();
...
void CMapView2DBase::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar) 
{
	...
	switch (nSBCode)
	{
#ifdef SLE //// SLE CHANGE - this shoudl fix the scroll sometimes not working (copied from Hammer-2013)
		case SB_LINELEFT:
		{
...
#ifndef SLE //// SLE CHANGE - don't; improves framerate
					m_bUpdateView = true;
#endif
				}

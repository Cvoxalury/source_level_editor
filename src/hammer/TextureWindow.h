...
public:
	...
	enum
	{
		...
		TYPEFILTER_SPECULAR = 0x8,

		TYPEFILTER_ALL = (TYPEFILTER_OPAQUE|TYPEFILTER_TRANSLUCENT|TYPEFILTER_SELFILLUM|TYPEFILTER_SPECULAR),
	};
	...
	void SetFavouritesOnly(bool set) { m_bFavouritesOnly = set; }
	int GetDisplaySize(void) {	return iDisplaySize; }
	
	bool m_bShouldUpdateTextureSelection = true; // if the favourites selected the active texture, don't make it update the main browser's texture
	bool m_bMouseOver; // used to detect if the favourites texture window was clicked on or the main one
	int m_iLastClickTimeStamp;
#endif
...
protected:
	...
	bool m_bShowErrors;
#ifdef SLE_TEXTUREBROWSER_FAVOURITES
	bool m_bFavouritesOnly = false; // won't be a visible checkbox, used internally to differentiate the fav browser
#endif

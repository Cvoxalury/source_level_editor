...
void CEntitySprinkleDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SPRINKLE_ALIGNTOSURFACE, m_SurfaceAlignControl);
}
...
BOOL CEntitySprinkleDlg::OnInitDialog()
{
	...
	m_SprinkleModeControl.SetCurSel( 2 ); // default to replace
	...
	m_RandomYawControl.SetCheck(BST_CHECKED);
...
void CEntitySprinkleDlg::OnBnClickedSprinkleUseGrid()
{
	...
	sprintf( temp, "%.3f", pDoc->GetGridSpacing() ); //// note, grid is allowed to be sub 1 float in SLE
...
// Output : 
//-----------------------------------------------------------------------------
bool CEntitySprinkleDlg::UseSurfaceAlign()
{
	return (m_SurfaceAlignControl.GetCheck() == BST_CHECKED);
}

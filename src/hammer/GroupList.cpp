...
void CGroupList::AddVisGroup(CVisGroup *pGroup)
{
	...
	AddItem(pGroup, pGroup->GetParent(), pGroup->GetName(), true);
	for (int i = 0; i < pGroup->GetChildCount(); i++)
	{
		AddVisGroup(pGroup->GetChild(i));
	}
#else
...
#ifdef SLE //// SLE CHANGE - expand all auto visgroups on creation for convenience

...
#include "particles/particles.h" //// SLE NEW - particle systems
//#include "vstdlib\jobthread.h" //// SLE NEW - threading support
#endif
#pragma warning(push, 1)
...
//// SLE_USE_HAMMER_LPREVIEW
//// SLE TODO - unused but left in for compilation
...
static int WrapFunctionWithMinidumpHandler( int (*pfn)(void *pParam), void *pParam )
{
	...
	if ( !Plat_IsInDebugSession() && CommandLine()->FindParm( "-writeminidumps") ) //// SLE NEW: default to no minidumps. Vast majority of users probably will never use them. Save their disk space
	{
...
#ifdef HAMMER2013_PORT_KEYBINDS
					g_pFullFileSystem->AddSearchPath(lpszParam, "level_editor_cfg", PATH_ADD_TO_HEAD);
...
{
#ifdef HAMMER2013_PORT_KEYBINDS
	if (m_CmdLineInfo)	delete m_CmdLineInfo;
...
{
	if (HINSTANCE(32) > ::ShellExecute(hwnd, "open", pszURL, NULL, NULL, 0)) // todo, make it bring browser window to front - test
	{
...
void CHammer::Help(const char *pszTopic)
{
	...
	//
#ifdef SLE //// SLE NEW - use offline help
...
InitReturnVal_t CHammer::HammerInternalInit()
{
	...
	*/
...
#ifdef SLE //// SLE CHANGE - increase Recent Files list to 10
	UINT numRecentFiles = 10;
#endif

	LoadStdProfileSettings(numRecentFiles);  // Load standard INI file options (including MRU)
...
#ifdef SLE	//// SLE NEW - particle systems
...
#endif
	{
		...
		{
			char msg[1024];
			V_snprintf( msg, sizeof( msg ), "Program did not shut down correctly the last time it was used.\nWould you like to load the last saved file?\n(%s)", (const char*)strLastGoodSave );
...
void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ORDER, m_Order); // maybe repurpose this as a "visit site" later? 
#endif
	//}}AFX_DATA_MAP
}
...
#ifdef SLE //// SLE NEW - immediately add this file to recent list. Otherwise if Hammer crashes it'll not appear there. // doesn't achieve what it should...
...
#ifdef SLE //// SLE CHANGE - this crashed the editor in case of some broken maps, because old "else" would be satisfied if !pMapDoc
	if (pMapDoc )
	{
		if( !CHammer::IsNewDocumentVisible())
		{
...
#ifdef HAMMER2013_PORT_SAVE_ON_CRASH
	GetDirectory(DIR_PROGRAM, szRootDir);
#else
...
void CHammer::RunFrame(void)
{
	...
	if (!IsActiveApp())
	{
#ifdef SLE //// a bit more power-saving
		Sleep(100);
#else
		Sleep(50);
#endif
...
unsigned CHammer::DoAutosave( void* _data )
{
	...
	nCurrentAutosaveNumber = GetNextAutosaveNumber( strAutosaveDirectory, &autosaveFiles, 
		&dwTotalAutosaveDirectorySize, &strMapTitle );
...
{	
	...
	CString strAutosaveDirectory( szRootDir );	
	{
		...
		{
#ifndef SLE //// this is allowed now
...
#ifdef SLE //// SLE NEW - show random mapping tip in the status bar //// SLE TODO - finish this
const char *MappingTips[5] =
{
	"hello",
	"hi",
	"how are you",
...
void CHammer::UpdateMappingTip(void)
{
//	int randomInt = RandomInt(0, 65536);
	char szBuf[256];
	strcpy(szBuf, MappingTips[RandomInt(0, 4)]);
//	sprintf(szBuf, MappingTips[RandomInt(0,sizeof(MappingTips)-1)]);
	SetStatusText(SBI_PROMPT, szBuf);
}
#endif

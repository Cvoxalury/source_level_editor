...
//	ScreenTransform( source, screen );
	...
	VectorMA (source, -scale, g_vecCurrentVUp, point);
	VectorMA (point, -scale, g_vecCurrentVRight, point);
...
void DrawSprite( const Vector &vecOrigin, float flWidth, float flHeight, color32 color )
{
	...
	VectorSubtract(g_vecCurrentRenderOrigin, vecOrigin, fwd );
	float flDist = VectorNormalize( fwd );
	if (flDist >= 1e-3)
	{
		CrossProduct(g_vecCurrentVUp, fwd, right );
		...
		else
		{
			...
			CrossProduct( fwd, g_vecCurrentVRight, up );
			VectorNormalize( up );
...
static void ComputeBeamPerpendicular( const Vector &vecBeamDelta, Vector *pPerp )
{
	...
	CrossProduct( g_vecCurrentVForward, vecBeamCenter, *pPerp );
				...
				int flags, float* color, float fadeLength, float flHDRColorScale )
{
	...
	CSpriteModel *pSprite = CSpriteCache::CreateSprite(spritemodel);
	if ( !pSprite )
	{
		return;
	}
	...
	IMaterial *pMaterial = materials->FindMaterial(spritemodel, TEXTURE_GROUP_OTHER, false);
	if( pMaterial )
	{
		static unsigned int nHDRColorScaleCache = 0;
		IMaterialVar *pHDRColorScaleVar = pMaterial->FindVarFast( "$hdrcolorscale", &nHDRColorScaleCache );
	...
	for ( i = 0; i < segments; i++ )
	{
		...
		//	factor = RandomFloat(1.0, 1.5) * scale;
			if ( flags & FBEAM_SINENOISE )
			{
				float	s, c;
				SinCos( fraction*M_PI*length + freq, &s, &c );
				VectorMA( curSeg.m_vPos, factor * s, g_vecCurrentVUp, curSeg.m_vPos );
				// Rotate the noise along the perpendicluar axis a bit to keep the bolt from looking diagonal
				VectorMA( curSeg.m_vPos, factor * c, g_vecCurrentVRight, curSeg.m_vPos );
			}
			else
			{
...
//			fadescale - 
//-----------------------------------------------------------------------------
void DrawTeslaSegs(CRender3D *pRender, int noise_divisions, float *prgNoise, /*const model_t**/const char* spritemodel,
				float frame, int rendermode, const Vector&  source, const Vector&  delta, 
				float startWidth, float endWidth, float scale, float freq, float speed, int segments,
				int flags, float* color, float fadeLength, float flHDRColorScale )
{
...
//	IMaterial *pMaterial = pSprite->GetMaterial( (RenderMode_t)rendermode );
	...
	for ( i = 0; i < segments; i++ )
	{
		...
		if ( i == (segments * 0.5) )
		{
			...
			//	vecEnd = source + delta + (MainViewUp() * 32) + (MainViewRight() * 32);
				vecEnd = source + delta + ( g_vecCurrentVUp * 32 ) + ( g_vecCurrentVRight * 32 );
				vecEnd -= vecStart;
	...
	if ( iBranches )
	{
		DrawTeslaSegs(pRender, noise_divisions, prgNoise, spritemodel, frame, rendermode, 
					...
					float freq, float* color, float flHDRColorScale )
{
	...
	while (pHead)
	{
		...
		if (pHead->next != NULL)
		{
			fraction = (pHead->die - CURTIME) * div;
			VectorScale( color, fraction, scaledColor );
			nColor[0] = (unsigned char)clamp( (int)(scaledColor[0] * 255.0f), 0, 255 );
			nColor[1] = (unsigned char)clamp( (int)(scaledColor[1] * 255.0f), 0, 255 );
			nColor[2] = (unsigned char)clamp( (int)(scaledColor[2] * 255.0f), 0, 255 );

...
#ifdef SLE
#include "collisionutils.h" //// used for rendering world text (point message)
#include "GlobalFunctions.h" //// used for snow rendering
...
#ifdef SLE //// SLE CHANGE - seems more stable
static bool g_bUseCullTree = false;
#else
...
#ifdef SLE //// SLE CHANGE - preference
	float fShade = 0.6f + (0.4f * DotProduct(Normal, Light));
#else
...
//	m_Stream.SetSeed(RandomInt(0,10000));
}
...
#ifdef SLE //// SLE NEW: 3d skybox preview
//-----------------------------------------------------------------------------
// Purpose: Draw 3d skybox objects in a separate pass so they can have separate fog
//-----------------------------------------------------------------------------
bool CRender3D::IsDrawing3DSkybox(void)
{
	return m_bDrawing3DSkybox;
}
#endif
//-----------------------------------------------------------------------------
...
#ifdef SLE //// SLE NEW - 2d skybox preview
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
void CRender3D::MakeSkyVec(float s, float t, int axis, float zFar, Vector& position, Vector2D &texCoord)
{
	if (!GetCamera()) return; 
...
#ifdef SLE //// SLE TODO - finish or throw out
//	CreateFramebufferTextures();
	...
	else
	{
		...
		b = GetBValue(Options.colors.clr3DBackground);
	...
	if (!IsPicking())
	{
		if (!Options.general.bShowToolsSkyFaces)
		{
			Draw2DSkybox(pRenderContext);
		}
	}
#endif
	//
...
#ifdef SLE //// SLE NEW: Enable fog in 3d Textured view.
void CRender3D::Do3DFog(CMatRenderContextPtr &pRenderContext)
{
	pRenderContext->FogColor3ub(54, 54, 54);
	pRenderContext->FogStart(GetCamera()->GetFarClip() - 200);
	pRenderContext->FogEnd(GetCamera()->GetFarClip());
	pRenderContext->FogMaxDensity(1.0);

	if (!Options.general.bShowToolsSkyFaces)
	{
		// Extract fog parameters from our fog controller, if it exists
		...
		if (pDoc)
		{
			...
			if (pWorld)
			{
				...
				Vector fogColor1, fogColor2;
				pWorld->GetFogParams(fogColor1, fogColor2, start, end, density);

				pRenderContext->FogStart(start);
				pRenderContext->FogEnd(end);
				pRenderContext->FogMaxDensity(density);
				pRenderContext->FogColor3ub(fogColor1[0], fogColor1[1], fogColor1[2]);
#else
				CMapEntityList matches;
				if (!IsDrawing3DSkybox())
				{
					pWorld->FindEntitiesByClassName(matches, "env_fog_controller", true);
					if(matches.Count() == 0) // useful for quicker a/b testing by hiding fog controller
					{
						pWorld->FindEntitiesByClassName(matches, "sky_camera", true);
					}
				}
				else
				{
					pWorld->FindEntitiesByClassName(matches, "sky_camera", true);
					if (matches.Count() == 0) // useful for quicker a/b testing by hiding sky_camera
					{
						pWorld->FindEntitiesByClassName(matches, "env_fog_controller", true);
					}
				}
				...
				{
					/*const char **/ std::string szfogstart = "100";
					/*const char **/ std::string szfogend = "1000";
					/*const char **/ std::string szfogcolor = "50 50 50";
					/*const char **/ std::string szfogdensity = "0.0";
					float start, end, density;
					int fogcolorR = 0;
					int fogcolorG = 0;
					int fogcolorB = 0;
					...
					if (pEntity != NULL && pEntity->GetKeyValue("fogenable") && !Q_strcmp(pEntity->GetKeyValue("fogenable"), "1"))
					{
						if (i == 0 || pEntity->GetSpawnFlag(1)) // spawnflag 1 means "Master" in env_fog_controllers
						{
							if (pEntity->GetKeyValue("fogstart"))
								szfogstart = pEntity->GetKeyValue("fogstart");
							if (pEntity->GetKeyValue("fogend"))
								szfogend = pEntity->GetKeyValue("fogend");
							if (pEntity->GetKeyValue("fogcolor"))
								szfogcolor = pEntity->GetKeyValue("fogcolor");
							if (pEntity->GetKeyValue("fogmaxdensity"))
								szfogdensity = pEntity->GetKeyValue("fogmaxdensity");

							sscanf(szfogstart.c_str(), "%f", &start);
							{
								pRenderContext->FogStart(start);
							}
							sscanf(szfogend.c_str(), "%f", &end);
							{
								pRenderContext->FogEnd(end);
							}
							sscanf(szfogdensity.c_str(), "%f", &density);
							{
								pRenderContext->FogMaxDensity(density);
							}
							sscanf(szfogcolor.c_str(), "%i %i %i", &fogcolorR, &fogcolorG, &fogcolorB);
							{
								pRenderContext->FogColor3ub(fogcolorR, fogcolorG, fogcolorB);
							}
						}
					}
				}
...
#endif
#ifdef SLE //// SLE NEW: Relay env_wind data so it can be picked up by
// tree sway shader, particle simulation and other.
void CRender3D::Do3DWind(CMatRenderContextPtr &pRenderContext)
{
	Vector vecWind = Vector(0, 0, 0);
	if (!Options.general.bShowToolsSkyFaces) // todo, later
	{
		if (GetElapsedTime() > 0.1f)
		{
			...
			if (pDoc)
			{
				...
				{
					CMapEntityList matches;
					pWorld->FindEntitiesByClassName(matches, "env_wind", true);

					for (int i = 0; i < matches.Count(); i++)
					{
						/*const char **/ std::string szminwindspeed = "20";
						/*const char **/ std::string szmaxwindspeed = "50";
						/*const char **/ std::string szwindangles = "0 0 0";
						...
						int angleZ = 0;
						int avgSpeed = 35;
						QAngle windAngle = QAngle(0, 0, 0);

						CMapEntity *pEntity = matches.Element(i);
						if (pEntity != NULL)
						{
							if (i == 0) // spawnflag 1 means "Master" in env_fog_controllers
							{
								szminwindspeed = pEntity->GetKeyValue("minwind");
								szmaxwindspeed = pEntity->GetKeyValue("maxwind");

								sscanf(szminwindspeed.c_str(), "%f", &minSpeed);
								{
									sscanf(szmaxwindspeed.c_str(), "%f", &maxSpeed);
									{
										avgSpeed = (minSpeed + maxSpeed) / 2; // m_Stream.RandomInt(minSpeed, maxSpeed);
									}
								}

								szwindangles = pEntity->GetKeyValue("angles");

								sscanf(szwindangles.c_str(), "%i %i %i", &angleX, &angleY, &angleZ);
								{
									windAngle = QAngle(0, angleY, 0);
								}

								AngleVectors(windAngle, &vecWind);
								vecWind *= avgSpeed;
							}
						}
					}
...
#endif
#ifdef SLE //// SLE NEW: Relay worldspawn data for snow direction.
void CRender3D::Do3DSnow(CMatRenderContextPtr &pRenderContext)
{
	Vector snowDir = Vector(0, 0, 1);
	float snowDot = 0.5f;

	if (GetActiveWorld() && GetActiveWorld()->GetKeyValue("snowdir") != NULL)
	{
		/*const char **/ std::string vecString = "";
		float vecX, vecY, vecZ;
		vecX = vecY = vecZ = 0;
		vecString = GetActiveWorld()->GetKeyValue("snowdir");
		sscanf(vecString.c_str(), "%f %f %f", &vecX, &vecY, &vecZ);
		snowDir = Vector(vecX, vecY, vecZ);
	}

	if (GetActiveWorld() && GetActiveWorld()->GetKeyValue("snowdot") != NULL)
	{
		snowDot = atof(GetActiveWorld()->GetKeyValue("snowdot"));
	}
	pRenderContext->SetVectorRenderingParameter(VECTOR_RENDERPARM_SNOW_DIRECTION, snowDir);
	pRenderContext->SetFloatRenderingParameter(FLOAT_RENDERPARM_SNOW_DOT, snowDot);
}
#endif
#ifdef SLE //// SLE NEW - experimental, show textured skybox in 3d view
void CRender3D::Draw2DSkybox(CMatRenderContextPtr &pRenderContext)
{
	int		i;
	Vector	normal;
	...
	/*const char **/ std::string skyname = "sky_day01_01";

	if (GetActiveWorld() && GetActiveWorld()->GetKeyValue("skyname") != NULL)
	{
		skyname = GetActiveWorld()->GetKeyValue("skyname");
	}
	for (int s = 0; s < 6; s++)
	{
		skies[s] = NULL;
		Q_snprintf(name, sizeof(name), "skybox/%s%s", skyname.c_str(), skyboxsuffix[s]);
		skies[s] = materials->FindMaterial(name, TEXTURE_GROUP_SKYBOX);

		if (IsErrorMaterial(skies[s]))
		{ // revert to default if trying to specify missing skybox
			Q_snprintf(name, sizeof(name), "skybox/sky_day01_01%s", skyboxsuffix[s]);
			skies[s] = materials->FindMaterial(name, TEXTURE_GROUP_SKYBOX);
		}
	...
	for (i = 0; i < 6; i++)
	{
		...
		if (skyboxMaterials[skytexorder[i]])
		{
			...
			MakeSkyVec(-1.0f, -1.0f, i, GetCamera()->GetFarClip(), positionArray[0], texCoordArray[0]);
			MakeSkyVec(-1.0f, 1.0f, i, GetCamera()->GetFarClip(), positionArray[1], texCoordArray[1]);
			MakeSkyVec(1.0f, 1.0f, i, GetCamera()->GetFarClip(), positionArray[2], texCoordArray[2]);
			MakeSkyVec(1.0f, -1.0f, i, GetCamera()->GetFarClip(), positionArray[3], texCoordArray[3]);
	...
	Do3DFog(pRenderContext);
//	Do3DWind(pRenderContext);
//	Do3DSnow(pRenderContext);
}

void CRender3D::SetupCamera(CCamera *pCamera)
{	
}
#endif //// SLE
	...
	if ( !IsPicking() )
	{
			...
			)
		{
				...
				( !g_pLPreviewOutputBitmap ) )
			{
				...
				{
#ifdef SLE //// SLE CHANGE - this class holds the ID unlike LightDesc_t
...
#ifdef SLE //// doesn't show otherwise, needs manual referencing
				add_0_to_1->AddRef();
#endif
...
// 				DIB_RGB_COLORS, SRCCOPY);
		...
		{
#ifndef SLE //// SLE REMOVE - don't bother
			//
			...
			Vector ViewPoint;
			GetCamera()->GetViewPoint(ViewPoint);
#ifdef SLE //// SLE NEW - report coordinates and camera angle
			int cameraYaw;
			float cameraPitch;
			cameraPitch = GetCamera()->GetPitch();
			cameraYaw = ( -90 - ( int )( GetCamera()->GetYaw() ) ) % 360;
			if ( cameraYaw < 180 && cameraYaw > -360 ) cameraYaw += 360;
			if ( cameraYaw < 0 && cameraYaw > -180 ) cameraYaw += 360;

			int nLen = sprintf(szText, "Pos=[%.f %.f %.f], Pitch=%.f, Ang=%i, FarZ = %.f, LOD = %i",
				ViewPoint[ 0 ], ViewPoint[ 1 ], ViewPoint[ 2 ], cameraPitch, cameraYaw, GetCamera()->GetFarClip(),
				Options.view3d.nLOD);
#else
			int nLen = sprintf(szText, "FPS=%3.2f Pos=[%.f %.f %.f]",
				m_fFrameRate, ViewPoint[ 0 ], ViewPoint[ 1 ], ViewPoint[ 2 ]);
#endif
			TextOut(m_WinData.hDC, 2, 18, szText, nLen);
		}
	}
#ifdef HAMMER2013_PORT_PROXIES
	float frameTime = Plat_FloatTime() - lastRenderTime;
#endif
}
...
void CRender3D::RenderWorldAxes()
{
	...
	meshBuilder3D.Color3ub(255, 32, 32);
	meshBuilder3D.Position3f(64, 0, 0);
	meshBuilder3D.AdvanceVertex();
	...
	meshBuilder3D.Color3ub(32, 255, 32);
	meshBuilder3D.Position3f(0, 64, 0);
	meshBuilder3D.AdvanceVertex();
	...
	meshBuilder3D.Color3ub(32, 32, 255);
	meshBuilder3D.Position3f(0, 0, 64);
	meshBuilder3D.AdvanceVertex();

	// draw letter 'X'
	meshBuilder3D.Color3ub(255, 32, 32);
	meshBuilder3D.Position3f(68, -1, -2);
	meshBuilder3D.AdvanceVertex();
	...
	// draw letter 'Z'
	meshBuilder3D.Color3ub(32, 32, 255);
	meshBuilder3D.Position3f(0, -2, 68);
	meshBuilder3D.AdvanceVertex();
...
#ifdef SLE //// SLE NEW - render world grid in 0, 0, 0 coordinates
	SetDrawColor(Color(128, 128, 128));

	Vector viewPoint, vOffset;
	viewPoint = Vector(0, 0, -0.05);
	...
	{
		Vector pos = viewPoint + vOffset * (fGrid*h);
		DrawLine(pos + Vector(0, 1, 0) * (fGrid*nSteps), pos - Vector(0, 1, 0) * (fGrid*nSteps));
	}

	vOffset = Vector(0,1,0);
	for (int v = -nSteps; v <= nSteps; v++)
	{
		Vector pos = viewPoint + vOffset * (fGrid*v);
		DrawLine(pos + Vector(1, 0, 0) * (fGrid*nSteps), pos - Vector(1, 0, 0) * (fGrid*nSteps));
	}
#endif
	PopRenderMode();

#ifdef SLE //// render the world grid
//	SetDrawColor(Color(128, 128, 128));
//	DrawPlane(Vector(-(fGrid*nSteps), -(fGrid*nSteps), -0.1), Vector(-(fGrid*nSteps), (fGrid*nSteps), -0.1), Vector((fGrid*nSteps), (fGrid*nSteps), -0.1), Vector((fGrid*nSteps), -(fGrid*nSteps), -0.1), true);
#endif
}
...
//		return;
	...
	{
		RenderTree(pDoc->GetMapWorld());
	}
	else
	{
		RenderMapClass(pDoc->GetMapWorld()); // Render the world without octree culling.
	}
...
#ifdef SLE //// SLE NEW - coloured pointfile :)
			float blue = (float)(i + 1) / (float)nPFPoints;
			meshBuilder3D.Color3f(blue, 0.0, (1.0f - blue));
#else
			meshBuilder3D.Color3ub(255, 0, 0);
#endif
			meshBuilder3D.AdvanceVertex();
		...
		while ( totalQuads > 0 )
		{
...
#ifdef SLE //// SLE CHANGE - thinner Portal File preview lines, looks better
			...
			for (; portalIndex < pDoc->m_pPortalFile->vertCount.Count(); portalIndex++)
			{
				...
				for ( int j = 0; j < vertCount; j++ )
				{
					...
					VectorNormalize(dir);
					dir *= edgeWidth;
#ifdef SLE //// SLE CHANGE - make Portal File lines less harsh looking
					int r = 60;
					int g = 60;
					int b = 255;
#else
...
//			chRed, chGreen, chBlue - Red, green, and blue color components for the box.
//-----------------------------------------------------------------------------
void CRender3D::RenderWireframeBox(const Vector &Mins, const Vector &Maxs,
									 unsigned char chRed, unsigned char chGreen, unsigned char chBlue
#ifdef SLE //// SLE NEW - option to render boxes with ignoreZ for multi-selections
	, bool bNoZ
#endif
)
{
	...
	//
#ifdef SLE
	if ( bNoZ ) PushRenderMode(RENDER_MODE_WIREFRAME_NOZ);
	else
#endif
	PushRenderMode( RENDER_MODE_WIREFRAME );
	SetDrawColor( chRed, chGreen, chBlue );
...
#ifdef SLE //// SLE NEW - 3d skybox preview, make Visgroups and Quick Hide work with it
			|| pMapClass->IsVisGroupShown() && !(m_pView->GetMapDoc()->QuickHide_IsObjectHidden(pMapClass)) 
			&& (pMapClass->Is3dSkybox() || pMapClass->GetParent() && pMapClass->GetParent()->Is3dSkybox())
#endif
			)
		{
#ifdef SLE
			...
			{
				CMapEntity *pMapEntity = dynamic_cast<CMapEntity*>(pMapClass);
				if ( pMapEntity != NULL && !pMapEntity->IsSelected() && !pMapEntity->IsHiddenAsEditorObject())
				{
					return;
				}
			}

			if (pMapClass->Is3dSkybox() || pMapClass->GetParent() && pMapClass->GetParent()->Is3dSkybox())
				m_bDrawing3DSkybox = true;
			else
				m_bDrawing3DSkybox = false;
#endif
			//
...
#ifdef SLE //// SLE NEW - 3d skybox preview
				if( pMapClass->Is3dSkybox()) 
					RenderWireframeBox(mins, maxs, 0, 255, 0);
				else 
#endif
...
//				should_appear &= pMapClass->ShouldAppearInRaytracedLightingPreview();
			...
			{
				if ((m_Pick.m_nFlags & FLAG_OBJECTS_AT_ONLY_SOLIDS) != 0)
				{
				...
				if ((m_Pick.m_nFlags & FLAG_OBJECTS_AT_ONLY_ENTITIES) != 0)
				{
					if (pMapClass->IsSolid())
					{
						should_appear = false;
					}
				}
...
void CRender3D::RenderInstanceMapClass_r(CMapClass *pMapClass)
{
	...
	if ( ( pMapClass != NULL ) && ( pMapClass->GetRenderFrame() != m_nInstanceCount ) )
	{
		if (pMapClass->IsVisible())
		{
#ifdef SLE //// SLE NEW: Editor objects display filter/toggle, make it play along with the instances
			if (Options.general.bShowEditorObjects)
			{
				CMapEntity *pMapEntity = dynamic_cast<CMapEntity*>(pMapClass);
				if (pMapEntity != NULL && !pMapEntity->IsSelected() && !pMapEntity->IsHiddenAsEditorObject())
...
void CRender3D::RenderOverlayElements(void)
{
	...
	if (m_RenderState.bCenterCrosshair)
		RenderCrossHair();
#ifdef SLE //// SLE TODO - do it somewhere else?
	if (g_bShowStatistics)
	{
		if (GetView()->GetDrawType() == VIEW3D_POLYGON
			|| GetView()->GetDrawType() == VIEW3D_TEXTURED
			|| GetView()->GetDrawType() == VIEW3D_TEXTURED_SHADED) // don't bother if in wireframe, light preview, lightmap, logical...
			RenderWorldText();
	}
#endif
	if ( bPopMode )
...
//-----------------------------------------------------------------------------
// Purpose: Renders text at position of chosen 3d entities.
//-----------------------------------------------------------------------------
void CRender3D::RenderWorldText(void)
{
	bool bPopMode = BeginClientSpace();
	
	//// SLE NEW: replicate point_message functionality in editor. Show their text above their entities.
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	if (pDoc)
	{
		CMapWorld *pWorld = pDoc->GetMapWorld();
		...
		{
			CMapEntityList matches;
			pWorld->FindEntitiesByClassName(matches, "point_message", true);

			for (int i = 0; i < matches.Count(); i++)
			{
				/*const char **/ std::string szMessageText = "";
				...
				if (pEntity != NULL)
				{
					...
					{
						if (pEntity->GetKeyValue("message") != NULL)
						{
							szMessageText = pEntity->GetKeyValue("message");
							Vector posWorld = Vector(0, 0, 0);
							Vector2D posWindow = Vector2D(0, 0);
							pEntity->GetOrigin(posWorld);
							TransformPoint(posWindow, posWorld);
							RoundVector(posWindow);

							Vector cameraPos, cameraAxis;
							GetCamera()->GetViewPoint(cameraPos);
							GetCamera()->GetViewForward(cameraAxis);

							if (IsPointInCone(posWorld, cameraPos, cameraAxis, cos(GetCamera()->GetFOV()), GetCamera()->GetFarClip()))
							{
								PushRenderMode(RENDER_MODE_DEFAULT);
								Color remColor;
								GetDrawColor(remColor);
								SetDrawColor(Color(255, 255, 255, 255));
								SetTextColor(255, 255, 255, 255);
								DrawText(szMessageText.c_str(), posWindow.x, posWindow.y + 4, CRender3D::TEXT_JUSTIFY_TOP | TEXT_CLEAR_BACKGROUND);
								SetDrawColor(remColor);
								PopRenderMode();
							}
						}
					}
...
void CRender3D::ResetFocus()
{
	...
	// sets some weird state that causes the whole screen to not be updated

...
#ifdef SLE //// SLE CHANGE - at default 5 it's too strong/fast
	m_flPaintValueGeo = 1.0f;
#else
...
bool CToolDisplace::OnLMouseDown3D( CMapView3D *pView, UINT nFlags, const Vector2D &vPoint )
{
...
#ifdef SLE //// SLE CHANGE - fix Face Normal disp painting
	//
	...
	//	return true; // don't return here, else locking with shift won't work
	}
#endif
	// Selection.
...
#ifdef SLE //// SLE NEW - disp vertex locking toolbar button
		|| Options.IsDispVertexLockEnabled())
#endif
		) 
	{
...
void CToolDisplace::LiftFaceNormal( CMapView3D *pView, const Vector2D &vPoint )
{
	...
	else
	{
		...
		pDisp->GetSurfNormal(vFaceNormal);

		m_vecPaintAxis = vFaceNormal;
	}
#else
	//
...
#ifdef SLE //// SLE CHANGE - no need to take entities into account, speeds up selection; instances should also be permitted
...
void CToolDisplace::RenderPaintSphere( CRender3D *pRender )
{
...
#ifdef SLE //// SLE CHANGE - this shouldn't be excluded
	else if (nPaintAxis == DISPPAINT_AXIS_FACE)
	{
		pDisp->GetSurfNormal(vPaintAxis);
	}
#endif
	float flBloat = flRadius * 0.15f;
	pRender->RenderArrow( vCenter, vCenter + ( vPaintAxis * ( flRadius + flBloat ) ), 255, 255, 0 );

#ifdef SLE	//// SLE NEW: render the back end of the axis too - useful when the arrow goes into the displacement
	...
	for ( int iAxis = 0; iAxis < 3; iAxis++ )
	{
		...
		vBoxMin[iAxis] = vCenter[iAxis] - 1; 
		vBoxMax[iAxis] = vCenter[iAxis] + 1;
#else
...
#ifdef SLE //// SLE NEW: draw vert coordinates for displace tool
	if (Options.general.bShowHelpers)
	{
		if (pRender->GetView()->GetDrawType() == VIEW3D_POLYGON
			|| pRender->GetView()->GetDrawType() == VIEW3D_TEXTURED
			|| pRender->GetView()->GetDrawType() == VIEW3D_TEXTURED_SHADED)
		{
		//	Vector vVert;
		...
		//		if (IsPointInBox(vVert,
		//			-Vector(vCenter.x - flRadius, vCenter.y - flRadius, vCenter.z - flRadius),
		//			Vector(vCenter.x + flRadius, vCenter.y + flRadius, vCenter.z + flRadius)))
		//		{
					Vector posWorld = vCenter;
					Vector2D posWindow = Vector2D(0, 0);
					pRender->TransformPoint(posWindow, posWorld);
					//	RoundVector(posWindow);

				//	Vector cameraPos, cameraAxis;
				//	pRender->GetCamera()->GetViewPoint(cameraPos);
				//	pRender->GetCamera()->GetViewForward(cameraAxis);

				//	if (IsPointInCone(posWorld, cameraPos, cameraAxis, cos(pRender->GetCamera()->GetFOV()), pRender->GetCamera()->GetFarClip()))
					{
						char szBuf[128];
						sprintf(szBuf, "%.1f, %.1f, %.1f", vCenter.x, vCenter.y, vCenter.z);
						pRender->SetTextColor(255, 255, 255, 255);
						pRender->DrawText(szBuf, posWindow.x - 4, posWindow.y - 2, CRender3D::TEXT_JUSTIFY_TOP | CRender3D::TEXT_CLEAR_BACKGROUND);
					}
		//		}
		//	}
		}
...
#ifdef SLE //// SLE CHANGE - this shouldn't be excluded
	else if (paintAxis == DISPPAINT_AXIS_FACE)
	{
		pDisp->GetSurfNormal(vecPaint);
	}
#endif
	// render the normal
...
#ifdef SLE //// SLE NEW: render the back end of the axis too - useful when the arrow goes into the displacement
	pRender->RenderArrow(hbOrigin - (vecPaint * length4), hbOrigin, 255, 255, 0);
#endif
}

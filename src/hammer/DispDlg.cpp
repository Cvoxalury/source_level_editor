...
#define DISPPAINT_DISTANCE_MIN			1
#ifdef SLE //// 
...
//=============================================================================
//
// Displacement Create Dialogue Functions
//
...
//=============================================================================
//
// Displacement Noise Dialogue Functions
//
...
//=============================================================================
//
// Displacement Paint Geometry Dialogue Functions
//
...
#ifdef SLE	//// SLE NEW - fixes AutoSew button forgetting to be set. 
	//// NOTE there are TWO autosew buttons - one in Paint Geo, one in Sculpt. This is for Paint Geo!
	m_AutoSew.SetCheck(pTool->GetAutoSew());
#endif
	return TRUE;
}
...
#ifdef SLE //// SLE NEW - disp free drag brush
		case DISPPAINT_EFFECT_FREEDRAG: { pFilterMgr = pTool->GetFilterRaiseLowerMgr(); break;}
#endif
		default: return;
		}
...
bool CDispPaintDistDlg::InitComboBoxAxis( void )
{
...
#ifdef SLE	//// SLE NEW: diagonal axii!!!
	strPaintDir = "+45 XY";
	m_comboboxAxis.AddString(strPaintDir);
	...
	{
#ifdef SLE
		m_comboboxAxis.SetCurSel( 2 ); //// Z axis by default
#else
...
#ifdef SLE
	DDX_Control(pDX, ID_DISP_PAINT_DIST_AUTOSEW, m_AutoSew); //// SLE NEW - fixes AutoSew button forgetting to be set. 
#endif
	//}}AFX_DATA_MAP										 //// NOTE there are TWO autosew buttons - one in Paint Geo, one in Sculpt. This is for Paint Geo!
}
...
#ifdef SLE					   //// SLE NEW: diagonal axii!!!
	case DISPPAINT_AXIS_p45: { pTool->SetPaintAxis(nAxis, Vector(1.0f, 1.0f, 0.0f)); return; }
	case DISPPAINT_AXIS_n45: { pTool->SetPaintAxis(nAxis, Vector(-1.0f, 1.0f, 0.0f)); return; }
#endif						   ////
...
#ifdef SLE		//// SLE NEW - fixes AutoSew button forgetting to be set. 
		//// NOTE there are TWO autosew buttons - one in Paint Geo, one in Sculpt. This is for Paint Geo!
		if((m_AutoSew.GetCheck() == 0 && pTool->GetAutoSew() == true) || (m_AutoSew.GetCheck() != 0 && pTool->GetAutoSew() == false))
#endif
			pTool->ToggleAutoSew();
	}
}
...
#ifdef SLE //// SLE NEW - disp free drag brush
void CDispPaintDistDlg::OnEffectFreeDrag(void)
{
	// get the displacement tool
	CToolDisplace *pTool = GetDisplacementTool();
	if( pTool )
...
void CDispPaintDistDlg::OnEditDistance( void )
{
	...
	if ( pTool )
	{
		...
		{
			CString str;
			str.Format("pen pressure is %.3f", WinTab_GetPressure());
			AfxMessageBox(str);
			flDistance *= WinTab_GetPressure();
		}
#endif
...
#ifdef SLE	//// SLE NEW - fixes AutoSew button forgetting to be set. 
			//// NOTE there are TWO autosew buttons - one in Paint Geo, one in Sculpt. This is for Paint Geo!
		m_AutoSew.SetCheck(pDispTool->GetAutoSew());
#endif
	}
...
//=============================================================================
//
// Displacement Paint Alpha Dialogue Functions
//
...
bool CDispPaintDataDlg::InitComboBoxBrushData(void)
{
	...
	if (pDispTool)
	{
		...
		case DISPPAINT_EFFECT_FREEDRAG:
		{
			pDispTool->SetEffect(DISPPAINT_EFFECT_FREEDRAG);
			SetEffectButtonData(DISPPAINT_EFFECT_FREEDRAG);
			FilterComboBoxBrushData(DISPPAINT_EFFECT_FREEDRAG, true);
			break;
		}
#endif
		default:
...
#ifdef SLE //// SLE NEW - disp free drag brush
		case DISPPAINT_EFFECT_FREEDRAG: { pFilterMgr = pDispTool->GetFilterRaiseLowerMgr(); break; }
#endif
		default: return;
		}
...
#ifdef SLE //// SLE NEW - disp free drag brush
void CDispPaintDataDlg::OnEffectFreeDragData(void)
{
	// get the displacement tool
	CToolDisplace *pDispTool = GetDisplacementTool();
	if (pDispTool)
	{
		pDispTool->SetEffect(DISPPAINT_EFFECT_FREEDRAG);
		SetEffectButtonData(DISPPAINT_EFFECT_FREEDRAG);
		FilterComboBoxBrushData(DISPPAINT_EFFECT_FREEDRAG, false);
		OnComboBoxBrushData();
	}
}
#endif
...
//=============================================================================
//
// Displacement Sculpt & Blending Dialog Functions
//
...
#ifdef SLE //// SLE NEW: Restore 'Project' function and implement multi-texutre Blending function
...
#endif
	m_ProjectButton.EnableWindow( TRUE );
...
#ifdef SLE //// SLE NEW - fixes AutoSew button forgetting to be set. 
			//// NOTE there are TWO autosew buttons - one in Paint Geo, one in Sculpt. This is for Sculpt!
void CPaintSculptDlg::OnPaint()
{
	CPaintDC dc(this); // device context for painting
	
	m_AutoSew.SetCheck(true);
}
#endif
//-----------------------------------------------------------------------------

	...
	: CDialog(CPasteSpecialDlg::IDD, pParent)
{
	...
	m_bCenterOnCamera = FALSE;
	m_bUseCameraAngles = FALSE;
#endif
...
#ifdef SLE //// SLE CHANGE - grid and offset can be a float now
	m_fOffsetX = atof(p);
	m_fOffsetY = atof(strchr(p, ' ')+1);
	m_fOffsetZ = atof(strrchr(p, ' ')+1);
#else
	m_iOffsetX = atoi(p);
...
#ifdef SLE //// SLE CHANGE - grid and offset can be a float now, round up to one decimal point
	str.Format("%.1f %.1f %.1f", m_fOffsetX, m_fOffsetY, m_fOffsetZ);
#else
...
#ifdef SLE //// SLE CHANGE - grid and offset can be a float now
	DDX_Text(pDX, IDC_OFFSETX, m_fOffsetX);
	DDX_Text(pDX, IDC_OFFSETY, m_fOffsetY);
	DDX_Text(pDX, IDC_OFFSETZ, m_fOffsetZ);
#else
...
#ifdef SLE //// SLE NEW - option to paste at camera origin for Paste Special
	DDX_Check(pDX, IDC_CENTERCAMERA, m_bCenterOnCamera);
	DDX_Check(pDX, IDC_CENTERCAMERAANGLES, m_bUseCameraAngles);
#endif

...
#ifdef SLE //// SLE CHANGE - re-enable
//#if 0
...
//		m_SpatialData.m_flRadius = m_StartingProjectedRadius * 1.5f;
	...
	GetPaintAxis( pView->GetCamera(), vPoint, vPaintAxis );
#ifdef SLE
	pDisp->GetSurfNormal(vDirection);
#else
	vDirection = vPaintAxis * m_Direction;
#endif
	switch( m_OffsetMode )
...
#ifdef SLE //// SLE NEW - Restore project sculpting function
...
{
#ifdef SLE_USE_STB
	m_FileDialog = new CFileDialog(TRUE, NULL, NULL, OFN_LONGNAMES | OFN_NOCHANGEDIR | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_READONLY, 
		"Image Files (*.jpg;*.png;*.bmp;*.tga;*.psd)|*.jpg; *.png; *.bmp; *.tga; *.psd||");
#else
	...
	m_pTexture = NULL;
	m_pMaterial = NULL;
#ifdef SLE //// SLE CHANGE - make the projection larger
	m_ProjectX = 256;
	m_ProjectY = 256;
	m_ProjectWidth = 256;
	m_ProjectHeight = 256;
...
#ifdef SLE //// SLE CHANGE - make the projection larger
	m_ProjectLocation.Init( 256, 256, 0.0f );
	m_OriginalProjectLocation = m_ProjectLocation;
	m_ProjectSize.Init( 256, 256, 0.0f );
#else
...
#ifdef SLE
	ON_BN_CLICKED(IDC_RESET_IMAGE, &CSculptProjectOptions::OnBnClickedResetImage)
#endif
...
void CSculptProjectOptions::DoPaintOperation( CMapView3D *pView, const Vector2D &vPoint, CMapDisp *pDisp, CMapDisp *pOrigDisp )
{
	...
	vPaintAxis *= ( m_ProjectSizeControl.GetPos());
#else
...
#ifdef SLE
void CSculptProjectOptions::OnBnClickedResetImage()
{
	m_ProjectSize.x = 256;
	m_ProjectSize.y = 256;
	m_TileWidth = m_OriginalTileWidth;
	m_TileHeight = m_OriginalTileHeight;
...
bool CSculptProjectOptions::ReadImage( CString &FileName )
{
	...
	m_pTexture = dynamic_cast< ITextureInternal * >( g_pMaterialSystem->CreateProceduralTexture("SculptProject", TEXTURE_GROUP_OTHER, m_Width, m_Height, IMAGE_FORMAT_BGRX8888,
...
/*
	...
	{
		const uint newWidth = SmallestPowerOfTwoGreaterOrEqual(m_Width);
		const uint newHeight = SmallestPowerOfTwoGreaterOrEqual(m_Height);

		auto resizedPixels = malloc(newWidth * newHeight * comp);
		stbir_resize(newPixels, m_Width, m_Height, m_Width * comp,
			resizedPixels, newWidth, newHeight, newWidth * comp,
			STBIR_TYPE_UINT8, comp, -1, 0, STBIR_EDGE_CLAMP, STBIR_EDGE_CLAMP,
			STBIR_FILTER_MITCHELL, STBIR_FILTER_MITCHELL, STBIR_COLORSPACE_LINEAR, nullptr);
		free(newPixels);
		newPixels = reinterpret_cast<byte*>( resizedPixels );
		m_Width = newWidth;
		m_Height = newHeight;
	}
*/
	...
	CString str; str.Format("%s", ( m_pMaterial->FindVar("$basetexture", NULL, false)->GetStringValue() )); AfxMessageBox(str, MB_OK);
...
//	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	...
	char	temp[ 128 ];
	sprintf( temp, "%d", m_ProjectSizeControl.GetPos());
...
#ifdef SLE
	m_ProjectSizeControl.SetRange( 1, 256 );
#else
...
#ifdef SLE //// SLE NEW - Implement multi-texture blending function //// SLE TODO - bring it back
#if 0

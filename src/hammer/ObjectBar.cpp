...
//
// Purpose: 
// SLE TODO: 1) add stairs tool 2) add geosphere, or icosphere 
// 3) add regular polyhedrons (dodecahedron, etc)
// 4) add cupolas
// 5) add chamfered boxes
// 6) add trapezohedron
//=============================================================================
...
{
	{"block", 0, 6, 6, false, false},
	{"wedge", 0, 5, 5, false, false}, // todo: add angle support for wedges later
...
SolidTypeInfo_t SolidTypes[] =
{
	{"block", 0, 6, 6, false,},
	{"wedge", 0, 5, 5, false},
#ifdef SLE //// SLE CHANGE - bumped up defaults for solids
	{"cylinder", 12, 3, 64, true},
	{"pyramid", 4, 3, 32, true}, // renamed for clarity
#else
...
SolidTypeInfo_t ExtendedSolidTypes[] =
{
	{"bipyramid", 8, 6, 64, true, true}, // things like regular octahedron
//	{"trapezohedron", 8, 3, 64, true, true},
//	{"dodecahedron", 0, 20, 20, false, true},
	{"icosahedron", 0, 20, 20, false, true},
//	{"cupola", 6, 2, 64, true, true},
	{"elongated bipyramid", 8, 3, 64, true, true},
	{"gyroelongated bipyramid", 8, 3, 64, true, true},
	{"antiprism", 8, 2, 64, true, true},
};
#endif
...
#ifdef SLE //// SLE NEW - more stock solids
static int FindExtSolidType(const char *pName)
{
	for ( int i = 0; i < ARRAYSIZE (ExtendedSolidTypes); i++ )
	{
		if ( Q_stricmp(pName, ExtendedSolidTypes[ i ].pszName) == 0 )
			return i;
	}
	return -1;
}
...
bool CObjectBar::UseAlignOnEntityPlacement()
{
	return ::SendMessage(::GetDlgItem(GetSafeHwnd(), IDC_ALIGNTOSURFACE), BM_GETCHECK, 0, 0) == BST_CHECKED;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
CMapClass *CObjectBar::CreateInBox(BoundBox *pBox, CMapView *pView)
{
	...
	if(ListType == listPrimitives)
	{
...
#ifdef SLE //// SLE NEW - set start angle for block tool
		int nAngle;
		m_StartAngle.GetWindowText(szBuf, 128);
		nAngle = atoi(szBuf);
#endif
		//
		...
		//
		// Ensure that the face count is within legal range (if applicable).
		//
...
#ifdef SLE //// SLE NEW - set start angle for block tool
				pStock->SetFieldData(StockWedge::fieldStartAngle, nAngle);	
#endif
				break;
			case 2:
...
#ifdef SLE //// SLE NEW - set start angle for block tool
				pStock->SetFieldData(StockCylinder::fieldStartAngle, nAngle);	
#endif
				break;
			case 3:
				pStock = new StockPyramid;
				pStock->SetFieldData(StockPyramid::fieldSideCount, nFaces);
#ifdef SLE //// SLE NEW - set start angle for block tool
				pStock->SetFieldData(StockPyramid::fieldStartAngle, nAngle);	
#endif
				break;
			default:
...
#ifdef SLE //// SLE NEW - set start angle for block tool
				pStock->SetFieldData(StockSphere::fieldStartAngle, nAngle);	
#endif
				break;
			}
...
#ifdef SLE //// SLE NEW - more stock solids
	else if ( ListType == listExtendedPrimitives )
	{
		int nFaces;
		char szBuf[128];
		m_Faces.GetWindowText(szBuf, 128);
		...
		// The index into the solid types array is stored in the item data.
		//
		int nExtendedSolidIndex = _iNewObjIndex;

		int nFacesMin = ExtendedSolidTypes[nExtendedSolidIndex].nFacesMin;
		int nFacesMax = ExtendedSolidTypes[nExtendedSolidIndex].nFacesMax;

		//
		// Ensure that the face count is within legal range (if applicable).
		//
		if ((ExtendedSolidTypes[nExtendedSolidIndex].bEnableFaceControl) && (nFaces < nFacesMin || nFaces > nFacesMax))
		{
			CString str;
			str.Format("The face count for a %s must be in the range of %d to %d.",
				ExtendedSolidTypes[nExtendedSolidIndex].pszName, nFacesMin, nFacesMax);
			AfxMessageBox(str);
			return NULL;
		}

		if ( nExtendedSolidIndex == 0 ) // new bipyramid
		{
			StockSolid *pStock = NULL;
			pStock = new StockBipyramid;
			pStock->SetFieldData(StockBipyramid::fieldSideCount, nFaces);
#ifdef SLE //// SLE NEW - set start angle for block tool
			pStock->SetFieldData(StockBipyramid::fieldStartAngle, nAngle);	
#endif
		...
		else if ( nExtendedSolidIndex == 1 ) // new icosahedron
		{
			StockSolid *pStock = NULL;
			pStock = new StockIcosahedron;
#ifdef SLE //// SLE NEW - set start angle for block tool
			pStock->SetFieldData(StockIcosahedron::fieldStartAngle, nAngle);	
#endif
		...
		/*
		else if ( nExtendedSolidIndex == 4 ) // new cupola
		{
			StockSolid *pStock = NULL;
			pStock = new StockCupola;
			pStock->SetFieldData(StockCupola::fieldSideCount, nFaces); // top faces, bottom is top x 2
		...
		*/
		else if ( nExtendedSolidIndex == 2 ) // new elongated bipyramid
		{
			StockSolid *pStock = NULL;
			pStock = new StockElongatedBipyramid;
			pStock->SetFieldData(StockElongatedBipyramid::fieldSideCount, nFaces);
#ifdef SLE //// SLE NEW - set start angle for block tool
			pStock->SetFieldData(StockElongatedBipyramid::fieldStartAngle, nAngle);	
#endif
		...
		else if ( nExtendedSolidIndex == 3 ) // new gyroelongated bipyramid
		{
			StockSolid *pStock = NULL;
			pStock = new StockGyroElongatedBipyramid;
			pStock->SetFieldData(StockGyroElongatedBipyramid::fieldSideCount, nFaces);
#ifdef SLE //// SLE NEW - set start angle for block tool
			pStock->SetFieldData(StockGyroElongatedBipyramid::fieldStartAngle, nAngle);	
#endif
		...
		else if ( nExtendedSolidIndex == 4 ) // new antiprism
		{
			StockSolid *pStock = NULL;
			pStock = new StockAntiprism;
			pStock->SetFieldData(StockAntiprism::fieldSideCount, nFaces);
#ifdef SLE //// SLE NEW - set start angle for block tool
			pStock->SetFieldData(StockAntiprism::fieldStartAngle, nAngle);	
#endif
...
bool CObjectBar::IsBlockToolCreatingPrimitive( void )
{
	if( ( m_iLastTool == TOOL_BLOCK ) && ( m_CategoryList.GetCurSel() == 0 ) )
		return true;

	return false;
}

bool CObjectBar::IsBlockToolCreatingExtendedPrimitive( void )
{
	if( ( m_iLastTool == TOOL_BLOCK ) && ( m_CategoryList.GetCurSel() == 1 ) )
		return true;

	return false;
}

bool CObjectBar::IsBlockToolCreatingPrefab( void )
{
	if( ( m_iLastTool == TOOL_BLOCK ) && ( m_CategoryList.GetCurSel() == 2 ) )
		return true;

	return false;
}

int CObjectBar::GetBlockToolActiveCategory(void)
{
	return m_CategoryList.GetCurSel();
}

int CObjectBar::GetBlockToolActivePrimitive(void)
{
	return _iNewObjIndex;
}

int CObjectBar::GetBlockToolFaceCount(void)
{	
	if( m_iLastTool != TOOL_BLOCK || m_CategoryList.GetCurSel() > 1) return 0;
...
int CObjectBar::GetBlockToolStartAngle(void)
{
	if( m_iLastTool != TOOL_BLOCK || m_CategoryList.GetCurSel() > 1) return 0;
...
#ifdef SLE //// SLE NEW: after creating a prefab, reset entity tool to Entities.
//-----------------------------------------------------------------------------
// Purpose: after creating a prefab, reset entity tool to Entities. 
//-----------------------------------------------------------------------------
void CObjectBar::ResetEntityTool(void)
{
	if (m_iLastTool == TOOL_ENTITY)
	{
...
#ifdef SLE
void CObjectBar::OnChangeFaceCount()
{
#ifdef SLE //// - with 2d/3d preview working, it's useful to update all views automatically	
...
void CObjectBar::OnChangeFaceSpin()
{
#ifdef SLE //// - with 2d/3d preview working, it's useful to update all views automatically	
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	if ( pDoc )
...
#ifdef SLE //// SLE NEW - set start angle for block tool
	m_StartAngle.SubclassDlgItem(IDC_STARTANGLE, this);
	m_StartAngleSpin.SubclassDlgItem(IDC_STARTANGLESPIN, this);
	m_StartAngleSpin.SetBuddy(&m_StartAngle);
#endif
	iBlockSel = -1;
	iEntitySel = -1;
	m_iLastTool = -1;
...
#ifdef SLE //// SLE NEW - more stock solids
void CObjectBar::LoadExtBlockCategories(void)
{
	m_CategoryList.SetRedraw(FALSE);

	// first item is the primitive list
//	m_CategoryList.ResetContent();
	m_CategoryList.AddString("Extended Primitives");
...
#ifdef SLE //// SLE NEW - more stock solids
void CObjectBar::LoadExtBlockItems(void)
{
	//
	// verify the block categories are loaded
	//
	...
	//
	// load extended primitive items
	//
	if ( m_CategoryList.GetCurSel() == 0 )
	{
		LoadBlockItems();
	}
	else if( m_CategoryList.GetCurSel() == 1 )
	{
		// set list type (primitives)
		ListType = listExtendedPrimitives;

		// set list type (primitives)
		CUtlVector<CString> suggestions;
		for( int i = 0; i < sizeof( ExtendedSolidTypes ) / sizeof( ExtendedSolidTypes[0] ); i++)
			suggestions.AddToTail( ExtendedSolidTypes[i].pszName );
...
#ifdef SLE //// SLE NEW - set start angle for block tool
		m_StartAngle.ShowWindow(SW_SHOW);
		m_StartAngleSpin.ShowWindow(SW_SHOW);
#endif
	}
...
#ifdef SLE //// SLE NEW - set start angle for block tool
		m_StartAngle.ShowWindow(SW_HIDE);
		m_StartAngleSpin.ShowWindow(SW_HIDE);
#endif
	}
...
#ifdef SLE //// SLE NEW - align placed entity to be perpendicular to surface
	GetDlgItem( IDC_ALIGNTOSURFACE )->ShowWindow(bShow ? SW_SHOW : SW_HIDE);
#endif
}
...
BOOL CObjectBar::EnableFaceControl(CWnd *pWnd, BOOL bModifyWnd)
{
	...
	if(bEnable)
	{
		...
		{
			int nExtSolidIndex = iBlockSel;
			if ( ExtendedSolidTypes[ nExtSolidIndex ].bEnableFaceControl )
			{
				bEnable = TRUE;
			}
		}
...
#ifdef SLE //// SLE NEW - set start angle for block tool
BOOL CObjectBar::EnableAngleControl(CWnd *pWnd, BOOL bModifyWnd)
{
	BOOL bEnable = CMapDoc::GetActiveMapDoc() ? TRUE : FALSE;
	...
	if(bEnable)
	{
		...
		//
		// Enable the control only if we are dealing with an object the
		// that has adjustable start angle.
		//
		if (ListType == listPrimitives)
		{
			int nSolidIndex = iBlockSel;
			if (SolidTypes[nSolidIndex].bEnableAngleControl)
			{
				bEnable = TRUE;
			}
		}
		...
		{
			int nExtSolidIndex = iBlockSel;
			if ( ExtendedSolidTypes[ nExtSolidIndex ].bEnableAngleControl )
			{
				bEnable = TRUE;
			}
		}
...
void CObjectBar::OnTextChanged( const char *pSelection )
{
	...
	switch (ListType)
	{
		...
		case listExtendedPrimitives:
		{
			_iNewObjIndex = FindExtSolidType(pSelection);
			Assert(_iNewObjIndex != -1);
			...
			if ( iBlockSel != -1 )
			{
				int nExtSolidIndex = iBlockSel;
				m_Faces.GetWindowText(szBuf, 128);
				ExtendedSolidTypes[nExtSolidIndex].nFaces = atoi(szBuf);
			}
			iBlockSel = _iNewObjIndex;
			break;
		}
...
#ifdef SLE //// SLE NEW - set start angle for block tool
		EnableAngleControl(&m_StartAngle, TRUE);
		EnableAngleControl(&m_StartAngleSpin, TRUE);
		m_StartAngleSpin.SetRange(0, 360);
		m_StartAngleSpin.SetPos(0);
#endif
	}
	else if ( ListType == listExtendedPrimitives )
	{
		...
		int nExtSolidIndex = _iNewObjIndex;

		m_FacesSpin.SetRange(ExtendedSolidTypes[ nExtSolidIndex ].nFacesMin, ExtendedSolidTypes[ nExtSolidIndex ].nFacesMax);
		m_FacesSpin.SetPos(ExtendedSolidTypes[ nExtSolidIndex ].nFaces);

		itoa(ExtendedSolidTypes[ nExtSolidIndex ].nFaces, szBuf, 10);
...
#ifdef SLE //// SLE NEW - set start angle for block tool
void CObjectBar::UpdateAngleControl(CCmdUI *pCmdUI)
{ 
	pCmdUI->Enable(EnableAngleControl(GetDlgItem(pCmdUI->m_nID), FALSE));
}

// this makes sure the previews are updated in all viewports
void CObjectBar::OnChangeStartAngle()
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	if ( pDoc )
	{
...
void CObjectBar::OnChangeStartAngleSpin()
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	if ( pDoc )
	{

...
#ifdef SLE //// SLE CHANGE - decals can only be placed on brushes, skip entities for speed; and allow placing in instances
...
#ifdef SLE //// SLE CHANGE - reset the selection to avoid having multiple overlays selected as a group unless intended
		m_pDocument->SelectObject(pEntity, scSelect|scClear);
#else
...
#ifdef SLE //// SLE CHANGE - decals can only be placed on brushes, skip entities for speed; and allow placing in instances
	int nHits = pView->ObjectsAt( vPoint, Objects, sizeof( Objects ) / sizeof( Objects[0] ), FLAG_OBJECTS_AT_RESOLVE_INSTANCES | FLAG_OBJECTS_AT_ONLY_SOLIDS);
#else
...
void CToolOverlay::OnDrag( Vector const &vecRayStart, Vector const &vecRayEnd, bool bShift )
{
	...
	if ( iFace == nFaceCount )
		return;
#ifdef SLE //// SLE CHANGE - overlays now obey snap to grid unless holding ALT (or snap is off globally)
	if( m_pDocument && m_pDocument->IsSnapEnabled() && ( GetKeyState(VK_MENU) & 0x8000 ) == 0 )
		m_pDocument->Snap(vecImpact, constrainSnap);
#endif
	if ( bShift )
	{
...
#ifdef SLE //// SLE NEW - switch to pointer tool on escape
...
void CToolOverlay::OnEscape(void)
{
	//

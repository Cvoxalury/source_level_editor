...
void CMapAnimationDlg::SelectionChanged(const CMapObjectList *pSelection)
#else
...
#endif
{
	...
	if (pSelection->Count() > 0 && pSelection->Count() < 2)
	{
		...
		if (pSelectedEntity->IsAnimationController())
		{
			m_bEnabled = true;
		}
	}
	...
	if (m_bEnabled)
	{
		if (pSelectedEntity != NULL && pSelectedEntity->IsAnimationController())
		{
			CMapAnimator *anim = pSelectedEntity->GetChildOfType((CMapAnimator*)NULL);
			ASSERT(anim != NULL);

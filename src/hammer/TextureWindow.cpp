...
#ifdef SLE //// SLE CHANGED - edited texture window params
const int iTexNameFontHeight = 9;
#else
...
#ifdef SLE //// SLE CHANGED - more inclusive determination of 'specular'
				pMaterial->FindVar("$envmap", &bSpecular, false);
#else
				if ( pMaterial->GetMaterialVarFlag( MATERIAL_VAR_BASEALPHAENVMAPMASK ) )
#endif
				if( bSpecular)
				{
					if (m_nTypeFilter & TYPEFILTER_SPECULAR)
						bFound = true;
				}

#ifdef SLE //// SLE CHANGED - extended criteria for what qualifies as 'translucent'.				
				if ( pMaterial->GetMaterialVarFlag( MATERIAL_VAR_TRANSLUCENT ) 
					|| pMaterial->GetMaterialVarFlag(MATERIAL_VAR_ALPHATEST) 
...
#ifndef SLE //// SLE CHANGE - do this always
	if (m_bEnableUpdate)
#endif
	{
		UpdateScrollSizes();
...
#ifdef SLE_TEXTUREBROWSER_FAVOURITES
//	if ( !GetFocus()) m_bMouseOver = false;

	if ( m_bMouseOver )
	{
		dc.SetTextColor(RGB(64, 0, 0));
	}
#endif
}
...
{
#ifdef SLE_TEXTUREBROWSER_FAVOURITES
	if ( !m_bShouldUpdateTextureSelection )
	{
	// UNDONE, needs more work as without updating it resets it completely
	//	return;
	}
#endif
...
#ifdef SLE_TEXTUREBROWSER_FAVOURITES
	m_bMouseOver = true;
	m_iLastClickTimeStamp = Plat_MSTime();
#endif

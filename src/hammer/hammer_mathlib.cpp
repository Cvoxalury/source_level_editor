...
void polyMake( float x1, float  y1, float x2, float y2, int npoints, float start_ang, Vector *pmPoints, bool round)
{
    int		point;
    ...
    for( point = 0; point < npoints; point++, angle += angle_delta )
    {
		...
		if (!round)
		{
			pmPoints[point][0] = (xCenter + (sin(DEG2RAD(angle)) * (float)xrad)); //// SLE REMOVE - no rounding for spheres
			pmPoints[point][1] = (yCenter + (cos(DEG2RAD(angle)) * (float)yrad));
		}
		else
		{

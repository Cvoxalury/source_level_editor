...
CMapFace::CMapFace(void)
{
	...
	{
#ifdef SLE
		m_pLightmapGrid = g_Textures.FindActiveTexture("editor/lightmapgrid");
#else
...
void CMapFace::UpdateFaceFlags( void )
{
	...
	m_nFaceFlags = 0;
	if (strstr(tname,"tools") || strstr(tname, "skybox"))
	{
...
void CMapFace::JustifyTextureUsingExtents(TextureJustification_t eJustification, Extents_t Extents)
{
	...
	if ( eJustification == TEXTURE_JUSTIFY_FIT_WIDTH )
	{
		texture.scale[ 0 ] = 1.0;
	}
	if ( eJustification == TEXTURE_JUSTIFY_FIT_HEIGHT )
	{
		texture.scale[ 1 ] = 1.0;
	}
	if ( eJustification == TEXTURE_JUSTIFY_FIT_025
		|| eJustification == TEXTURE_JUSTIFY_FIT_05
		|| eJustification == TEXTURE_JUSTIFY_FIT_2X2
		|| eJustification == TEXTURE_JUSTIFY_FIT_3X3
		|| eJustification == TEXTURE_JUSTIFY_FIT_4X4
		|| eJustification == TEXTURE_JUSTIFY_FIT_8X8
		|| eJustification == TEXTURE_JUSTIFY_FIT_16X16
		)
	{
		texture.scale[ 0 ] = 1.0;
		texture.scale[ 1 ] = 1.0;
	}
	if ( eJustification == TEXTURE_JUSTIFY_FIT_DEFAULT )
	{
		texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		texture.scale[ 1 ] = g_pGameConfig->GetDefaultTextureScale();
	}
	...
	switch (eJustification)
	{
		...
		{
			texture.VAxis[3] = (int)(-BottomRight[1] + m_pTexture->GetHeight());
			break;
		}
		...
		{
			texture.UAxis[3] = (int)(-BottomRight[0] + m_pTexture->GetWidth());
			break;
		}
		...
		{
			texture.UAxis[3] = (int)(-Center[0] + (m_pTexture->GetWidth() / 2));
			texture.VAxis[3] = (int)(-Center[1] + (m_pTexture->GetHeight() / 2));
			break;
		}
...
#ifdef SLE //// SLE NEW - extra fit modes
		case TEXTURE_JUSTIFY_FIT_WIDTH:
		{
			// Calculate the appropriate scale.
			if ( m_pTexture && m_pTexture->GetWidth() )
			{
				texture.scale[ 0 ] = ( BottomRight[ 0 ] - TopLeft[ 0 ] ) / m_pTexture->GetWidth();
			} 
			else
		...
		case TEXTURE_JUSTIFY_FIT_HEIGHT:
		{
			// Calculate the appropriate scale.
			if ( m_pTexture && m_pTexture->GetHeight() )
			{
				texture.scale[ 1 ] = ( BottomRight[ 1 ] - TopLeft[ 1 ] ) / m_pTexture->GetHeight();
			} 
			else
		...
		case TEXTURE_JUSTIFY_FIT_025:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 0.25f );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 0.25f );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_05:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 0.5f );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 0.5f );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_2X2:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 2.0 );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 2.0 );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_3X3:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 3.0 );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 3.0 );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_4X4:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 4.0 );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 4.0 );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_8X8:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 8.0 );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 8.0 );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_16X16:
		{
			if ( m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight() )
			{
				texture.scale[ 0 ] = ( float )( BottomRight[ 0 ] - TopLeft[ 0 ] ) / ( float )( m_pTexture->GetWidth() * 16.0 );
				texture.scale[ 1 ] = ( float )( BottomRight[ 1 ] - TopLeft[ 1 ] ) / ( float )( m_pTexture->GetHeight() * 16.0 );
			} 
			else
			{
				texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_FIT_DEFAULT:
		{
			texture.scale[ 0 ] = g_pGameConfig->GetDefaultTextureScale();
			texture.scale[ 1 ] = g_pGameConfig->GetDefaultTextureScale();
		...
		case TEXTURE_JUSTIFY_RANDOM_X:
		{
			texture.UAxis[ 3 ] = -TopLeft[ 0 ] + RandomInt(-1024,1024);
			break;
		}

		case TEXTURE_JUSTIFY_RANDOM_Y:
		{
			texture.VAxis[ 3 ] = -TopLeft[ 1 ] + RandomInt(-1024, 1024);
			break;
		}
#endif
	}
							 ...
							 bool ignoreLighting, Color &pColor )
{
...
#ifdef SLE //// SLE CHANGE - more lightmap tints
		else if (texture.nLightmapScale <= 2 )
		{
			pColor[0] = 255;
			pColor[1] = 150;
			pColor[2] = 50;
		}
		else if (texture.nLightmapScale <= 8 )
		{
			pColor[0] = 255;
			pColor[1] = 225;
			pColor[2] = 100;
		}
		else if (texture.nLightmapScale > 8 && texture.nLightmapScale < 16)
		{
			pColor[0] = 255;
			pColor[1] = 255;
			pColor[2] = 100;
		}
		else if (texture.nLightmapScale >= 16 && texture.nLightmapScale < 32)
		{
			pColor[0] = 255;
			pColor[1] = 255;
			pColor[2] = 255;
		}
		else if (texture.nLightmapScale >= 32 && texture.nLightmapScale < 64)
		{
			pColor[0] = 150;
			pColor[1] = 255;
			pColor[2] = 255;
		}
		else if (texture.nLightmapScale >= 64 && texture.nLightmapScale < 128)
		{
			pColor[0] = 100;
			pColor[1] = 255;
			pColor[2] = 100;
		}
		else if (texture.nLightmapScale >= 128)
		{
			pColor[0] = 100;
			pColor[1] = 150;
			pColor[2] = 255;
	...
	case RENDER_MODE_WIREFRAME_NOZ:
#endif
		if (bRenderAsSelected)
...
#endif	
#ifdef SLE //// used for see-thru solid edges
	MaterialPrimitiveType_t type = (mode == RENDER_MODE_WIREFRAME || mode == RENDER_MODE_WIREFRAME_NOZ) ? 
#else
...
//	IMesh *pMesh = pRenderContext->CreateStaticMesh(fmt, TEXTURE_GROUP_OTHER);
    ...
    for (int nPoint = 0; nPoint < nPoints; nPoint++)
    {
...
#ifdef SLE //// SLE NEW: 3d skybox preview - move the face points to coordinates X sky camera scale, if those faces are marked as 3d skybox
        ...
        if ( hasParent )
        {
			...
			{
				meshBuilder.Position3f((point[0] + skyDelta[0]) * skyScale, (point[1] + skyDelta[1]) * skyScale, (point[2] + skyDelta[2]) * skyScale);
			}
			else
#endif
			{
        ...
        else
        {
			...
			{
				meshBuilder.Position3f((Points[nPoint][0] + skyDelta[0]) * skyScale, (Points[nPoint][1] + skyDelta[1]) * skyScale, (Points[nPoint][2] + skyDelta[2]) * skyScale);
			}
			else
#endif
			{
...
//-----------------------------------------------------------------------------
// Draws a batch of faces. // this draws all the opaque faces on brushes.
//-----------------------------------------------------------------------------
...
void CMapFace::RenderFaces( CRender3D* pRender, int nCount, MapFaceRender_t **ppFaces )
{
	...
	for ( int nFace = 0; nFace < nCount; nFace++ )
	{
		...
		if ( ( ( nVertexCount + newVertices ) > nMaxVerts ) || ( ( nIndexCount + newIndices )  > nMaxIndices ) )
		{
			...
			AssertMsg(( nFace - nBatchStart ) > 0, "A face is too big for rendering meshbuilder to handle! Look for faces with texture '%s'", ppFaces[nFace]->m_pTexture->GetName());
...
//-----------------------------------------------------------------------------
// draws a single face (displacement or regular)
//-----------------------------------------------------------------------------
void CMapFace::RenderFace3D( CRender3D* pRender, Vector& viewPoint, EditorRenderMode_t renderMode, bool renderSelected, SelectionState_t faceSelectionState )
{
	...
	// Draw the texture axes
#ifdef SLE //// used for see-thru solid edges
	if( renderMode == RENDER_MODE_WIREFRAME || renderMode == RENDER_MODE_WIREFRAME_NOZ)
#else
...
{
#ifdef SLE //// used for see-thru solid edges
	if (mode == RENDER_MODE_WIREFRAME || mode == RENDER_MODE_WIREFRAME_NOZ)
#else
...
#ifdef SLE
			mapFace.m_pMapFace->RenderFace3D( pRender, viewPoint, mapFace.m_RenderMode, mapFace.m_RenderSelected, mapFace.m_FaceSelectionState );
#else
...
{	
	...
	if ( HasDisp() && ( bIsParallel || bRenderSelected ) )
	{
		...
		bool bDrawDispMap = Options.view2d.bDrawModels && ( (sizeX+sizeY) > 20 || bRenderSelected ); //// SLE NEW: display disps in 2d views from farther away
...
#ifdef SLE //// SLE NEW - display selected solids in filled mode
		if (bRenderSelected)
		{
...
void CMapFace::Render3D( CRender3D *pRender )
{
	...
	//
	EditorRenderMode_t eCurrentRenderMode = pRender->GetCurrentRenderMode();
#ifndef SLE //// SLE REMOVE - not used
...
#ifdef SLE //// SLE CHANGE - a different method of sorting nodraws
	if (!Options.general.bShowNoDrawBrushes && eSolidSelectionState == SELECT_NONE && m_pTexture->HasVariable("%compilenodraw"))
#else
	if ( !Options.general.bShowNoDrawBrushes && eSolidSelectionState == SELECT_NONE && m_pTexture == g_Textures.GetNoDrawTexture() )
#endif	
	return;
#ifdef SLE
	//// SLE NEW: Tool Brush Texture display toggle/filter.
	bool isToolBrushTexture = false;

	//// SLE NEW: ToolsSky Texture display toggle/filter. 
	bool isToolsSkyTexture = false;

	if (!V_strncmp(m_pTexture->GetFileName(), "tools/", 6)) // if it's anything from the tools folder, then...
	{
		...
		{
			isToolsSkyTexture = true;
		}
		else if (V_strncmp(m_pTexture->GetFileName(), "tools/toolsblack", 16) // and if it's not black, white, or nodraw, count it as a tool texture.
			&& V_strncmp(m_pTexture->GetFileName(), "tools/toolswhite", 16) 
			&& V_strncmp(m_pTexture->GetFileName(), "tools/toolsnodraw", 17))
		{
			isToolBrushTexture = true;
		}
	}

	if (!Options.general.bShowToolBrushFaces && eSolidSelectionState == SELECT_NONE && isToolBrushTexture)
		return;
	
//	if (m_pTexture == g_Textures.GetToolTexture_Sky() ||
//		m_pTexture == g_Textures.GetToolTexture_Sky2D()) isToolsSkyTexture = true;

	if (!Options.general.bShowToolsSkyFaces && eSolidSelectionState == SELECT_NONE && isToolsSkyTexture)
		return;
#endif //// SLE
	//
	// Draw the face.
	//
#ifdef SLE //// SLE CHANGE - don't tint selected solids if helpers are off
	...
	// don't do the split route of rendering opaques in a batch. Render everything in skybox
	// as individual faces.
	if (!Options.general.bShowToolsSkyFaces && (Is3dSkybox() ||( GetParent() && GetParent()->Is3dSkybox())))
	{
		// Set up the texture to use
		pRender->BindTexture(m_pTexture);

		RenderFace3D(pRender, ViewPoint, eCurrentRenderMode, renderSelected, eFaceSelectionState);
		if ((renderSelected && pRender->NeedsOverlay()))
		{
			RenderFace3D(pRender, ViewPoint, RENDER_MODE_SELECTION_OVERLAY, renderSelected, eFaceSelectionState);
		}
	}
	else
#endif 
...
#ifdef SLE //// SLE NEW - 3d skybox preview. Blow up the cull box so it's always drawn for faces marked as 3d sky
	if (!Options.general.bShowToolsSkyFaces && (Is3dSkybox() || (GetParent() && GetParent()->Is3dSkybox())))
	{
		boundMin = -Vector(MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2);
		boundMax = Vector(MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2);
		return true;
	}
#endif
	//
...
void CMapFace::DoTransform(const VMatrix &matrix)
{
	...
	if ( GetTexture() )
	{
		...
		//// SLE TODO - this could be the place to fix UV Shearing being broken by mirroring/rotating
...
#ifdef SLE
//-------------------------------------------------------------------
// Purpose: neighbouring faces are those that share at least 1 vertex
// *in - pass in a list of all the world's solid's faces
// &out - returns a new list of faces that share verts with the current one
CMapFaceList CMapFace::GetAllNeighbouringFaces(CMapFaceList *in)
{
	CMapFaceList list;
	int myPointCount = GetPointCount();
	Vector myPoint;
	for ( int i = 0; i < in->Count(); i++ )
	{
		CMapFace *face = ( CMapFace* )( in->Element(i) );
		for ( int j = 0; j < face->GetPointCount(); j++ )
		{
			Vector facePoint;
			face->GetPoint(facePoint, j);
...
//-------------------------------------------------------------------
// Purpose: adjacent faces are those that share at least 2 vertices
// *in - pass in a list of all the world's solid's faces
// &out - returns a new list of faces that share verts with the current one
void CMapFace::GetAllAdjacentFaces(CMapFaceList &in, CMapFaceList &out)
{
//	CMapFaceList *list;
	int myPointCount = GetPointCount();

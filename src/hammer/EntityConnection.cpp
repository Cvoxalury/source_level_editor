...
#ifdef SLE //// SLE CHANGE - only visible entities here
			pWorld->FindEntitiesByName( matches, m_szSourceEntity, true );
#else
...
void CEntityConnection::LinkTargetEntities()
{
	...
	if (pDoc)
	{
		...
		if (pWorld)
		{
			CMapEntityList matches;
#ifdef SLE //// SLE CHANGE - only visible entities here
			pWorld->FindEntitiesByName( matches, m_szTargetEntity, true );
#else
...
#ifdef SLE //// SLE CHANGE - allow any names whatsoever that start with !. A single check.
	if ( !strncmp(pszTarget, "!", 1) )
	{
		return true;
	}
#else

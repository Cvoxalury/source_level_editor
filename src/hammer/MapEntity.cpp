...
void CMapEntity::GetRenderColor( CRender2D *pRender, unsigned char &red, unsigned char &green, unsigned char &blue )
{
	if ( IsSelected() )
	{
		...
		red = GetRValue(Options.colors.clr2DSelection);
		green = GetGValue(Options.colors.clr2DSelection);
		blue = GetBValue(Options.colors.clr2DSelection);
#else
...
#ifdef SLE	//// SLE NEW: automatically texture some of the classes with appropriate
			//// textures. I. e. triggers with toolstrigger, areaportals with toolsareaportal...
//-----------------------------------------------------------------------------
// Purpose: 
// Inputs : pszClass - 
//			bLoading - True if this is being called from Postload, false otherwise.
//-----------------------------------------------------------------------------
static BOOL ApplyTextureToSolidClass (CMapSolid *pSolid, LPCTSTR pszTexture)
{
...
void CMapEntity::SetClass(LPCTSTR pszClass, bool bLoading)
{
	...
	if (IsSolidClass()) // means we're a solid entity
	{
		if (!Q_strnicmp(GetClassName(), "trigger_", strlen("trigger_")))
		{
			const CMapObjectList *pSelList = pDoc->GetSelection()->GetList();
...
void CMapEntity::SignalChanged( void )
{
	if ( m_EntityTypeFlags & ENTITY_FLAG_IS_LIGHT )
	{
		...
		CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
		if ( pDoc && pDoc->GetMapWorld() && pDoc->GetBSPLighting() ) pDoc->InternalUpdateLightPreview();
		{
			//SignalUpdate( EVTYPE_LIGHTING_CHANGED ); // should be revised in the future
		}
#else
...
void CMapEntity::PostloadWorld(CMapWorld *pWorld)
{
	...
	if (!stricmp(GetClassName(), "env_fog_controller"))
	{		
		CMapWorld *pWorld = (CMapWorld *)GetWorldObject(this);
		if (pWorld)
		{
			float fog_start, fog_end, fog_density;
			Vector fog_color1, fog_color2;

			pszValue = m_KeyValues.GetValue("fogstart", &nIndex);
			if (pszValue != NULL)
			{
				fog_start = atof(pszValue);
				RemoveKey(nIndex);
			}

			pszValue = m_KeyValues.GetValue("fogend", &nIndex);
			if (pszValue != NULL)
			{
				fog_end = atof(pszValue);
				RemoveKey(nIndex);
			}

			pszValue = m_KeyValues.GetValue("fogmaxdensity", &nIndex);
			if (pszValue != NULL)
			{
				fog_density = atof(pszValue);
				RemoveKey(nIndex);
			}

			pszValue = m_KeyValues.GetValue("fogcolor", &nIndex);
			if (pszValue != NULL)
			{
				sscanf(pszValue, "%f %f %f", &fog_color1[0], &fog_color1[1], &fog_color1[2]);
				RemoveKey(nIndex);
			}

			pszValue = m_KeyValues.GetValue("fogcolor2", &nIndex);
			if (pszValue != NULL)
			{
				sscanf(pszValue, "%f %f %f", &fog_color2[0], &fog_color2[1], &fog_color2[2]);
				RemoveKey(nIndex);
			}

			pWorld->SetFogParams(fog_color1, fog_color2, fog_start, fog_end, fog_density);
		}
	}
#endif
	//
...
#ifdef SLE	//// SLE NEW - de-assign referenced faces from env_cubemap.
			// Usually it doesn't make sense to have a cloned, moved away cubemap still
			// referencing these faces - it won't be near them after it's moved.
	if (!stricmp(GetClassName(), "env_cubemap"))
	{
		((CMapEntity *)pClone)->DeleteKeyValue("sides");
	}
...
void CMapEntity::OnKeyValueChanged(const char *pszKey, const char *pszOldValue, const char *pszValue)
{
	...
	{
		UpdateAllDependencies(this); //// SLE todo: fix how goddamn slow this is.
	}
#ifdef SLE_CACHE_FOG_PARAMS
	//// SLE NEW
	if (!stricmp(GetClassName(), "env_fog_controller"))
	{
		...
		if (pWorld)
		{
			...
			const char	*scanValue = m_KeyValues.GetValue("fogstart", &nIndex);
			if (scanValue != NULL)
			{
				fog_start = atof(scanValue);
				RemoveKey(nIndex);
			}

			scanValue = m_KeyValues.GetValue("fogend", &nIndex);
			if (scanValue != NULL)
			{
				fog_end = atof(scanValue);
				RemoveKey(nIndex);
			}

			scanValue = m_KeyValues.GetValue("fogmaxdensity", &nIndex);
			if (scanValue != NULL)
			{
				fog_density = atof(scanValue);
				RemoveKey(nIndex);
			}

			scanValue = m_KeyValues.GetValue("fogcolor", &nIndex);
			if (scanValue != NULL)
			{
				sscanf(scanValue, "%f %f %f", &fog_color1[0], &fog_color1[1], &fog_color1[2]);
				RemoveKey(nIndex);
			}

			scanValue = m_KeyValues.GetValue("fogcolor2", &nIndex);
			if (scanValue != NULL)
			{
				sscanf(scanValue, "%f %f %f", &fog_color2[0], &fog_color2[1], &fog_color2[2]);
				RemoveKey(nIndex);
			}
...
void CMapEntity::OnAddToWorld(CMapWorld *pWorld)
{
	...
	//
	// If we are a node class, we must ensure that we have a valid unique ID.
	//
	if (IsNodeClass())
	{
...
#ifdef SLE	//// SLE NEW - placing a new entity (path_corner or path_track)
			// while having an entity of the same class selected, and holding down SHIFT
			// (similar to clone-dragging w/ holding SHIFT), names the newly added
			// entity in continuation with the selected entity, and links them together.
			// In other words: this works like clone-dragging but doesn't require actually
			// moving the old entity. This works better for 3d view and wanting to place
			// entity chains in uneven landscape.
	...
	if (bShift)
	{
		...
		if( pSelection->Count() == 1) // Note: things get messy if you try and chain more than one thing at a time. This limit is arbitrary, but intentional.
		{
			// SLE TODO: make this FGD - driven instead of hardcoded
			bool isPath = (!stricmp(GetClassName(), "path_corner") || !stricmp(GetClassName(), "path_track"));
			bool isKeyframe = (!stricmp(GetClassName(), "move_rope") || !stricmp(GetClassName(), "keyframe_rope"));
			if( isPath || isKeyframe)
			{
				CMapEntity *pSelectedEntity = dynamic_cast<CMapEntity*>(pSelection->Element(0));
				Assert(pSelectedEntity != NULL);
				if (!pSelectedEntity)
					return;

				// create a new targetname for the clone
				char newName[128];
				const char *oldName = pSelectedEntity->GetKeyValue("targetname");
				if (!oldName || oldName[0] == 0)
				{
					if( isPath ) oldName = "path";
					else if (isKeyframe) oldName = "keyframe";
				}

				pWorld->GenerateNewTargetname(oldName, newName, sizeof(newName), true, NULL);
				SetKeyValue("targetname", newName);

				// point the current path or keyframe at the clone
				if (isPath)
				{
					pSelectedEntity->SetKeyValue("target", newName); 
					if (!stricmp(GetClassName(), "path_track"))
					{
						const char *pradius = pSelectedEntity->GetKeyValue("radius");
						if (pradius)
						{
							SetKeyValue("radius", pradius);
						}
					}
				}
				else if (isKeyframe)
				{
					pSelectedEntity->SetKeyValue("NextKey", newName);

					// copy our slack, subdivision and width params
					const char *slack = pSelectedEntity->GetKeyValue("Slack");
					if (slack)
					{
						SetKeyValue("Slack", slack);
					}
					const char *subdiv = pSelectedEntity->GetKeyValue("Subdiv");
					subdiv = GetKeyValue("Subdiv");
					if (subdiv)
					{
						SetKeyValue("Subdiv", subdiv);
					}
					const char *width = pSelectedEntity->GetKeyValue("Width");
					width = GetKeyValue("Width");
					if (subdiv)
					{
						SetKeyValue("Width", width);
					}
					const char *tscale = pSelectedEntity->GetKeyValue("TextureScale");
					tscale = GetKeyValue("TextureScale");
					if (tscale)
					{
						SetKeyValue("TextureScale", tscale);
					}
				}
			}
		}
	...
	if (!stricmp(GetClassName(), "env_fog_controller"))
	{
		...
		if (pWorld)
		{
			...
			const char *pszValue = m_KeyValues.GetValue("fogstart", &nIndex);
			if (pszValue != NULL)
			{
				fog_start = atof(pszValue);
				RemoveKey(nIndex);
...
{		    
#ifdef SLE //// SLE CHANGE - don't render entities with 0 active inputs or outputs
	if ( Connections_GetCount() == 0 ) return;
#endif
	// Render all our children (helpers & solids)
...
bool CMapEntity::IsLogical(void)
{
	GDclass *pClass = GetClass();
#ifdef SLE
	return pClass && ( ( pClass->GetInputCount() > 0 ) || ( pClass->GetOutputCount() > 0 ) );
#else
...
#ifdef SLE //// SLE NEW: 3d skybox preview
void CMapEntity::UpdateSkyCameraVariables()
{
	if ( !stricmp(GetClassName(), "sky_camera") )
	{
		CMapWorld *pWorld = ( CMapWorld * )GetWorldObject(this);
		if ( pWorld )
		{
			Vector delta = vec3_origin;
			GetOrigin(delta);
			pWorld->SetSkyCameraDelta(-delta);

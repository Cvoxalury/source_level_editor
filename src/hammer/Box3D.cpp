...
#ifdef SLE //// SLE NEW - prototyping 3d gizmo
//#define NEW_3DMOVE_GIZMOS
#endif
...
#ifdef SLE	//// SLE NEW: rotation pivot (render in 2d)
...
{
#ifdef SLE //// SLE NEW: rotation pivot (render the rotation pivot in 3d)
	if ( m_pDocument && m_pDocument->m_use_rotation_pivot_bool /*&& IsTranslating() && GetTranslateMode() == modeRotate*/ )
	{
		Vector vecViewPoint;
		pRender->GetCamera()->GetViewPoint(vecViewPoint);
		float flDist = ( m_pDocument->m_rotation_pivot_vec - vecViewPoint ).Length();

		pRender->PushRenderMode(RENDER_MODE_FLAT_NOZ);
		pRender->RenderSphere(m_pDocument->m_rotation_pivot_vec, flDist * 0.04, 12, 12, 255, 128, 128);
		pRender->PopRenderMode();

		// draw a line connecting the box center to the pivot
		pRender->PushRenderMode(RENDER_MODE_WIREFRAME_NOZ);
		pRender->SetDrawColor(255, 255, 0);
		pRender->DrawLine(m_pDocument->m_rotation_pivot_vec, ( ( mins + maxs ) / 2 ));
		pRender->PopRenderMode();
	}
#endif
...
#ifdef NEW_3DMOVE_GIZMOS	
	//// SLE NEW: draw movement arrows in 3d view
	if (m_TranslateMode == modeScale)
	{
		pRender->RenderArrow(vCenter, vCenter + Vector(0, 0, vDelta.Length() * 1.5), 50, 50, 200);
		pRender->RenderArrow(vCenter, vCenter + Vector(0, -vDelta.Length() * 1.5, 0), 200, 50, 50);
		pRender->RenderArrow(vCenter, vCenter + Vector(vDelta.Length() * 1.5, 0, 0), 50, 200, 50);
	}
#endif
	if (m_TranslateMode == modeRotate)
	{
...
#ifdef SLE //// SLE NEW: rotation pivot (detect cursor hits on the pivot)
	m_hit_rotation_pivot_bool = false;
	if ( m_pDocument && m_pDocument->m_use_rotation_pivot_bool /*&& IsTranslating() && GetTranslateMode() == modeRotate*/ )
	{
		if ( HitRect(pView, ptClient, m_pDocument->m_rotation_pivot_vec, HANDLE_RADIUS))
		{
			m_hit_rotation_pivot_bool = true;
		//	m_LastHitTestHandle = m_pDocument->m_rotation_pivot_vec;
			bHit = true;
...
#ifdef NEW_3DMOVE_GIZMOS	
	//// SLE NEW: detect hit on movement arrows
	if (m_TranslateMode == modeScale)
	{
		Vector pos = vCenter;
		for (int i = 0; i < 3; i++)
		{
			if (i == 0)
				pos = vCenter + Vector(0, 0, vDelta.Length() * 1.4);
			else if (i == 1)
				pos = vCenter + Vector(0, -vDelta.Length() * 1.4, 0);
			else if (i == 2)
				pos = vCenter + Vector(vDelta.Length() * 1.4, 0, 0);

			if (HitRect(pView, ptClient, pos, 16))
			{
...
//-----------------------------------------------------------------------------
bool Box3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
...
bool Box3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	const Vector *pCustomHandleBox )
{
	...
	else
	{
		...
		if (m_TranslateMode == modeRotate)
		{			
#ifdef SLE //// SLE NEW: rotation pivot (get the point from map document)
			if ( m_pDocument && m_pDocument->m_use_rotation_pivot_bool )
			{
				m_vTranslationFixPoint = m_pDocument->m_rotation_pivot_vec;
			}
			else
#endif
		...
		if (!IsEmpty())
		{
			...
			switch (m_eWorldUnits)
			{
				...
				case Units_Feet_Inches:
				{
					...
					int nFeetTall = (int)fabs(size.z) / 12;
					int nInchesTall = (int)fabs(size.z) % 12;

					sprintf(pszBuf, " %d' %d\"w %d' %d\"l %d' %d\"h", nFeetWide, nInchesWide, nFeetLong, nInchesLong, nFeetTall, nInchesTall);
					break;
				}
#ifdef SLE
				case Units_Centimeters: //// SLE NEW - show units in cm
				{
					float fCMWide = (int)fabs(size.x) * 2.54;
					float fCMLong = (int)fabs(size.y) * 2.54;
					float fCMTall = (int)fabs(size.z) * 2.54;

					sprintf(pszBuf, "%.3fcm wide %.3fcm long %.3fcm tall", fCMWide, fCMLong, fCMTall);
					break;
				}

				case Units_Meters_Centimeters: //// SLE NEW - show units in m + cm
				{
					float fMCMWide = (int)fabs(size.x) * 0.0254;
					float fMCMLong = (int)fabs(size.y) * 0.0254;
					float fMCMTall = (int)fabs(size.z) * 0.0254;

					sprintf(pszBuf, " %.4fm wide %.4fm long %.4fm tall", fMCMWide, fMCMLong, fMCMTall);
					break;
				}
#endif
			}
...
#ifdef SLE //// SLE NEW: Avoid divide by zero.
					if (!fDistance) fDistance = 1;
					//	Error("Divide by zero in Box3D::GetVisibleHandles() function");
#endif
...
bool Box3D::UpdateTranslation(const Vector &vUpdate, UINT uConstraints)
{
	...
	else if ( m_TranslateMode == modeRotate )
	{
		...
		if ( uConstraints & constrainSnap )
		{
			...
			float snap = 15.0;
			switch ( Options.view2d.iRotateConstrainAngle )
			{
				case 0:
					snap = 0.25f;
					break;
			...
			angle += (float)(snap / 2);
			angle -= fmod(double(angle), double(snap));
#else
			angle += 7.5;
...
#ifdef SLE
		LimitBox( newMins, newMaxs, g_MAX_MAP_COORD * 4 ); 
#else
...
//-----------------------------------------------------------------------------
#ifdef SLE
void Box3D::ToggleTranslateMode( bool allowShear )
#else
...
#endif
{
	...
	{
#ifdef SLE
		m_TranslateMode = allowShear ? modeShear : modeScale;
#else
...
void Box3D::RenderTool2D(CRender2D *pRender)
{
	...
	pRender->PopRenderMode();	
#ifdef SLE
	if (false) //(!IsTranslating())
	{
		//// SLE NEW: draw selection box as a flat colour box. Doesn't look too good, but maybe use the code elsewhere
		pRender->PushRenderMode(RENDER_MODE_TRANSLUCENT_FLAT);
		pRender->SetDrawColor(Color(63, 127, 127, 16));
		pRender->DrawRectangle(mins, maxs, true, 0);
		pRender->PopRenderMode();
	}
#endif
...
#ifdef NEW_3DMOVE_GIZMOS
	CGizmo *pGizmo = new CGizmo(0, 0, 0);
	pGizmo->Initialize();
	pGizmo->SetPosition(Vector((bmaxs + bmins)/2));
	pGizmo->Render(pRender);
#endif

...
#ifdef SLE //// SLE CHANGE - enable "thicklines" for better visibility
	SetDrawFlags(Box3D::expandbox | Box3D::boundstext | thicklines);
#else
...
void Selection3D::SetEmpty(void)
{
	...
	if (m_bSelected) m_bSelected = NULL;
#endif
...
void Selection3D::RenderTool2D(CRender2D *pRender)
{
	if ( !m_pSelection->IsEmpty() && IsTranslating() && !IsBoxSelecting() )
	{
		...
		COLORREF clr = Options.colors.clr2DSelection;
#else
...
bool Selection3D::OnContextMenuLogical(CMapViewLogical *pView, UINT nFlags, const Vector2D &vPoint) 
{
	...
	if (!bInit)
	{
		bInit = true;
		menu.LoadMenu(IDR_POPUPS);
#ifdef SLE //// moved down by one in SLE
		menuSelection.Attach(::GetSubMenu(menu.m_hMenu, 9));
#else
...
#ifdef SLE //// SLE NEW - add context menu in 3d view
bool Selection3D::OnContextMenu3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint)
{
	static CMenu menu, menuSelection, menu3DGlobal;
	static bool bInit = false;
	...
	if (!bInit)
	{
		...
		menu.LoadMenu(IDR_POPUPS);
		menuSelection.Attach(::GetSubMenu(menu.m_hMenu, 0));
		menu3DGlobal.Attach(::GetSubMenu(menu.m_hMenu, 10));
	}
	...
	if (!IsEmpty() && !IsBoxSelecting())
	{
		...
		else
		{
			CPoint ptScreen(vPoint.x, vPoint.y);
			pView->ClientToScreen(&ptScreen);
			menu3DGlobal.TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_LEFTALIGN, ptScreen.x, ptScreen.y, pView);
			return true;
		}
	}
...
#ifdef SLE //// SLE NEW: rotation pivot (move/update document's pivot to the cursor pos)
	if ( m_pDocument && m_pDocument->m_use_rotation_pivot_bool && m_hit_rotation_pivot_bool )
	{
		m_updating_rotation_pivot_bool = true;
		Vector vecWorld;
		pView->ClientToWorld(vecWorld, vPoint);

		//	m_pDocument->SetRotationPivotPos(vecWorld);
		//	return true;
	}
	else
	{
		m_updating_rotation_pivot_bool = false;
	}
#endif
...
unsigned int Selection3D::GetConstraints(unsigned int nKeyFlags)
{
	...
	if ( m_TranslateMode==modeRotate )
	{
		...
		if ( 
#ifndef SLE //// SLE REMOVE: holding ALT already does it, and we want holding SHIFT to make rotate-cloning possible. 
			(nKeyFlags & MK_SHIFT) || 
#endif
			!Options.view2d.bRotateConstrain )
		{
			uConstraints = 0;
		}
	}
...
bool Selection3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint) 
{
	...
	if ( m_updating_rotation_pivot_bool ) return true;
#endif
...
#ifdef SLE //// SLE NEW: rotation pivot (move/update document's pivot to the cursor pos)
	if ( m_bMouseDragged[MOUSE_LEFT] && m_pDocument && m_pDocument->m_use_rotation_pivot_bool && m_hit_rotation_pivot_bool )
	{
	//	m_pDocument->SetRotationPivotPos(vecWorld);
	//	return true;
	}
...
#ifdef SLE //// SLE NEW: rotation pivot (override the case for point ents)
				if ( m_pDocument && m_pDocument->m_use_rotation_pivot_bool && GetTranslateMode() == modeRotate )
				{
					pRefPoint = &(m_pDocument->m_rotation_pivot_vec);
				}
				else
#endif
				pRefPoint = &refPoint;
...
#ifdef SLE //// SLE NEW: rotation pivot (move/update document's pivot to the cursor pos)
	if ( m_pDocument && m_pDocument->m_use_rotation_pivot_bool && m_updating_rotation_pivot_bool )
	{
		Vector vecWorld;
		pView->ClientToWorld(vecWorld, vPoint);
...
bool Selection3D::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	switch (nChar)
	{
#ifdef SLE //// SLE NEW: press R to cycle through 3d Edit modes - faster than left-clicking (which calls the SUPER slow scene-pick function)
		case 'r':
		case 'R':
		{
			if (m_b3DEditMode)
			{
				ToggleTranslateMode( false );
				pView->UpdateView(MAPVIEW_UPDATE_TOOL);
			}
			return true;
		}
...
#ifdef SLE //// SLE NEW - allow double clicking to bring up object properties in 2d view
//-----------------------------------------------------------------------------
// Purpose: bring up object properties in 2d view
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseDblClk2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	if (!m_pSelection->IsEmpty())
	{
		if (m_pSelection->GetCount() == 1)
...
bool Selection3D::OnLMouseDown3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint) 
{
	...
	m_bSelected = false;
#ifndef SLE //// SLE REMOVE - this isn't used
	//
	...
	else if ( m_b3DEditMode && HitTest(pView,vPoint, true) )
	{
		...
		//// SLE NEW: not checking hit tests unless we're multi-selecting (holding ctrl) or clicking outside the box, 
			// makes it **much** less painful to move and rotate objects in 3d mode. (it starts immediately
			// instead of waiting/lagging while hit-testing every object in the frame).
			// the downside is clicking doesn't change their transform mode... BUT it's already
			// taken care of by the R hotkey.
		pView->ObjectsAt( vPoint, &HitData, 1 );

#ifndef SLE //// SLE NEW: Disallow clicking within the selected object's bounding box but outside the object.
	...
	{
#ifdef SLE	//// SLE NEW: don't check for, or change, selection unless it's clear (user hit ESC) or looking away (current sel. not in camera pov)
		/*
		Vector cameraPos, cameraAxis, selectionPos;
		pView->GetCamera()->GetViewPoint(cameraPos);
		pView->GetCamera()->GetViewForward(cameraAxis);
		if (m_pSelection) m_pSelection->GetBoundsCenter(selectionPos);
		bool selectedNotOnScreen = true;

		if (IsPointInCone(selectionPos, cameraPos, cameraAxis, cos(pView->GetCamera()->GetFOV() / 2), pView->GetCamera()->GetFarClip()))
			selectedNotOnScreen = false;

		if (!m_bSelected || (m_bSelected && m_pSelection && selectedNotOnScreen))
		*/
#endif
		{
			m_TranslateMode = modeScale;
...
#ifndef SLE //// SLE REMOVED: Confusing feature, not sure what was the intended purpose of it.
...
#ifdef SLE //// SLE NEW - add context menu in 3d view
bool Selection3D::OnRMouseUp3D(CMapView3D *pView, UINT nFlags, const Vector2D &vPoint)
{

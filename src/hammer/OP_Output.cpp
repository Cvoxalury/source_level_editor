...
#ifdef SLE //// SLE NEW - indicate the type of parameter in output param field
/* // todo, figure out later
const char *PARAM_STRING_FLOAT = "<float>";
const char *PARAM_STRING_INT = "<integer>";
const char *PARAM_STRING_BOOL = "<bool>";
const char *PARAM_STRING_STRING = "<string>";
const char *PARAM_STRING_TARGET = "<target>";
const char *PARAM_STRING_VECTOR2D = "<vector2d>";
const char *PARAM_STRING_VECTOR3D = "<vector3d>";
const char *PARAM_STRING_VECTOR4D = "<vector4d>";
const char *PARAM_STRING_COLOR = "<color (RGB, 0-255)>";
*/
#endif
//
	...
	: CObjectPage(COP_Output::IDD), m_ComboTarget( this )
{
	...
	m_fDelay = 0;
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
	m_iTimesToFire = EVENT_FIRE_ALWAYS;
#endif
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
				char buf[ MAX_PATH ];
				int iTimesToFire = pConnection->GetTimesToFire();
				sprintf(buf, "%i", iTimesToFire);
				m_ListCtrl.SetItemText(nItemCount, ONLY_ONCE_COLUMN, buf);
#else
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
	DDX_Text(pDX, IDC_EDIT_CONN_TIMESTOFIRE, m_iTimesToFire);
#else
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
	CEdit *pTimesToFireEdit = ( CEdit * )GetDlgItem(IDC_EDIT_CONN_TIMESTOFIRE);
	pTimesToFireEdit->EnableWindow(bValue);
#else
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
		pTimesToFireEdit->SetWindowText("-1");
#endif
	}
}
...
void COP_Output::UpdateEditControls(void)
{
	...
	if (m_EditList.Count() > 0)
	{
		...
		sprintf(szTemp2, "%i", m_iTimesToFire);
		pTimesToFireEdit->SetWindowText(szTemp2);
#else
...
#endif
#ifdef SLE //// SLE NEW - Mark Button for entities in output argument param
void COP_Output::OnMarkParam(void)
{
	int			nCount = m_ListCtrl.GetItemCount();
	CMapDoc*	pDoc = CMapDoc::GetActiveMapDoc();
		...
		for ( int nItem = nCount - 1; nItem >= 0; nItem-- )
		{
			if ( m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED )
			{
				...
				if ( pDocActive != NULL )
				{
					...
					{
						CMapEntityList Found;
						pDocActive->FindEntitiesByName(Found, strParam, false);
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
	m_ListCtrl.InsertColumn(ONLY_ONCE_COLUMN, "Times To Fire (-1 = always)", LVCFMT_LEFT, 150);
#else
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times To Fire
		CAnchorDef( IDC_EDIT_CONN_TIMESTOFIRE, k_eSimpleAnchorBottomSide),
		CAnchorDef( IDC_TIMESTOFIRE_LABEL, k_eSimpleAnchorBottomSide ),
#else
...
#ifdef SLE //// SLE NEW - Mark Button for entities in output argument param
		CAnchorDef(IDC_MARK_PARAM, k_eSimpleAnchorBottomSide),
#endif
#ifdef SLE //// SLE CHANGE, anchor like the rest of them, resizing looks wonky otherwise
		CAnchorDef(IDC_SHOWHIDDENTARGETS, k_eSimpleAnchorBottomSide)
#else
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
void COP_Output::UpdateEditedTimesToFire(void)
{
	//
	// Get the info
	CEdit *pTimesToFireEdit = ( CEdit* )GetDlgItem(IDC_EDIT_CONN_TIMESTOFIRE);

	if ( pTimesToFireEdit->IsWindowEnabled() )
	{
		char strTimesToFire[ MAX_IO_NAME_LEN ];
		pTimesToFireEdit->GetWindowText(strTimesToFire, sizeof(strTimesToFire));
		int iTimesToFire = atoi(strTimesToFire);
		...
		for ( int nConn = 0; nConn < nConnCount; nConn++ )
		{
			...
			if ( pConnection != NULL )
			{
				pConnection->SetTimesToFire(iTimesToFire);
			}
		}
		...
		for ( int nItem = 0; nItem < m_ListCtrl.GetItemCount(); nItem++ )
		{
			if ( m_ListCtrl.GetItemState(nItem, LVIS_SELECTED) & LVIS_SELECTED )
			{
				m_ListCtrl.SetItemText(nItem, ONLY_ONCE_COLUMN, strTimesToFire);
			}
		}
		ResizeColumns();
	}
...
void COP_Output::SetConnection(CEntityConnectionList *pConnectionList)
{
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
	pTimesToFireEdit->EnableWindow(true);
#else
	...
	for (int nConn = 0; nConn < nConnCount; nConn++)
	{
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
		// Fill in times to fire, disable for non-compatible connections
		if ( pTimesToFireEdit->IsWindowEnabled() )
		{
			if ( bFirst )
			{
				m_iTimesToFire = pConnection->GetTimesToFire();
			} 
			else if ( m_iTimesToFire != pConnection->GetTimesToFire() )
			{
				m_iTimesToFire = EVENT_FIRE_ALWAYS;
				pTimesToFireEdit->EnableWindow(false);
			}
		}
#else
		...
		if (pFireEdit->IsWindowEnabled())
		{
			...
			{
				m_bFireOnce = false;
			//	pFireEdit->EnableWindow(false); //// SLE CHANGE - allow to change multiple outputs at once to Fire Once
			}
		}
#endif
		bFirst = false;
...
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
void COP_Output::OnEditTimesToFire(void)
{
	UpdateEditedTimesToFire();
}
#else
//-----------------------------------------------------------------------------
...
void COP_Output::OnShowHiddenTargetsAsBroken()
{
	...
	//UpdateConnectionList();
	((CObjectProperties*)GetParent())->UpdateOutputButton();
}

...
int CMapSolid::Split( PLANE *pPlane, CMapSolid **pFront, CMapSolid **pBack )
{
	...
	//	face.SetTexture(GetDefaultTextureName()); //// SLE UNDONE: This would set the split face texture to the default,
...
ChunkFileResult_t CMapSolid::LoadVMF(CChunkFile *pFile, bool &bValid)
{
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		{
#ifdef SLE //// SLE NEW - print out bad solid ids
			CString str;
			str.Format("Solid %i failed to load from the chunk.\nThis is safe to ignore.\nThe solid will be removed on next save.", this->GetID());
...
#ifdef SLE //// SLE NEW - render in wireframe noz if see-thru on selected solids is enabled
		{
			pRender->PushRenderMode(Options.view3d.bSolidsEdgesNoZ ? RENDER_MODE_WIREFRAME_NOZ : RENDER_MODE_WIREFRAME);
		}
#else
		{
...
{
#ifdef SLE //// SLE NEW - Freeze/unfreeze - render in gray if frozen	
	if ( IsFrozen() || (GetParent() && GetParent()->IsFrozen() ))
	{ 
		// todo - add Frozen colour later
		color32 clrFrozen;
		clrFrozen.r = 128;
		clrFrozen.g = 128;
		clrFrozen.b = 128;
	...
	else
	{
		...
		clr.r = GetRValue(Options.colors.clr2DSelection);
		clr.g = GetGValue(Options.colors.clr2DSelection);
		clr.b = GetBValue(Options.colors.clr2DSelection);
#else
...
#ifdef SLE //// SLE NEW - 3d skybox preview. Blow up the cull box so it's always drawn for objects marked as 3d sky
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CMapSolid::GetCullBox(Vector &mins, Vector &maxs)
{
	m_CullBox.GetBounds(mins, maxs);

	////
	if (!Options.general.bShowToolsSkyFaces && ( Is3dSkybox()
		|| ( GetParent() && GetParent()->Is3dSkybox() ) ) )
	{
		mins = -Vector(MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2);
		maxs = Vector(MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2, MAX_COORD_INTEGER * 2);
	}
	////
}
#endif
...
bool CMapSolid::HitTest2D(CMapView2D *pView, const Vector2D &point, HitInfo_t &HitData)
{
	...
	if ( IsFrozen() )
	{
		return false; // frozen objects can't be picked at all
	}
#endif
...
{
#ifdef SLE //// SLE CHANGE - DXF export now works per selection
	if (!IsSelected())
	{
		return true;
	}
...
bool CMapSolid::SaveSMD(ExportSMDInfo_s *pInfo)
{
	if (!IsSelected())
	{
		return true;

...
bool Cordon3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	{
		bool overrideCordon = false; //// SLE NEW: prevent accidental removal of existing cordon

		Vector2D mPoint = vPoint;
		if (m_pDocument->IsCordoning())
		{
			if (AfxMessageBox("There is already an active cordon. Create new?", MB_YESNO) == IDYES)
			{
				overrideCordon = true;
				m_pDocument->SetCordoning(false);
			}
		}
		else
		{
		...
		if (overrideCordon)
		{
			...
			Vector vSize = Vector(1024, 1024, 1024);
			StartNew(pView, mPoint, vecWorld, vSize);
		}
	}
...
bool Cordon3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	...
	{
#ifdef SLE //// SLE NEW: prevent accidental removal of existing cordon
		bool overrideCordon = false;
		...
		{
			// getvisiblepoint fills in any coord that's still set to COORD_NOTINIT:
#ifdef SLE  //// SLE NEW: base cordon size on the last selected thing
			Vector vecStart;
...
#ifdef SLE //// SLE NEW: base cordon size on the last selected thing
			StartNew(pView, vPoint, vecWorld, pView->GetViewAxis() * (bmaxs - bmins));
#else

...
#ifndef SLE //// SLE CHANGE - this has been reworked, see MapFace.cpp for reference
	m_pNoDrawTexture = NULL;
#ifdef SLE //// SLE NEW: Tool Brush Texture display toggle/filter. TODO: Isn't there a better way?
	m_pToolTexture_Areaportal = NULL;
	m_pToolTexture_Blockbullets = NULL;
	m_pToolTexture_BlockLight = NULL;
	m_pToolTexture_BlockLOS = NULL;
	m_pToolTexture_Clip = NULL;
	m_pToolTexture_Fog = NULL;
	m_pToolTexture_Hint = NULL;
	m_pToolTexture_Invisible = NULL;
	m_pToolTexture_InvisibleLadder = NULL;
	m_pToolTexture_NPCClip = NULL;
	m_pToolTexture_Occluder = NULL;
	m_pToolTexture_Origin = NULL;
	m_pToolTexture_PlayerClip = NULL;
	m_pToolTexture_Skip = NULL;
	m_pToolTexture_Trigger = NULL;
	m_pToolTexture_Sky = NULL;
	m_pToolTexture_Sky2D = NULL;
	m_pToolTexture_Imposter = NULL;
#endif
#endif
}
...
void CTextureSystem::LoadMaterials(CGameConfig *pConfig)
{
	...
	for ( int i=0; i < m_Textures.Count(); i++ )
	{
		if (V_strcmp(m_Textures[i]->GetName(), "tools/toolsnodraw") == 0)
		{
...
#ifdef SLE //// SLE NEW: Tool Brush Texture display filter/toggle. TODO: Isn't there a better way?
	m_pToolTexture_Areaportal = NULL;
	m_pToolTexture_Blockbullets = NULL;
	m_pToolTexture_BlockLight = NULL;
	m_pToolTexture_BlockLOS = NULL;
	...
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsareaportal") == 0)
		{
			m_pToolTexture_Areaportal = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsblockbullets") == 0)
		{
			m_pToolTexture_Blockbullets = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsblocklight") == 0)
		{
			m_pToolTexture_BlockLight = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsblock_los") == 0)
		{
			m_pToolTexture_BlockLOS = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsclip") == 0)
		{
			m_pToolTexture_Clip = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsfog") == 0)
		{
			m_pToolTexture_Fog = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolshint") == 0)
		{
			m_pToolTexture_Hint = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsinvisible") == 0)
		{
			m_pToolTexture_Invisible = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsinvisibileladder") == 0)
		{
			m_pToolTexture_InvisibleLadder = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsnpcclip") == 0)
		{
			m_pToolTexture_NPCClip = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsoccluder") == 0)
		{
			m_pToolTexture_Occluder = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsorigin") == 0)
		{
			m_pToolTexture_Origin = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsplayerclip") == 0)
		{
			m_pToolTexture_PlayerClip = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsskip") == 0)
		{
			m_pToolTexture_Skip = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolstrigger") == 0)
		{
			m_pToolTexture_Trigger= m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsskybox") == 0)
		{
			m_pToolTexture_Sky = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "tools/toolsskybox2d") == 0)
		{
			m_pToolTexture_Sky2D = m_Textures[i]; break;
		}
	}
	for (int i = 0; i < m_Textures.Count(); i++)
	{
		if (V_stricmp(m_Textures[i]->GetName(), "dev/dev_windowportal") == 0)
		{
			m_pToolTexture_Imposter = m_Textures[i]; break;
		}
	}
	
	if (!m_pToolTexture_Areaportal) m_pToolTexture_Areaportal = CMaterial::CreateMaterial("tools/toolsareaportal", true);
	if (!m_pToolTexture_Blockbullets) m_pToolTexture_Blockbullets = CMaterial::CreateMaterial("tools/toolsblockbullets", true);
	if (!m_pToolTexture_BlockLOS) m_pToolTexture_BlockLight = CMaterial::CreateMaterial("tools/toolsblocklight", true);
	if (!m_pToolTexture_BlockLOS) m_pToolTexture_BlockLOS = CMaterial::CreateMaterial("tools/toolsblock_los", true);
	if (!m_pToolTexture_Clip) m_pToolTexture_Clip = CMaterial::CreateMaterial("tools/toolsclip", true);
	if (!m_pToolTexture_Fog) m_pToolTexture_Fog = CMaterial::CreateMaterial("tools/toolsfog", true);
	if (!m_pToolTexture_Hint) m_pToolTexture_Hint = CMaterial::CreateMaterial("tools/toolshint", true);
	if (!m_pToolTexture_Invisible) m_pToolTexture_Invisible = CMaterial::CreateMaterial("tools/toolsinvisible", true);
	if (!m_pToolTexture_InvisibleLadder) m_pToolTexture_InvisibleLadder = CMaterial::CreateMaterial("tools/toolsinvisibleladder", true);
	if (!m_pToolTexture_NPCClip) m_pToolTexture_NPCClip = CMaterial::CreateMaterial("tools/toolsnpcclip", true);
	if (!m_pToolTexture_Occluder) m_pToolTexture_Occluder = CMaterial::CreateMaterial("tools/toolsoccluder", true);
	if (!m_pToolTexture_Origin)	m_pToolTexture_Origin = CMaterial::CreateMaterial("tools/toolsorigin", true);
	if (!m_pToolTexture_PlayerClip)	m_pToolTexture_PlayerClip = CMaterial::CreateMaterial("tools/toolsplayerclip", true);
	if (!m_pToolTexture_Skip) m_pToolTexture_Skip = CMaterial::CreateMaterial("tools/toolsskip", true);
	if (!m_pToolTexture_Trigger) m_pToolTexture_Trigger = CMaterial::CreateMaterial("tools/toolstrigger", true);
	if (!m_pToolTexture_Sky) m_pToolTexture_Sky = CMaterial::CreateMaterial("tools/toolsskybox", true);
	if (!m_pToolTexture_Sky2D) m_pToolTexture_Sky2D = CMaterial::CreateMaterial("tools/toolsskybox2d", true);
	if (!m_pToolTexture_Imposter) m_pToolTexture_Imposter = CMaterial::CreateMaterial("dev/dev_windowportal", true);
#endif
#endif
}
...
//-----------------------------------------------------------------------------
// Purpose: Constructor.
// Input  : pszName - Name of group, ie "Materials"
//-----------------------------------------------------------------------------

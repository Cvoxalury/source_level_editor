...
void CAngleBox::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (m_bDragging)
	{
		...
		// Calculate new yaw.
		//
#ifdef SLE //// SLE CHANGED - ported from Hammer-2013 - use rotation angle snap unless holdint ALT
		int nNewYaw = 180 - (int)lineangle(point.x, point.y, m_ptClientCenter.x, m_ptClientCenter.y);

		float angleConstrain = 15.0f; // we could hook it up to angle snap selection, but it seems too granular to be practical

		if ( ( GetAsyncKeyState( VK_MENU ) & 0x8000 ) == 0 )
			nNewYaw = rint( nNewYaw / angleConstrain) * angleConstrain;
		m_vecAngles.Init();
...
void CAngleBox::OnPaint(void)
{
	...
	CBrush brushWindow(SLE_DARK_THEME_CLR_BACK);
#else

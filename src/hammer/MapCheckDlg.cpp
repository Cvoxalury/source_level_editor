...
typedef enum
{
	...
	ErrorEmptyEntity,
	ErrorDuplicateKeys,
#ifndef SLE //// SLE REMOVE - unused Quake 2, might return later if Quake modding becomes a goal
	ErrorSolidContents,
#endif
	...
	ErrorNoTonemapController,
	ErrorDisplacementTiedToEntity,
...
} g_MapErrorStrings[] =
{
	...
	{IDS_BAD_OVERLAY,					IDS_BAD_OVERLAY_DESC },
#ifdef SLE	//// SLE NEW - new 'Map Problems' types 
	{IDS_NOTONEMAPCONT,					IDS_NOTONEMAPCONT_DESC},
	{IDS_DISPASBRUSHENT,				IDS_DISPASBRUSHENT_DESC},
	{IDS_EMPTYSOUNDNAME,				IDS_EMPTYSOUNDNAME_DESC},
	{IDS_EMPTYMODELNAME,				IDS_EMPTYMODELNAME_DESC},
	{IDS_DISPLACEMENTNODRAW,			IDS_DISPLACEMENTNODRAW_DESC},
	{IDS_SHORTFOGFARZ,					IDS_SHORTFOGFARZ_DESC}
#endif
};
...
#ifdef SLE //// SLE NEW - new 'Map Problems' types 
static void FixDisplacementTiedToEntity(MapError *pError);
#endif
...
void CMapCheckDlg::UpdateAnchors(CWnd *pPage)
{
	if ( !GetSafeHwnd() )
		return;
	...
	{
		CAnchorDef(IDC_ERRORS, k_eSimpleAnchorAllSides),
		CAnchorDef(IDC_GO, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FIX, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_FIXALL, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_DESCRIPTION, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_CHECK_VISIBLE_ONLY, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDC_DESCRIPTIONBOX, k_eSimpleAnchorBottomSide),
		CAnchorDef(IDOK, k_eSimpleAnchorBottomSide),
	};
	m_AnchorMgr.Init(GetSafeHwnd(), anchorDefs, ARRAYSIZE(anchorDefs));
}

void CMapCheckDlg::OnSize(UINT nType, int cx, int cy)
{
	m_AnchorMgr.OnSize();
}
#endif
...
void CMapCheckDlg::Fix(MapError *pError, UpdateBox &ub)
{
	...
	//	{
	//		FixDisplacementTiedToEntity(pError);
	//		break;
	//	}
#endif
	}
...
#ifdef SLE
//-----------------------------------------------------------------------------
//// SLE NEW - new 'Map Problems' types 
//-----------------------------------------------------------------------------
static BOOL FindTonemapController(CMapEntity *pObject, DWORD)
{
	if (!IsCheckVisible(pObject))
		return TRUE;

	if (pObject->IsPlaceholder() && pObject->IsClass("env_tonemap_controller"))
	{
		return(FALSE);
	}
	return(TRUE);
...
#ifdef SLE
	// and a tonemap controller 
	if (pWorld->EnumChildren((ENUMMAPCHILDRENPROC)FindTonemapController, 0, MAPCLASS_TYPE(CMapEntity)))
	{
		// if rvl is !0, it was not stopped prematurely.. which means there is 
		// NO player start.
		AddError(pList, ErrorNoTonemapController, 0);
	}
#endif
}
...
#else
	if (!strncmp(pTargetName, "!", 1)) //// SLE CHANGE - allow any names whatsoever that start with !. A single check.
#endif
		return;
...
static BOOL _CheckSolidIntegrity(CMapSolid *pSolid, CListBox *pList)
{
	...
	for (int i = 0; i < nFaces; i++)
	{
		...
		if (!pFace->CheckFace(&cfi))
		{
			...
			pSolid->SetRenderColor(255, 100, 25);
			break;
		}
	}
...
{
	if (CMapDoc::GetActiveMapDoc() && CMapDoc::GetActiveMapDoc()->GetGame())
	{
...
static BOOL _CheckInvalidTextures(CMapSolid *pSolid, CListBox *pList)
{
	...
	for(int i = 0; i < nFaces; i++)
	{
#ifndef HAMMER2013_PORT_MISSING_CHECKER //// SLE REMOVE, doesn't play well with our option for missing materials as error texture
		const CMapFace *pFace = pSolid->GetFace(i);
		IEditorTexture *pTex = pFace->GetTexture();
		char lastTexName[260];

		if (pTex)
		{
			if (pTex->IsDummy() && pFace->texture.texture != lastTexName)
			{
				AddError(pList, ErrorInvalidTexture, (DWORD)pFace->texture.texture, pSolid);
				strncpy(lastTexName, pFace->texture.texture, sizeof(pFace->texture.texture));
				return TRUE;
			}
		}
...
//// SLE NEW - new 'Map Problems' types  
//-----------------------------------------------------------------------------
static BOOL _CheckDisplacementsTiedToEntity(CMapEntity *pObject, CListBox *pList)
{
	const CMapObjectList *pChildren = pObject->GetChildren();

	FOR_EACH_OBJ(*pChildren, pos)
	{
		CMapSolid *pMapSolid = assert_cast<CMapSolid*>(pChildren->Element(pos));
		if (pMapSolid && pMapSolid->HasDisp())
		{
			AddError(pList, ErrorDisplacementTiedToEntity, (DWORD)pObject->GetClassName(), pObject);
			return TRUE;
		}
	}
...
static void CheckDisplacementsTiedToEntity(CListBox *pList, CMapWorld *pWorld)
{
	pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckDisplacementsTiedToEntity, (DWORD)pList, MAPCLASS_TYPE(CMapEntity));
}

static BOOL _CheckDisplacementsNodraw(CMapSolid *pSolid, CListBox *pList)
{
	if ( !IsCheckVisible( pSolid ) )
		return TRUE;
	...
	{
		const CMapFace *pFace = pSolid->GetFace(i);
		if( !pFace->HasDisp()) continue; 
		IEditorTexture *pTex = pFace->GetTexture();
		if (pTex && pTex->HasVariable("%compilenodraw"))
		{
			AddError(pList, ErrorDisplacementNodraw, i, pSolid);
			return TRUE;
		}
	}
...
static void CheckDisplacementsNodraw(CListBox *pList, CMapWorld *pWorld)
{
...
static void FixDisplacementTiedToEntity(MapError *pError)
{
	// todo, copy the OnEditToWorld code from mapdoc or just call it directly
//	CMapEntity *pEntity = (CMapEntity *)pError->pObjects[0];
}
#endif
//-----------------------------------------------------------------------------

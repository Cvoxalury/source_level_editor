...
#define SPRINKLE_PATH			"scripts/editor/"
				...
				for( float x = -flXSize; x <= flXSize; x += flGridXSize )
				{
					for( float y = -flYSize; y <= flYSize; y += flGridYSize )
					{
						...
						if ( CToolEntitySprinkle::FindWorldSpot( vOrigin ) == true )
						{
							...
							{
								Vector vDir;
								m_pHitFace->GetFaceNormal(vDir);
								vEnd = vOrigin + vDir * 64;
							}
...
void CToolEntitySprinkle::RemoveMapObjects( Vector &vOrigin, KeyValues *pSprinkleType, int nMode, int nDensity, CUtlVector< CMapEntity * > *pRemovedEntities, CMapEntity *pTouchedEntity )
{
	...
	FOR_EACH_OBJ( *pEntityList, pos )
	{
		...
		{
			//// SLE NEW - only let it remove objects placed by the tool
		//	if( pEntity->GetKeyValue("populated"))
				bRemove = true;
		}
		else
		{
...
void CToolEntitySprinkle::PopulateEntity( CMapEntity *pEntity, KeyValues *pFields )
{
	if ( pFields != NULL )
	{
		for ( KeyValues *pSub = pFields->GetFirstSubKey() ; pSub != NULL; pSub = pSub->GetNextKey() )
		{
			...
			//	pEntity->SetKeyValue( "populated", "1");
			}
		}
	}
}
...
// Output : 
//-----------------------------------------------------------------------------
void CToolEntitySprinkle::CreateMapObject( Vector &vOrigin, Vector &vSurfaceNormal, KeyValues *pSprinkleType, int nMode, bool bRandomYaw, bool bSurfaceAlign, CMapEntity *pExisting )
{
	int			nTotal = 0;
	...
	float z = 0;
	KeyValues	*pHeight = pUseInfo->FindKey("z_offset");
	if (pHeight == NULL)
	{
		if (pBaseInfo != NULL)
		{
			pHeight = pBaseInfo->FindKey("z_offset");
			z = pHeight->GetFloat();
		}
	}
	...
	if (bSurfaceAlign == true)
	{
		Vector xaxis(1.0f, 0.0f, 0.0f);
		Vector yaxis;
		CrossProduct(vSurfaceNormal, xaxis, yaxis);
		...
		CrossProduct(yaxis, vSurfaceNormal, xaxis);
		VectorNormalize(xaxis);

		VMatrix entToWorld;
		entToWorld.SetBasisVectors(xaxis, yaxis, vSurfaceNormal);
...
{
	Vector vStart, vEnd, vDir;
	Vector vFinalStart, vFinalEnd;
	Vector vHitPos, vHitNormal;

	m_pHitFace->GetFaceNormal(vDir);
	vStart = vOrigin - vDir * 64; // -Vector(0.0f, 0.0f, 600.0f);
	vEnd = vOrigin + vDir * 64;// (+Vector(0.0f, 0.0f, 600.0f);
...
void CToolEntitySprinkle::PerformSprinkle( bool bInitial )
{
	...
	bool						bSurfaceAlign = pSprinkleDlg->UseSurfaceAlign();
			...
			for( int i = 0; i < ReplacedEntities.Count(); i++ )
			{
				...
				else
				{
					...
					if (m_pHitFace->TraceLineInside(vHitPos, vHitNormal, vFinalStart, vFinalEnd))
					{
						...
						CreateMapObject(vOrigin, vFinalHitNormal, pSprinkleType, nMode, bRandomYaw, bSurfaceAlign);
					}
				}
			}
			break;
		...
		default:
			for( float x = -flXSize; x <= flXSize; x += flGridXSize )
			{
				for( float y = -flYSize; y <= flYSize; y += flGridYSize )
				{
					...
					Vector	vOrigin = vCenter + Vector( x, y, 0.0f );
					Vector vFinalHitNormal = Vector(0, 0, 1);
					if( bSurfaceAlign)
					{
						Vector vStart, vEnd;
						Vector vFinalStart, vFinalEnd;

...
//			separating the views. All views in a given MDI child window reflect
//			a single document.
//
//===========================================================================//
...
#ifdef SLE
		case VIEW3D_ENGINE: //// SLE NEW: Attempted to restore "Engine View" functionality. So far unsuccessful.
#endif
...
void CChildFrame::OnView3dBSPLightingPreview(void)
{
	SetViewType(VIEW3D_LIGHTING_PREVIEW);
}
#endif
...
//-----------------------------------------------------------------------------
#ifdef SLE //// SLE NEW - separate commands for switching 3d view modes through F1-4 hotkeys.
//-----------------------------------------------------------------------------
void CChildFrame::Switch3dViewToSpecified(DrawType_t eViewType)
{
	...
	while (viewPos)
	{
		...
		if ( pView )
		{
			if ( pView->IsKindOf(RUNTIME_CLASS(CMapView3D)) )
			{
				...
				pView3D->SetDrawType(eViewType);
				break;
			} 
			else if ( pView->IsKindOf(RUNTIME_CLASS(CMapViewLogical)) ) //// SLE TODO - if we'll allow having a separate logical window, disable this
			{
				pView = ( CMapView3D * )ReplaceView(RUNTIME_CLASS(CMapView3D));
				CMapView3D *pView3D = ( CMapView3D * )pView;
				pView3D->SetDrawType(eViewType);
				break;
			}
...
void CChildFrame::OnSwitch3dToWireframe(void)
{
	Switch3dViewToSpecified(VIEW3D_WIREFRAME);
	/*
	if( GetActiveView() && GetActiveView()->IsKindOf(RUNTIME_CLASS(CMapView3D)))
	{
		SetViewType(VIEW3D_WIREFRAME);
	}
	*/
...
void CChildFrame::OnSwitch3dToFlatPolys(void)
{
	Switch3dViewToSpecified(VIEW3D_POLYGON);
}

void CChildFrame::OnSwitch3dToTextured(void)
{
	Switch3dViewToSpecified(VIEW3D_TEXTURED);
}

void CChildFrame::OnSwitch3dToShaded(void)
{
	Switch3dViewToSpecified(VIEW3D_TEXTURED_SHADED);
}

void CChildFrame::OnSwitch3dToLightmaps(void)
{
	Switch3dViewToSpecified(VIEW3D_LIGHTMAP_GRID);
}

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_3DENGINE command when the active view is a 2D view.
//-----------------------------------------------------------------------------
 //// SLE NEW: Attempted to restore "Engine View" functionality. So far unsuccessful.
...
#ifdef SLE //// SLE CHANGE - default to textured for first-time launch
		eDrawType[0][0] = (DrawType_t)APP()->GetProfileInt("Splitter", "DrawType0,0", VIEW3D_TEXTURED);
#else
...
#ifdef SLE //// SLE NEW: With restored "Logical View", need it here so users don't get stuck with it in the 3d view.
 //// Also want same treatment for lighting preview - they tend to crash editor so we don't want them
 //// re-enabled on next launch.
				if ((eDrawType[nRow][nCol] == VIEW3D_ENGINE)
				|| (eDrawType[nRow][nCol] == VIEW_LOGICAL)
				|| (eDrawType[nRow][nCol] == VIEW3D_LIGHTING_PREVIEW)  //// SLE NEW: BSP Lighting
				|| (eDrawType[nRow][nCol] == VIEW3D_LIGHTING_PREVIEW2)
				|| (eDrawType[nRow][nCol] == VIEW3D_LIGHTING_PREVIEW_RAYTRACED) 
				|| (eDrawType[nRow][nCol] >= VIEW_TYPE_LAST))
#else
				...
				switch (eDrawType[nRow][nCol])
				{
					...
					case VIEW3D_SMOOTHING_GROUP:
#ifdef SLE 
					case VIEW3D_ENGINE: //// SLE NEW: Attempted to re-enable "Engine View", so far no success.
#endif
					{
...
#ifdef SLE
//-----------------------------------------------------------------------------
// Purpose: //// SLE NEW - space for maximising/restoring 3d viewport exclusively
//-----------------------------------------------------------------------------
void CChildFrame::OnMaximize3DView(void)
{
	if (!bUsingSplitter)
	{
		return;
	...
	for ( int nRow = 0; nRow < 2; nRow++ )
	{
		...
		{
			CMapView *pView = dynamic_cast<CMapView*>( m_wndSplitter->GetPane(nRow, nCol) );
			if ( pView != NULL && !pView->IsOrthographic() )
			{
				pView->ActivateView(true);
				m_wndSplitter->ToggleMax(m_wndSplitter->GetPane(nRow, nCol));
			}
		}
	}
}

void CChildFrame::OnUpdateMaximize3DView(CCmdUI *pCmdUI)
{
	pCmdUI->Enable();
}
#endif

...
#ifdef SLE //// SLE NEW - ported from Hammer-2013 - autosave on a separate thread	
	static int GetNextAutosaveNumber( const CString& autoSaveDir, CUtlMap<FILETIME, 
		WIN32_FIND_DATA, int> *pFileMap, DWORD *pdwTotalDirSize, const CString* );
#else
	int GetNextAutosaveNumber( CUtlMap<FILETIME, WIN32_FIND_DATA, int> *pFileMap, DWORD *pdwTotalDirSize, const CString * ) const;
#endif
	void UpdateMappingTip(); //// SLE NEW - show a random mapping tip in the status bar

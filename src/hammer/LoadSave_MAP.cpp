...
int CMapSolid::SerializeMAP(std::fstream& file, BOOL fIsStoring)
{
	...
	if (fIsStoring)
	{
#ifdef SLE //// SLE CHANGE - add a comment signing the solid
		CString str;
		str.Format("// solid # %i", GetID());
		file << str << ENDLINE;
#endif ////
		// save the brush
		file << "{" << ENDLINE;
...
int CMapEntity::SerializeMAP(std::fstream &file, BOOL fIsStoring)
{
	...
	if (fIsStoring)
	{
		...
		// Save it.
		//
#ifdef SLE //// SLE CHANGE - add a comment signing the entity
		CString str;
		str.Format("// entity # %i", GetID());
		file << str << ENDLINE;
#endif ////
		file << "{" << ENDLINE;
...
#ifdef SLE //// SLE CHANGE - return to older .map version supported by compilers
			strcpy(tmpkv.szValue, "220");
#else
...
// Writing is similar to MAP, but with extra keyvalues/subkeys
// todo: relocate it into its own file in the future
// todo: probably should support cordoning same as MAP does?
int CMapClass::SerializeCommentaryTXT(std::fstream& file, BOOL fIsStoring)
{
	// just a bridge to the entity saving 
	return fileOk;
}

int CEditGameClass::SerializeCommentaryTXT(std::fstream& file, BOOL fIsStoring)
{
	int iRvl;
	...
	if( fIsStoring )
	{
		...
		for ( int z=m_KeyValues.GetFirst(); z != m_KeyValues.GetInvalidIndex(); z=m_KeyValues.GetNext( z ) )
		{
			...
			// it's okay to use SerializeMAP here, no difference with txt
			iRvl = KeyValue.SerializeMAP(file, fIsStoring);
			if (iRvl != fileOk)
			{
				return(iRvl);
			...
			for (int i = 0; i < nVariableCount; i++)
			{
				...
				if (pVar != NULL)
				{
					...
					if (p == NULL) 
					{
						...
						//
						// Only write the key value if it is non-zero.
						//// change - disable for commentary nodes and info targets, do as the comm editor does
						//
					//	if ((pKey->szKey[0] != 0) && (pKey->szValue[0] != 0) && (stricmp(pKey->szValue, "0")))
						{
							// it's okay to use SerializeMAP here, no difference with txt
...
int CMapEntity::SerializeCommentaryTXT(std::fstream &file, BOOL fIsStoring)
{
	CMapClass::SerializeCommentaryTXT(file, fIsStoring);
	// load/save an object
	if (fIsStoring)
	{
		//
		// Currently only nodes and info targets are meant to be exported
		//
		//if (GetClassName() != "point_commentary_node" && GetClassName() != "info_target")
		if( !IsSelected())
		{
			return(fileOk);
		}
		...
		//
		// Save it. No "// entity" comment - unnecessary for the commentary file
		//
		file << "\"entity\"" << ENDLINE;
		file << "{" << ENDLINE;
		...
		// Save keyvalues & other data.
		//
		CEditGameClass::SerializeCommentaryTXT(file, fIsStoring);

		//
		// If this is a placeholder and either has no class or is not a solid class,
		// save our origin.
		// This should always happen for nodes and info targets.
		//
		if (IsPlaceholder() && (!IsClass() || !IsSolidClass()))
		{
			MDkeyvalue tmpkv;
...
static int SaveCommentaryTXTEntities(CMapClass *pObject, std::fstream& file)
{
	CMapWorld *pWorld = (CMapWorld *)CMapClass::GetWorldObject(pObject);
	...
	FOR_EACH_OBJ(*pChildren, pos)
	{
		...
		if ( pChild->IsMapClass(MAPCLASS_TYPE(CMapEntity)) )
		{
			if ( !bSaveVisiblesOnly || pChild->IsVisible() )
			{
				iRvl = pChild->SerializeCommentaryTXT(file, TRUE);
			}
		} 
		else if ( pChild->IsMapClass(MAPCLASS_TYPE(CMapGroup)) )
		{
			iRvl = SaveCommentaryTXTEntities(pChild, file);
		}
...
int CMapWorld::SerializeCommentaryTXT(std::fstream &file, BOOL fIsStoring)
{
	...
	if ( fIsStoring )
	{
		file << "\"Entities\"" << ENDLINE;
		file << "{" << ENDLINE;

		//
		// Save the entities.
		//
		if ( SaveCommentaryTXTEntities(this, file) == fileOsError )
		{
			goto FatalError;
		}
				
		file << "}" << ENDLINE; // close the Entities block
	} 
	// todo: support loading txt into the map as well?

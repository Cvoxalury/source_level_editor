...
void CMapClass::GetBoundingBox( Vector &mins, Vector &maxs )
{
	...
	if (!Options.general.bShowToolsSkyFaces && Is3dSkybox())
	{
		mins = -Vector(MAX_COORD_INTEGER, MAX_COORD_INTEGER, MAX_COORD_INTEGER);
		maxs = Vector(MAX_COORD_INTEGER, MAX_COORD_INTEGER, MAX_COORD_INTEGER);
	}
#endif
}
...
BOOL CMapClass::EnumChildrenOfInstance(ENUMMAPCHILDRENPROC pfn, unsigned int dwParam, MAPCLASSTYPE Type, const char* instanceName)
{
	FOR_EACH_OBJ(m_Children, pos)
	{
		...
		if ( pEntity )
		{
			pEntity->SetParentInstanceName(instanceName);
		}
		...
		// enum this child's children
		if ( !pChild->EnumChildrenOfInstance(pfn, dwParam, Type, instanceName) )
		{
			return FALSE;
		}
	}

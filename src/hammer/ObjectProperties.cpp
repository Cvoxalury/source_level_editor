...
void CObjectProperties::SetOutputButtonState(int nState)
{
	...
	{
#ifdef SLE //// SLE FIX - fix entities w/o i/o not updating the icons
		m_pOutputButton->SetIcon(NULL);
		m_pOutputButton->ShowWindow(SW_SHOW);
...
void CObjectProperties::UpdateInputButton()
{
	...
	FOR_EACH_OBJ( m_DstObjects, pos )
	{
		...
		{
			CMapEntity *pEntity = (CMapEntity *)pObject;
			int nStatus = CEntityConnection::ValidateInputConnections(pEntity, true);
			if (nStatus == CONNECTION_BAD)
			{
...
#ifdef SLE //// SLE NEW: separate pointer for directly loading object properties of an item.
//// SLE TODO - this is supposed to help open worldspawn settings without selecting all,
...
void CObjectProperties::LoadData(CMapObjectList *pObjects, bool bSave)
{
	...
	m_DstObjects.AddVectorToTail(*pObjects);

//	const CMapObjectList *addObjects = pObjects;
//	FOR_EACH_OBJ(*addObjects, pos)
//	{
//		m_DstObjects.AddToTail(addObjects->Element(pos));
//	}
	...
	{
#ifdef SLE //// SLE CHANGE - show number of objects in dialogue title
		char szTitle[MAX_PATH];
		sprintf(szTitle, "Object Properties: %i objects", m_DstObjects.Count());
		SetWindowText(szTitle);
#else
...
void CObjectProperties::ReloadData()
{
	...
	if (m_DstObjects.Count() == 1)
	{
		...
		if ( pEntity )
		{
			if ( strcmpi( pEntity->GetClassNameClass(), "func_instance" ) == 0 )
			{
			...
			else if ( strcmpi( pEntity->GetClassNameClass(), "func_instance_parms" ) == 0 )
			{
				if ( pDoc )
				{

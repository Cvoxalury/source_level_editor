...
bool Morph3D::CanDeselectList( void )
{
	FOR_EACH_OBJ( m_StrucSolids, pos )
	{
		...
		if ( pSSolid )
		{
			...
			{
				//// SLE NEW: Instead of posing the ultimatum (destroy displacements or you can't deselect the brush),
				// warn the user about this invalid solid, but let them evaluate it and fix or delete it themselves.
				AfxMessageBox("Warning: this solid will not be valid for displacements. Evaluate it and either fix it or delete its displacement faces.");
			}
		}
	}
...
bool Morph3D::MorphHitTest(CMapView *pView, const Vector2D &vPoint, MORPHHANDLE *pInfo)
{
	...
	float skyScale = 16;
	bool isSkybox = false;
	//// move the verts according to skybox scale so they match the solid
	if ( !Options.general.bShowToolsSkyFaces )
	{
		...
		{
			CMapClass *pSolid = List.Element(i);
			if ( pSolid && ( pSolid->Is3dSkybox() || pSolid->GetParent() && pSolid->GetParent()->Is3dSkybox() ) )
				isSkybox = true;
		}
	...
	FOR_EACH_OBJ( m_StrucSolids, pos )
	{
		...
		if ( m_HandleMode & hmVertex )
		{
			for ( int i = 0; i < pStrucSolid->m_nVertices; i++ )
			{
				...
				if ( isSkybox )
				{
					saveV[ 0 ] = (saveV[ 0 ] + skyDelta[ 0 ]) * skyScale;
					saveV[ 1 ] = (saveV[ 1 ] + skyDelta[ 1 ]) * skyScale;
					saveV[ 2 ] = (saveV[ 2 ] + skyDelta[ 2 ]) * skyScale;
				}
				if ( HitRect(pView, vPoint, saveV, HANDLE_RADIUS) )
#else
...
#ifdef SLE //// SLE NEW - 3d skybox preview
				Vector saveE = e.ptCenter;
				if ( isSkybox )
				{
					saveE[ 0 ] = ( saveE[ 0 ] + skyDelta[ 0 ] ) * skyScale;
					saveE[ 1 ] = ( saveE[ 1 ] + skyDelta[ 1 ] ) * skyScale;
					saveE[ 2 ] = ( saveE[ 2 ] + skyDelta[ 2 ] ) * skyScale;
				}
				if ( HitRect(pView, vPoint, saveE, HANDLE_RADIUS) )
#else
...
SSHANDLE Morph3D::Get2DMatches( CMapView2D *pView, CSSolid *pStrucSolid, SSHANDLEINFO &hi, CUtlVector<SSHANDLE>*pSimilarList)
{
	...
	if(hi.Type == shtVertex)
	{
		for(int i = 0; i < pStrucSolid->m_nVertices; i++)
		{
			...
			// YWB Fixme, scale tolerance to zoom amount?
...
void Morph3D::StartTranslation( CMapView *pView, const Vector2D &vPoint, MORPHHANDLE *pInfo )
{
	if(m_bScaling)
	{
		...
		if ( m_ScaleDlg.m_iMode == 1 )
		{
			m_ScaleDlg.m_cTransformX.SetWindowText("1.0");
			m_ScaleDlg.m_cTransformY.SetWindowText("1.0");
			m_ScaleDlg.m_cTransformZ.SetWindowText("1.0");
		}
		else
		{
			m_ScaleDlg.m_cTransformX.SetWindowText("0.0");
			m_ScaleDlg.m_cTransformY.SetWindowText("0.0");
			m_ScaleDlg.m_cTransformZ.SetWindowText("0.0");
		}
#else
...
#ifdef SLE //// SLE NEW - allow box-selecting edges
		if ( m_HandleMode == hmEdge )
		{
			for ( int i = 0; i < pStrucSolid->m_nEdges; i++ )
			{
				...
				for ( i2 = 0; i2 < 3; i2++ )
				{
					if ( e.ptCenter[ i2 ] < bmins[ i2 ] || e.ptCenter[ i2 ] > bmaxs[ i2 ] )
						break;
				}
...
void Morph3D::OnScaleCmd(BOOL bReInit)
{
	...
	if(!bReInit)
	{
		...
		m_bUpdateOrg = true;
#ifdef SLE
		m_ScaleDlg.m_idummy = 1; // set to Scale by default
#endif
...
void Morph3D::UpdateScale()
{
...
#ifdef SLE //// SLE NEW - better vertex tool scaling
	float fTransformX = m_ScaleDlg.m_fTransformX;
	float fTransformY = m_ScaleDlg.m_fTransformY;
	float fTransformZ = m_ScaleDlg.m_fTransformZ;
	...
	for ( int i = 0; i < m_SelectedHandles.Count(); i++ )
	{
		...
		float deltaX = pOrigPos[ 0 ] - m_ScaleOrg[ 0 ];
		float deltaY = pOrigPos[ 1 ] - m_ScaleOrg[ 1 ];
		float deltaZ = pOrigPos[ 2 ] - m_ScaleOrg[ 2 ];

		if ( m_ScaleDlg.m_iMode == 0 ) // rotate
		{
			VMatrix matrix;
			QAngle hammerAngle(-fTransformY, fTransformZ, fTransformX);
			matrix.SetupMatrixOrgAngles(vec3_origin, hammerAngle);
			Vector vOffset;
			matrix.V3Mul(m_ScaleOrg, vOffset);
			vOffset = m_ScaleOrg - vOffset;
			matrix.SetTranslation(vOffset);
			// use this vector as a stop-gap if user wants to cancel/zero the rotation
			Vector rotated = pOrigPos;
			TransformPoint(matrix, rotated);

			newpos = rotated;
		}

		else if ( m_ScaleDlg.m_iMode == 1 ) // scale
		{
			newpos[ 0 ] = m_ScaleOrg[ 0 ] + ( deltaX * fTransformX );
			newpos[ 1 ] = m_ScaleOrg[ 1 ] + ( deltaY * fTransformY );
			newpos[ 2 ] = m_ScaleOrg[ 2 ] + ( deltaZ * fTransformZ );
		}

		else if ( m_ScaleDlg.m_iMode == 2 ) // move
		{
			newpos[ 0 ] = m_ScaleOrg[ 0 ] + ( deltaX + fTransformX );
			newpos[ 1 ] = m_ScaleOrg[ 1 ] + ( deltaY + fTransformY );
			newpos[ 2 ] = m_ScaleOrg[ 2 ] + ( deltaZ + fTransformZ );
		...
		for (int nFace = 0; nFace < nFaceCount; nFace++)
		{
			...
			for (int nEdge = 0; nEdge < nEdgeCount; nEdge++)
			{
				...
				if ((nPass == 1) || (nPass == 2))
				{
					...
					if ( isSkybox )
					{
						Vertex[ 0 ] = ( Vertex[ 0 ] + skyDelta[ 0 ] ) * skyScale;
						Vertex[ 1 ] = ( Vertex[ 1 ] + skyDelta[ 1 ] ) * skyScale;
						Vertex[ 2 ] = ( Vertex[ 2 ] + skyDelta[ 2 ] ) * skyScale;
					}
#endif
				...
				else
				{
					if (pSolid->ShowEdges())
					{
						...
						if ( isSkybox )
						{
							CenterPoint[ 0 ] = ( CenterPoint[ 0 ] + skyDelta[ 0 ] ) * skyScale;
							CenterPoint[ 1 ] = ( CenterPoint[ 1 ] + skyDelta[ 1 ] ) * skyScale;
							CenterPoint[ 2 ] = ( CenterPoint[ 2 ] + skyDelta[ 2 ] ) * skyScale;
						}
#endif
					...
					if (pSolid->ShowVertices())
					{
						...
						if ( isSkybox )
						{
							vPoint[ 0 ] = ( vPoint[ 0 ] + skyDelta[ 0 ] ) * skyScale;
							vPoint[ 1 ] = ( vPoint[ 1 ] + skyDelta[ 1 ] ) * skyScale;
							vPoint[ 2 ] = ( vPoint[ 2 ] + skyDelta[ 2 ] ) * skyScale;
						}
#endif
...
#ifdef SLE //// SLE NEW - delete/collapse edge
		case VK_DELETE:
		{
			DeleteEdge();
			break;
...
bool Morph3D::SelectAt( CMapView *pView, UINT nFlags, const Vector2D &vPoint )
{
	...
	int nHits = pView->ObjectsAt( vPoint, Objects, sizeof(Objects) / sizeof(Objects[0]), FLAG_OBJECTS_AT_ONLY_SOLIDS); //// SLE CHANGE - makes sense to use the FLAG_OBJECTS_AT_ONLY_SOLIDS flag, vertex edit only works with solids
...
void Morph3D::SnapSelectedToGrid(float flGridSpacing)
#else
...
#endif
{
	...
	FOR_EACH_VEC( vecHandles, i )
	{
		...
		{
			vSnappedPos = Vector((vCurPos[0] / flGridSpacing) * flGridSpacing,
				(vCurPos[1] / flGridSpacing) * flGridSpacing,
				(vCurPos[2] / flGridSpacing) * flGridSpacing);
		}
		else
		{
			vSnappedPos = Vector(	V_rint(vCurPos[0] / flGridSpacing) * flGridSpacing,
									V_rint(vCurPos[1] / flGridSpacing) * flGridSpacing,
									V_rint(vCurPos[2] / flGridSpacing) * flGridSpacing);
		}		
#else
...
bool Morph3D::OnRMouseDown2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	return true; //// SLE NEW - do nothing on RMB in vertex morph mode. This used to always break morph by throwing vertices to infinitiy
}

bool Morph3D::OnRMouseUp2D(CMapView2D *pView, UINT nFlags, const Vector2D &vPoint)
{
	return true;
}
#ifdef SLE //// SLE NEW - delete/collapse edge
void Morph3D::DeleteEdge()
{
	if ( !IsEmpty() && ( GetSelectedHandleCount() > 0 ) && GetSelectedType() == shtEdge )
	{
		if ( AfxMessageBox("Collapse the selected edge(s) and merge their end vertices?", MB_YESNO) != IDYES )
			return;

		for ( int i = 0; i < m_SelectedHandles.Count(); i++ )
		{
			CSSolid *pSolid = m_SelectedHandles[ i ].pStrucSolid;
			for ( int j = 0; j < pSolid->m_nEdges; j++ )
			{
				CSSEdge& e = pSolid->m_Edges[ j ];

				if ( e.m_bSelected )
				{
					Vector edgeCenter;
					pSolid->m_Edges[ j ].GetCenterPoint(edgeCenter);

					// todo: this is cumbersome, is there no better way to get the verts from an edge?
					SSHANDLEINFO hi1;
					SSHANDLEINFO hi2;
					pSolid->GetHandleInfo(&hi1, pSolid->m_Edges[ j ].hvStart);
					pSolid->GetHandleInfo(&hi2, pSolid->m_Edges[ j ].hvEnd);

					pSolid->SetVertexPosition(hi1.iIndex, edgeCenter);
					pSolid->SetVertexPosition(hi2.iIndex, edgeCenter);

					int nDeleted;
					SSHANDLE *pDeleted = pSolid->MergeSameVertices(nDeleted);
					// ensure deleted handles are not marked
					for ( int i = 0; i < nDeleted; i++ )
					{
						MORPHHANDLE mh;
						mh.ssh = pDeleted[ i ];
						mh.pStrucSolid = pSolid;
						mh.pMapSolid = pSolid->m_pMapSolid;
						SelectHandle(&mh, scUnselect);
					}
				}
			}

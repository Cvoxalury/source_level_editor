...
#ifdef SLE
//	m_pPicker = new CMDLPicker(NULL, CMDLPicker::PAGE_RENDER | CMDLPicker::PAGE_SKINS | CMDLPicker::PAGE_INFO | CMDLPicker::PAGE_ATTACHMENTS); // Hammer-2013 port
#endif
...
#ifdef SLE 
	m_VGuiWindow.Create( NULL, _T("ModelViewer"), WS_VISIBLE|WS_CHILD, CRect(0,0,800,600), this, 1001);
#else
...
//	pMainPanel->InvalidateLayout(true);
	...
	//// SLE NEW: Enable fog in Model Browser.
	//// 0 density because we don't really want
	//// to see the model fogged. Otherwise, main 3d render's
	//// fog makes it fogged.
	pRenderContext->FogColor3ub(90, 90, 90);
	pRenderContext->FogStart(10000);
	pRenderContext->FogEnd(11000);
	pRenderContext->FogMaxDensity(0.0);
	pRenderContext->FogMode(MATERIAL_FOG_LINEAR);
	pRenderContext->ClearColor3ub(90, 90, 90);
...
void CModelBrowser::OnDestroy()
{
	...
	// model browser destroys our default cubemap, reload it

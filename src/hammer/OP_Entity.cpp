...
#ifdef SLE
#include "EntityReportDlg.h" //// for the used models stat
#include "keyvalues.h" //// for the actbusy browser
#include "dlglistmanage.h" //// backported from 2015, for VScript
#endif
...
#ifdef SLE //// SLE NEW - backported from 2015, for VScript
...
void CPickAnglesTarget::OnNotifyPickAngles(const Vector &vecPos)
{
	...
	FOR_EACH_OBJ( *m_pDlg->m_pObjectList, pos )
	{
		...
		if (pEntity != NULL)
		{
			...
			if (pEntity->GetClassNameClass() && (!strnicmp(pEntity->GetClassNameClass(), "light_", 6)))
			{
				angFace[PITCH] *= -1;
			}
	...
	m_InstanceParmData( CStringLessFunc )
{
	...
	pModelBrowser = NULL;
#ifdef SLE //// SLE NEW - particle browser
...
void COP_Entity::UpdateData( int Mode, PVOID pData, bool bCanEdit )
{
	...
	if (Mode == LoadFirstData)
	{
		...
		V_strcpy_safe( szBuf, pEdit->GetClassNameClass() );
		...
		UpdateEditClass(pEdit->GetClassNameClass(), true);
		UpdateDisplayClass(pEdit->GetClassNameClass());
	...
	else if (Mode == LoadData)
	{
		...
		if (strcmpi(str, pEdit->GetClassNameClass()))
		{
			//
		...
		{
			LoadClassList();
			m_cClasses.SelectItem( pEdit->GetClassNameClass() );
		}
...
//	SetupQuickAccess(); // been reworked as a drop-down list
#endif
	return TRUE;
}
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
/*
void COP_Entity::SetupQuickAccess() // been reworked as a drop-down list
{
	CWnd *pButton1 = GetDlgItem(IDC_ENTITY_PRESET1);
	CWnd *pButton2 = GetDlgItem(IDC_ENTITY_PRESET2);
	CWnd *pButton3 = GetDlgItem(IDC_ENTITY_PRESET3);
	CWnd *pButton4 = GetDlgItem(IDC_ENTITY_PRESET4);

//	COP_Entity *pSheet = (COP_Entity*)GetParent();

	pButton1->SetWindowText(g_pGameConfig->szQuickAccessClass1);
	pButton2->SetWindowText(g_pGameConfig->szQuickAccessClass2);
	pButton3->SetWindowText(g_pGameConfig->szQuickAccessClass3);
	pButton4->SetWindowText(g_pGameConfig->szQuickAccessClass4);
}
*/
#endif
void COP_Entity::UpdateAnchors()
{
	CAnchorDef anchorDefs[] =
	{
...
#ifndef SLE
		CAnchorDef( IDC_SMARTEDIT, k_eAnchorRight, k_eAnchorTop, k_eAnchorRight, k_eAnchorBottom), //// 
		CAnchorDef( IDC_ENTITY_HELP, k_eAnchorRight, k_eAnchorTop, k_eAnchorRight, k_eAnchorBottom),
#endif
#ifdef SLE //// SLE CHANGE, anchor like the rest of them, resizing looks wonky otherwise
		...
		CAnchorDef(IDC_ENTITY_PRESETS_CAPTION, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_ENTITY_PRESET_LIST, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_ENTITY_PRESET1, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_ENTITY_PRESET2, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_ENTITY_PRESET3, k_eSimpleAnchorRightSide),
		CAnchorDef(IDC_ENTITY_PRESET4, k_eSimpleAnchorRightSide),
#else
		CAnchorDef( IDC_ANGLES_LABEL, k_eSimpleAnchorRightSide ),
		CAnchorDef( IDC_ANGLEEDIT, k_eSimpleAnchorRightSide ),
		CAnchorDef( IDC_ANGLEBOX, k_eSimpleAnchorRightSide ),
...
#ifdef SLE //// SLE NEW - more data types
			|| ( eType == ivParticleSystem ) //// SLE NEW - particle browser
			|| ( eType == ivCableMat )
			|| ( eType == ivSkyMat )
			|| ( eType == ivSoundscape ) //// SLE NEW - soundscape browser
#endif
			)
		{
...
#ifdef SLE //// SLE NEW - more data types
		else if ((eType == ivScript) || (eType == ivScriptList))
		{
...
#ifdef SLE //// SLE CHANGE: part of new buttons setup
		ButtonRect.left = rectAngleBox.left;
#else
...
#ifdef SLE //// SLE CHANGE: Part of new buttons setup
	if (m_pEditClass && m_pEditClass->IsPointClass())
	{
		float newButtonPos = ButtonRect.left - 96;
		CButton *pPointAtButton = new CButton;
		pPointAtButton->CreateEx(0, "Button", "Point At...", WS_TABSTOP | WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX | BS_PUSHLIKE,
			newButtonPos + 80, ButtonRect.top, 72, ButtonRect.Height() + 2, GetSafeHwnd(), (HMENU)IDC_PICK_ANGLES);
		pPointAtButton->SendMessage(WM_SETFONT, (WPARAM)hControlFont);

		//// SLE NEW: new angle-related buttons.
		//// Random Yaw does what it says - it's equivalent of "Random Yaw" checkbox in the entity creation interface,
		//// but that only applies to objects being placed. This button lets user randomise yaw after placing the entity.
		//// Fixup Angles button rounds the angles. The issue was: when rotating an entity on all three axes, the transform function
		//// almost always creates angles that are very fractionalised. E. g. 0.192623 -351.002881 5.000001. 
		//// This created unnecessarily long angle values. The function that is tied to this button, takes the
		//// angles and re-pastes them rounded to one character after the comma. So, 0.19263 becomes 0.2. 
		CButton *pRYawButton = new CButton;
		pRYawButton->CreateEx(0, "Button", "Rnd Yaw", WS_TABSTOP | WS_CHILD | WS_VISIBLE | BS_AUTOCHECKBOX | BS_PUSHLIKE,
			newButtonPos, ButtonRect.top, 72, ButtonRect.Height() + 2, GetSafeHwnd(), (HMENU)IDC_ANGLE_RANDOM_YAW);
		pRYawButton->SendMessage(WM_SETFONT, (WPARAM)hControlFont);

		CWinApp *pApp = AfxGetApp();
		HICON hIcon = pApp->LoadIcon(IDI_CROSSHAIR);
		pPointAtButton->SetIcon(hIcon);
		pRYawButton->SetIcon(hIcon);

		m_SmartControls.AddToTail(pPointAtButton);
		m_SmartControls.AddToTail(pRYawButton);
	}
#else
...
#ifdef SLE //// SLE NEW - more data types
	else if ( pVar->GetType() == ivActbusy ) //// SLE NEW - actbusy browser
	{
		KeyValues *pKVAnimData = new KeyValues("ActBusyAnimDatafile");
		if ( pKVAnimData->LoadFromFile(g_pFileSystem, "scripts/actbusy.txt", "MOD") )
		{
			// Now try and parse out each act busy anim
			KeyValues *pKVAnim = pKVAnimData->GetFirstSubKey();
			while ( pKVAnim )
			{
				const char *pString = pKVAnim->GetName();
				if ( pString )
				{
					pCombo->AddString(pString);
				}
...
void COP_Entity::CreateSmartControls_BrowseAndPlayButtons(GDinputvariable *pVar, CRect &ctrlrect, HFONT hControlFont)
{
	...
	if ( pVar->GetType() == ivSound || pVar->GetType() == ivScene )
	{
		...
		if ( pVar->GetType() == ivSound )
		{
			...
			CButton *pStopButton = new CButton;
			pStopButton->CreateEx(0, "Button", "Stop", WS_TABSTOP | WS_CHILD | WS_VISIBLE,
				ButtonRect.left, ButtonRect.top, ButtonRect.Width(), ButtonRect.Height(),
				GetSafeHwnd(), ( HMENU )IDC_STOPSOUND);
			pStopButton->SendMessage(WM_SETFONT, ( WPARAM )hControlFont);

			m_SmartControls.AddToTail(pStopButton);
		}
#endif
	}
}
...
void COP_Entity::CreateSmartControls_BrowseScripts(GDinputvariable *pVar, CRect &ctrlrect, HFONT hControlFont)
{
	if ( m_eEditType == ivScript ) //// at the moment, only singular scripts are supported
	{
		...
		m_SmartControls.AddToTail(pButton); // so that the browse button gets removed when not needed
	}
#if 0 //// SLE TODO - until the dialogue is replaced with a new, less trash version, this is disabled
...
#ifdef SLE //// SLE CHANGE - also create Mark for filter destinations
		|| (eType == ivFilterClass)
#endif
		)
	{
...
bool COP_Entity::BrowseModels( char *szModelName, int length, int &nSkin )
{
	...
	{
#ifdef SLE	//// SLE NEW: Rather than hiding and showing the browser
			//// window, destroy and recreate it. This prevents the window
			//// from occasionally freezing. The downside is it currently	
			//// 'forgets' the selected folder, or rather doesn't apply the
			//// folder filter, but it's a much lesser inconvenience right now.
		pModelBrowser->DestroyWindow(); 
		pModelBrowser = new CModelBrowser(GetMainWnd());
#else
		pModelBrowser->Show();
#endif
...
#ifdef SLE //// 2015 model browser's Used Models functionality
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
...
void COP_Entity::BrowseSkybox(void)
{
	int iSel = GetCurVarListSelection();
...
//	pBrowser->SetInitialTexture(szInitialTexture);
	...
	if ( pBrowser->DoModal() == IDOK )
	{
		...
		// strip "skybox" and the side suffix	
		szName[ strlen(szName) - 2 ] = '\0';

		CString skyName = szName;
		skyName = StripDirPrefix(szName, "skybox");

		m_pSmartControl->SetWindowText(skyName);

		// also set variable
		m_kv.SetValue(pVar->GetName(), skyName);
...
void COP_Entity::OnBrowse(void)
{
...
#ifdef SLE //// SLE NEW - more data types
	if ( m_eEditType == ivCableMat )
	{
		BrowseTextures("cable/", false);
		return;
	}

	if ( m_eEditType == ivSkyMat )
	{
		BrowseSkybox();
		return;
	}
	...
	switch (m_eEditType)
	{
		...
		case ivSoundscape:
		{
			...
			CSoundBrowser soundDlg(currentValue);
			soundDlg.m_SoundType = SOUND_TYPE_SOUNDSCAPE;
			int nRet = soundDlg.DoModal();
			if ( nRet == IDOK )
			{
				m_pSmartControl->SetWindowText(soundDlg.GetSelectedSound());
...
#ifdef SLE
			if ( m_eEditType == ivScript )
			{
				CStringListTokenizer::TrimPrefixes(szTemp, "scripts/vscripts/");
			}
#endif
...
#ifdef SLE //// SLE NEW - more data types, backported from 2015
bool COP_Entity::BrowseScriptList( CStringList &lstBrowse )
{	
#if 0 //// SLE TODO: replace it with a not trash new dialogue
...
#ifdef SLE //// SLE NEW - online help button
...
bool COP_Entity::CustomDrawItemValue( const LPDRAWITEMSTRUCT p, const RECT *pRect )
{
	...
	if ( pVar && (pVar->GetType() == ivColor255 || pVar->GetType() == ivColor1) )
	{
		...
		if ( pValue )
		{
			...
			RECT rc = *pRect;
#ifdef SLE //// SLE CHANGE - smaller padding
			Rectangle(p->hDC, rc.left + 1, rc.top + 1, rc.right, rc.bottom - 1);
#else
			Rectangle( p->hDC, rc.left+6, rc.top+2, rc.right-6, rc.bottom-2 );
#endif

#ifdef SLE //// SLE NEW - type out colour value on top of colour brush
			CDC* pDC = CDC::FromHandle(p->hDC);
...
#ifdef SLE //// SLE NEW: New buttons as explained in commentary above, in CreateSmartControls_Angle(...)
//-----------------------------------------------------------------------------
// Purpose: New buttons as explained in commentary above, in CreateSmartControls_Angle(...)
//-----------------------------------------------------------------------------
void COP_Entity::OnSetRandomYaw(void)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	if (pDoc == NULL)
	...
	if ((m_pObjectList != NULL) && (pWorld != NULL))
	{
		...
		FOR_EACH_OBJ(*m_pObjectList, pos)
		{
			...
			if (pEntity != NULL)
			{
				...
				QAngle angles;
				QAngle rndAngle = RandomAngle(-180, 180);
				pEntity->GetAngles(angles);
				// Preserve pitch and rall, randomise the yaw.
				char szAngles[80];
				sprintf(szAngles, "%.1f %.1f %.1f", angles[PITCH], rndAngle[YAW], angles[ROLL]);
				pEntity->SetKeyValue("angles", szAngles);
...
// Purpose: 
//-----------------------------------------------------------------------------
void COP_Entity::OnFixupAngles(void)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	if (pDoc == NULL)
	...
	if ((m_pObjectList != NULL) && (pWorld != NULL))
	{
		...
		FOR_EACH_OBJ(*m_pObjectList, pos)
		{
			...
			if (pEntity != NULL)
			{
				...
				QAngle angles;
				pEntity->GetAngles(angles);
				// Round the angles to 1 digit after the comma
				char szAngles[80];
				sprintf(szAngles, "%.1f %.1f %.1f", angles[PITCH], angles[YAW], angles[ROLL]);
				pEntity->SetKeyValue("angles", szAngles);
...
#ifdef SLE //// SLE NEW - quick access class buttons for setting current entity's class
//-----------------------------------------------------------------------------
//// SLE TODO: make it check that Quick Access class is point or solid versus current entity being 
// point or solid. I. e. prevent changing props into triggers or func's into env's, etc.
void COP_Entity::OnEntityPresetList()
{
	CMenu menu;
	menu.CreatePopupMenu();

	menu.AppendMenu(MF_STRING, COP_Entity::id_QuickAccess1, g_pGameConfig->szQuickAccessClass1);
	menu.AppendMenu(MF_STRING, COP_Entity::id_QuickAccess2, g_pGameConfig->szQuickAccessClass2);
	menu.AppendMenu(MF_STRING, COP_Entity::id_QuickAccess3, g_pGameConfig->szQuickAccessClass3);
	menu.AppendMenu(MF_STRING, COP_Entity::id_QuickAccess4, g_pGameConfig->szQuickAccessClass4);

	// track menu
	CWnd *pButton = GetDlgItem(IDC_ENTITY_PRESET_LIST);
	CRect r;
	pButton->GetWindowRect(r);
	menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON, r.left, r.bottom, this, NULL);
}

BOOL COP_Entity::InsertEntityPreset(UINT id)
{
	CWnd *pButton = GetDlgItem(IDC_ENTITY_PRESET_LIST);

	UINT uCmd = IDC_ENTITY_PRESET1;

	switch ( id )
	{
		case id_QuickAccess1:
			uCmd = IDC_ENTITY_PRESET1;
			break;
		case id_QuickAccess2:
			uCmd = IDC_ENTITY_PRESET2;
			break;
		case id_QuickAccess3:
			uCmd = IDC_ENTITY_PRESET3;
			break;
		case id_QuickAccess4:
			uCmd = IDC_ENTITY_PRESET4;
			break;
		default:
			break;
	}

	return OnEntityPreset(uCmd);

	return TRUE;
}

BOOL COP_Entity::OnEntityPreset(UINT cmd)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	...
	if ( m_pObjectList == NULL ) return false;

//	AfxMessageBox("works", MB_OK);
	...
	switch ( cmd )
	{
		case IDC_ENTITY_PRESET1:
			str.Format(g_pGameConfig->szQuickAccessClass1);
			break;
		case IDC_ENTITY_PRESET2:
			str.Format(g_pGameConfig->szQuickAccessClass2);
			break;
		case IDC_ENTITY_PRESET3:
			str.Format(g_pGameConfig->szQuickAccessClass3);
			break;
		case IDC_ENTITY_PRESET4:
			str.Format(g_pGameConfig->szQuickAccessClass4);
			break;
		default:
			break;
	}
	...
	FOR_EACH_OBJ(*m_pObjectList, pos)
	{
		...
		if ( pEntity != NULL )
		{
			...
			m_cClasses.SelectItem(str);
			UpdateDisplayClass(str);
			UpdateEditClass(str, true);

			OnSelchangeKeyvalues();
		}
	}

//	CMapDoc::GetActiveMapDoc()->SetModifiedFlag(); // this instantly reverts the change to the classname

	return( TRUE );
}

/*
void COP_Entity::OnQuickAccess1(void)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	if (pDoc == NULL)
	...
	if ((m_pObjectList != NULL) && (pWorld != NULL))
	{
		...
		FOR_EACH_OBJ(*m_pObjectList, pos)
		{
			...
			if (pEntity != NULL)
			{
				...
				m_cClasses.SelectItem(g_pGameConfig->szQuickAccessClass1);
				UpdateDisplayClass(g_pGameConfig->szQuickAccessClass1);
				UpdateEditClass(g_pGameConfig->szQuickAccessClass1, true);
...
void COP_Entity::OnQuickAccess2(void)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	if (pDoc == NULL)
	...
	if ((m_pObjectList != NULL) && (pWorld != NULL))
	{
		...
		FOR_EACH_OBJ(*m_pObjectList, pos)
		{
			...
			if (pEntity != NULL)
			{
				...
				m_cClasses.SelectItem(g_pGameConfig->szQuickAccessClass2);
				UpdateDisplayClass(g_pGameConfig->szQuickAccessClass2);
				UpdateEditClass(g_pGameConfig->szQuickAccessClass2, true);
...
void COP_Entity::OnQuickAccess3(void)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	if (pDoc == NULL)
	...
	if ((m_pObjectList != NULL) && (pWorld != NULL))
	{
		...
		FOR_EACH_OBJ(*m_pObjectList, pos)
		{
			...
			if (pEntity != NULL)
			{
				...
				m_cClasses.SelectItem(g_pGameConfig->szQuickAccessClass3);
				UpdateDisplayClass(g_pGameConfig->szQuickAccessClass3);
				UpdateEditClass(g_pGameConfig->szQuickAccessClass3, true);
...
void COP_Entity::OnQuickAccess4(void)
{
	CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
	Assert(pDoc != NULL);
	if (pDoc == NULL)
	...
	if ((m_pObjectList != NULL) && (pWorld != NULL))
	{
		...
		FOR_EACH_OBJ(*m_pObjectList, pos)
		{
			...
			if (pEntity != NULL)
			{
				...
				m_cClasses.SelectItem(g_pGameConfig->szQuickAccessClass4);
				UpdateDisplayClass(g_pGameConfig->szQuickAccessClass4);
				UpdateEditClass(g_pGameConfig->szQuickAccessClass4, true);

...
static float Snap(/*int*/ float iValue, float flGridSize)
{
	if (flGridSize >= 1.0f)
	{
		return (V_rint(iValue / flGridSize) * flGridSize);
	}
	else
	{
		return (iValue / flGridSize) * flGridSize;
	}
}
#else
...
#endif
{
	...
	{
#ifdef SLE //// SLE NEW - allow grid as float, for sub-1 grid sizes
		bmins[i] = (float)Snap(bmins[i], flGridSize);
#else

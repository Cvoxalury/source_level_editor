...
#ifdef SLE //// SLE NEW - count decal textures as used textures in addition to face/overlay materials
static BOOL AddDecalTextures(CMapDecal *pDecal, CUsedTextureList *pList)
{
	if ( !pDecal->IsVisible() )
		return TRUE;

	UsedTexture_t Tex;
	Tex.pTex = pDecal->GetMaterial();
	Tex.nUsageCount = 0;
...
void CMapWorld::CullTree_FreeNode(CCullTreeNode *pNode)
{
	...
	if (nChildCount != 0)
	{
		for (int nChild = 0; nChild < nChildCount; nChild++)
		{
			...
			if( pChild && pChild != pNode ) CullTree_FreeNode(pChild);
		}
	}
...
void CMapWorld::PostloadVisGroups()
{
	...
	{
#ifdef SLE //// SLE CHANGE - remove orphans from other groups and don't spam the message
		pDoc->VisGroups_CreateNamedVisGroup( orphans, "_orphaned hidden", true, true );
#else
...
CMapEntity *CMapWorld::FindEntityByName( const char *pszName, bool bVisiblesOnly, bool bSearchInstanceParms )
{
	...
	if ( bSearchInstanceParms == true )
	{
		...
		FOR_EACH_OBJ( *pEntities, pos )
		{
			...
			if ( pEntity && pEntity->ClassNameMatches( "func_instance" ) == true )
#else
